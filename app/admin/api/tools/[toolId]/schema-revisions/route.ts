...
/**
 * POST /admin/api/tools/{toolId}/schema-revisions — 创建 ToolSchemaRevision
 * （阶段 6 S06-C02）。
 *
 ...
 *   （创建 SchemaRevision 视为对 Tool 的修改操作；与 GET/PATCH /tools/{toolId} 一致）。
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  //    创建 SchemaRevision 视为对 Tool 的修改操作（与 PATCH /tools/{toolId} 复用 scope）
...
/**
 * GET /admin/api/tools/{toolId}/schema-revisions — 列出 ToolSchemaRevision（S11-W03）。
 *
