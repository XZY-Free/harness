 ...
 *
 * Next.js 类型验证器不识别 `[failure_id]:resolve` 为标准动态段（生成 Promise<{}>），
 * 故使用 Record 宽类型；运行时 params key 为 "failure_id:resolve"。
 */
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  // Next.js 把 [failure_id]:resolve 段名作为 key；failure_id 是冒号前的部分。
  const rawValue = params["failure_id:resolve"];
  const rawSegment = typeof rawValue === "string" ? rawValue : "";
  const failureId = rawSegment.split(":")[0] ?? "";
