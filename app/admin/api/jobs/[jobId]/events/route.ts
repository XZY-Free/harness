...
/**
 * GET /admin/api/jobs/{jobId}/events — 列出 Job 事件流（S11-W04）。
 *
 * 行为：
 * - 解析 admin 主体（读操作，无需专门 action scope）。
 * - 校验 Job 存在且属于当前租户（跨租户隐藏为 404）。
 * - 支持查询参数 limit。
 * - 调用 getJobEvents（按 event_sequence 升序，跨租户隔离）。
 * - 投影为 snake_case。
 *
 * 错误映射：
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  const events = await getJobEvents(principal.tenantId, jobId, { limit });
