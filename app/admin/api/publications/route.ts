...
import { listPublicationRecords } from "@/lib/publications/persistence/publication-record-queries";
...
export async function GET(request: Request): Promise<Response> {
  ...
  const items = await listPublicationRecords({
  ...
  return apiSuccess(
    { items: items.map(projectPublicationRecord), total: items.length },
