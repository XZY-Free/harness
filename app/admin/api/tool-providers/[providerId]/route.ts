...
/**
 * GET / PATCH /admin/api/tool-providers/{providerId} — ToolProvider 单资源（阶段 6 S06-C02）。
 *
...
// ─── GET /admin/api/tool-providers/{providerId} ───────
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (!provider) {
    return resourceNotFound(requestId, `ToolProvider 不存在或无权访问: ${providerId}`);
  }

  return apiSuccess(projectProvider(provider), {
    status: 200,
    headers: {
...
// ─── PATCH /admin/api/tool-providers/{providerId} ─────
