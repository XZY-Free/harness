...
/**
 * GET /admin/api/security-incidents/{securityIncidentId}/containments — 事故隔离动作列表（S12-W09）。
 *
...
export async function GET(
  request: Request,
  context: { params: Promise<{ securityIncidentId: string }> },
): Promise<Response> {
  ...
  // 3. 解析路径参数
  const { securityIncidentId: incidentId } = await context.params;
  if (!incidentId) {
