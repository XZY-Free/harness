...
/**
 * POST /admin/api/security-incidents/{securityIncidentId}/containments/{containmentId} — 隔离动作状态推进（S12-W09）。
 *
...
export async function POST(
  request: Request,
  context: {
    params: Promise<{ securityIncidentId: string; containmentId: string }>;
  },
): Promise<Response> {
  ...
  // 3. 解析路径参数 + 请求体
  const { securityIncidentId: incidentId, containmentId } = await context.params;
  if (!incidentId) {
    return schemaInvalidTable(requestId, "缺少路径参数 securityIncidentId");
  }
  if (!containmentId) {
    return schemaInvalidTable(requestId, "缺少路径参数 containmentId");
  }
