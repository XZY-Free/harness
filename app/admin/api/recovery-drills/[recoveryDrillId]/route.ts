...
/**
 * POST/GET /admin/api/recovery-drills/{recoveryDrillId} — 恢复演练详情与执行（S12-W08）。
 *
...
export async function GET(
  request: Request,
  context: { params: Promise<{ recoveryDrillId: string }> },
): Promise<Response> {
  ...
  // 3. 解析路径参数
  const { recoveryDrillId: drillId } = await context.params;
  if (!drillId) {
    return schemaInvalidTable(requestId, "缺少路径参数 recoveryDrillId");
  }
