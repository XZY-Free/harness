...
/**
 * GET /admin/api/tool-calls/{toolCallId}/effects — 查询 ToolCall 的 EffectRecord（S11-W04）。
 *
 * 行为：
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  const body = {
    ...
    owner_kind: effect.ownerKind,
    owner_ref: effect.ownerRef,
    invocation_id: effect.invocationId,
    operation_key: effect.operationKey,
    request_digest: effect.requestDigest,
    dispatch_intent_at: effect.dispatchIntentAt?.toISOString() ?? null,
    dispatch_evidence: effect.dispatchEvidence ?? null,
