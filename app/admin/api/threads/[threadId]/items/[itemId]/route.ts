...
/**
 * GET /admin/api/threads/{threadId}/items/{itemId} — ThreadItem 单资源详情（S11-W04）。
 *
 * 行为：
...
interface RouteContext {
  params: Promise<{ threadId: string; itemId: string }>;
}

export async function GET(request: Request, context: RouteContext): Promise<Response> {
  const requestId = getRequestId(request);
  const { threadId, itemId } = await context.params;
