import { POST } from "@/app/api/v1/threads/[thread_id]/turns/route";
...
function turnRequest(): NextRequest {
  return new NextRequest("https://snow.example.com/api/v1/threads/th1/turns", {
    method: "POST",
    headers: {
...
describe("POST /api/v1/threads/{id}/turns 原子性", () => {
  ...
  it("调度成功时不做兜底收口", async () => {
    ...
    const response = await POST(turnRequest(), {
      params: Promise.resolve({ thread_id: "th1" }),
    });
