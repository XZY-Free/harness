 ...
 *
 * 事实源：../v11-agentkit-platform/11-api-and-event-boundaries.md §3.9、
 ...
 *
 * Next.js 类型验证器不识别 `[turn_id]:regenerate` 为标准动态段（生成 Promise<{}>），
 * 故使用 Record 宽类型；运行时 params key 为 "turn_id:regenerate"。
 */
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  // Next.js 把 [turn_id]:regenerate 段名作为 key；turn_id 是冒号前的部分。
  const rawValue = params["turn_id:regenerate"];
