...
function uploadRequest(file: File, idempotencyKey: string | null = "upload-test-1"): Request {
  ...
  body.set("file", file);
  return new Request("http://localhost/api/threads/test-thread-1/attachments", {
    method: "POST",
...
const context = { params: Promise.resolve({ threadId: "test-thread-1" }) };

describe("POST /api/threads/:threadId/attachments", () => {
  afterEach(() => {
    ...
    idempotencyMock.enforce.mockResolvedValue({
      ...
      record: {
        ...
        responseRedactedJson: JSON.stringify({
          ...
          url: "/api/threads/test-thread-1/attachments/attachment-existing",
          filename: "a.txt",
          size: 1,
          type: "text/plain",
        }),
  ...
  it.each([
    ...
    expect(body.url).toBe(`/api/threads/test-thread-1/attachments/${body.attachment_id}`);
    ...
    expect(attachmentRepositoryMock.create).toHaveBeenCalledWith(
      ...
      },
      {
        ...
        responseBody: expect.objectContaining({ attachmentId: body.attachment_id }),
      },
    );
