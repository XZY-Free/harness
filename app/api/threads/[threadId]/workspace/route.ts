 ...
 *
 * GET /api/threads/{threadId}/workspace → 列出本会话 workspace 中的用户可见文件。
 *
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ threadId: string }> }) {
  ...
  try {
    const files = await listWorkspaceFiles(threadId, { skipInternal: true });
    return jsonOk({ threadId: threadId, files });
  } catch (error) {
