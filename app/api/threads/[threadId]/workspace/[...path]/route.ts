 ...
 *
 * GET /api/threads/{threadId}/workspace/[...path] → 读文件内容 + stat（含 revision）。
 * GET /api/threads/{threadId}/workspace/[...path]?raw=1 → 直接返回原始字节 + Content-Type，
 *   供 `<img>` / `<iframe>` / `<source>` 等直接加载。
 * PUT /api/threads/{threadId}/workspace/[...path] → 写文件内容（revision-aware）。
 *
...
export async function GET(
  ...
  try {
    if (rawMode) {
      ...
      const preStat = await workspaceStat(threadId, relPath);
      if (preStat === null) {
        return jsonError(404, "file_not_found", "文件不存在");
      }
      ...
      const bytes = await readWorkspaceFileBytes(threadId, relPath);
      if (bytes === null) {
        return jsonError(404, "file_not_found", "文件不存在");
      }
    ...
    const stat = await workspaceStat(threadId, relPath);
    if (stat === null) {
      return jsonError(404, "file_not_found", "文件不存在");
    }
