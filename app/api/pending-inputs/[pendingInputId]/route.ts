/**
 * PATCH  /api/pending-inputs/{pendingInputId} — 编辑 PendingInput 内容（S04-C04，§3.9）。
 * DELETE /api/pending-inputs/{pendingInputId} — 移除 PendingInput（S04-C04，§3.10）。
 *
...
interface RouteContext {
  params: Promise<{ pendingInputId: string }>;
}
...
export async function PATCH(request: Request, context: RouteContext): Promise<Response> {
  ...
  const { pendingInputId } = await context.params;
