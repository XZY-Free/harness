...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (path) {
      const content = await readSkillFile(sk.name, path);
      if (content === null) return jsonError(404, "file_not_found", "文件不存在");
      return jsonOk({ path, content });
    }
    return jsonOk({ files: await listSkillFiles(sk.name) });
  } catch (e) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const denied = await assertSkillWriteAccess(sk, r.user.id);
  ...
  try {
    await writeSkillFile(sk.name, path, body.content);
  } catch (e) {
