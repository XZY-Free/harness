...
/**
 * POST /gateway/agent-calls/{callId}/cancel — 取消 AgentCall（A2A tasks/cancel）
...
function extractCallId(url: string): string | null {
  // 路径形如 /gateway/agent-calls/{callId}/cancel
  const match = url.match(/\/gateway\/agent-calls\/([^/?#]+)\/cancel/);
  const id = match?.[1];
