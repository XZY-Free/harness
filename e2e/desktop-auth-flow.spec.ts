...
import { type LaunchedDesktop, launchDesktopApp, readBrandName } from "./support/launch-desktop";
...
test.describe("Desktop 登录闭环", () => {
  ...
  test("未登录 → 失败反馈 → 登录 → 退出 → 回到登录页", async () => {
    ...
    await expect(window.getByText(brandName, { exact: true })).toHaveCount(1);
