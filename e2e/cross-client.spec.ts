...
test.describe("§20.6 跨端一致性", () => {
  ...
  test("Web 创建 Thread → Desktop 打开同一 Thread → Desktop 发 Turn → Web 收到 Event", async ({
    ...
    // Web 侧第一条回复到达。
    await expect(page.getByTestId("assistant-message").first()).toBeVisible({ timeout: 90_000 });
    await expect.poll(async () => page.getByTestId("assistant-message").count()).toBe(1);
    ...
    // 也必须看到 Web 那一轮的回复。
    await expect(desktopWindow.getByTestId("assistant-message").first()).toBeVisible({
      timeout: 90_000,
    });
    ...
    // Desktop 侧出现第二条回复。
    await expect
      .poll(async () => desktopWindow.getByTestId("assistant-message").count(), {
        timeout: 120_000,
      })
      .toBe(2);
    ...
    await expect
      .poll(async () => page.getByTestId("assistant-message").count(), { timeout: 120_000 })
      .toBe(2);
    ...
    // 但都绑定到同一条 Route / 同一 RuntimeRevision——两端共用同一执行模型（§0.8），
    // 不存在第二套控制面。ExecutionBinding 冻结架构不再携带任何 Agent evidence（字段缺席）。
    ...
    // 冻结架构（专题01 §5/§6）：两端 ExecutionBinding 都绝不携带 agent_* 字段
    // （serializeExecutionBinding 已移除，字段缺席 undefined；fresh DB 无 Agent，§24.1 count=0）。
    // 同时仍核对 Route / Runtime / Publication / Conformance 相同且非空。
    for (const [label, binding] of [
      ...
      expect(
        (binding as unknown as Record<string, unknown>).agent_revision_id,
        `${label} agent_revision_id 应缺席（字段已删）`,
      ).toBeUndefined();
      expect(
        (binding as unknown as Record<string, unknown>).agent_contract_snapshot_id,
        `${label} agent_contract_snapshot_id 应缺席`,
      ).toBeUndefined();
      expect(
        (binding as unknown as Record<string, unknown>).agent_contract_digest,
        `${label} agent_contract_digest 应缺席`,
      ).toBeUndefined();
      expect(
        (binding as unknown as Record<string, unknown>).agent_publication_record_id,
        `${label} agent_publication_record_id 应缺席`,
      ).toBeUndefined();
    ...
    console.log(
      `[e2e][cross] 两端共用控制面事实：route=${webBinding?.deployment_route_id} ` +
        `runtimeRevision=${webBinding?.runtime_revision_id}`,
    );
  });
});
