 ...
 *
 * 规则来源: SnowHarness 01专题后续四批整改总方案 §二十八
 *
 * 层次:
...
module.exports = {
  forbidden: [
    // ─── Domain 规则 ──────────────────────────────
    {
      ...
      comment: "Domain 不得依赖 Persistence",
      severity: "error",
    ...
    {
      name: "domain-no-v11",
      comment: "正式 Domain 不得依赖 lib/v11",
      severity: "error",
    ...
    // ─── Application 规则 ─────────────────────────
    {
      ...
      comment: "Application Service 不得直接依赖 MySQL Store 实现",
      severity: "error",
    ...
    // ─── API Route 规则 ──────────────────────────
    {
      ...
      comment: "API Route 不得直接依赖 Drizzle Schema",
      severity: "warn",
      from: { path: "^app/" },
    ...
    },
    {
      ...
      comment: "API Route 不得直接依赖 MySQL Store",
      severity: "warn",
      from: { path: "^app/" },
      to: { path: "^lib/[^/]+/persistence/mysql-" },
    },
    {
      ...
      comment: "API Route 不得直接使用 db client",
      severity: "warn",
      from: { path: "^app/" },
