...
//
// 生成文件（openapi.json）只能由生成器更新；--check 模式检测手工修改时失败。
// 输出固定包含 HTTP 操作数、Event 数、错误码数和一致性用例数，便于打卡留证。
//
// 用法：
//   node scripts/contracts.mjs          # 校验（CI 用）
//   node scripts/contracts.mjs --write  # 重新生成 OpenAPI 后再校验

import { spawnSync } from "node:child_process";
import { resolve } from "node:path";

const ROOT = resolve(import.meta.dirname, "..");
const SCRIPTS_DIR = resolve(ROOT, "docs/contracts/scripts");
...
//    校验器输出固定格式：“Contracts valid: N operations, N events, N errors, N conformance cases”
runPy(VALIDATOR);
