 ...
 * 大网页若抽取后仍超 toolOutputThreshold，由 V3.3a oversized tool output 机制自动摘要。
 */
...
export async function rawFetch(params: {
  ...
  // P1-15: SSRF 入口守卫——协议白名单 + 内网/元数据拒绝 + DNS rebinding 校验。
  ...
  try {
    const res = await fetchWithValidatedRedirects({
      ...
      validateRedirect: async (nextUrl) => {
        // P1-15: redirect 目标也过 SSRF 守卫(含 DNS rebinding),防 redirect 到内网/元数据绕过入口校验。
        try {
