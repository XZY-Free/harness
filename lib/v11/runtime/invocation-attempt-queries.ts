 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.4（InvocationAttempt L389-403）
...
import type { InvocationAttemptState, V11InvocationAttempt } from "@/lib/v11/schema/runtime";
import { v11InvocationAttempt } from "@/lib/v11/schema/runtime";
 ...
 */
export async function createAttempt(params: CreateAttemptParams): Promise<V11InvocationAttempt> {
  // 分配 attemptNo（COALESCE(MAX(attemptNo), 0) + 1）
  const [maxRow] = await db
    .select({ maxNo: sql<number>`COALESCE(MAX(${v11InvocationAttempt.attemptNo}), 0)` })
    .from(v11InvocationAttempt)
    .where(eq(v11InvocationAttempt.invocationId, params.invocationId));
  ...
  const [row] = await db
    .select()
    .from(v11InvocationAttempt)
    .where(eq(v11InvocationAttempt.id, attemptId))
    .limit(1);
  if (!row) {
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §9.1（事务边界）。
...
export async function createAttemptInternal(
  ...
  const attemptId = randomUUID();
  const now = new Date();
  await tx.insert(v11InvocationAttempt).values({
    id: attemptId,
    invocationId: params.invocationId,
    attemptNo,
    attemptState: "queued",
...
export async function getAttemptById(attemptId: string): Promise<V11InvocationAttempt | null> {
  const [row] = await db
    .select()
    .from(v11InvocationAttempt)
    .where(eq(v11InvocationAttempt.id, attemptId))
...
export async function getLatestAttempt(invocationId: string): Promise<V11InvocationAttempt | null> {
  const [row] = await db
    .select()
    .from(v11InvocationAttempt)
    .where(eq(v11InvocationAttempt.invocationId, invocationId))
    .orderBy(desc(v11InvocationAttempt.attemptNo))
    .limit(1);
  return row ?? null;
}

/** 列出 Invocation 的所有 Attempt（按 attemptNo 升序）。 */
export async function getAttemptsByInvocation(
  invocationId: string,
): Promise<V11InvocationAttempt[]> {
  return db
    .select()
    .from(v11InvocationAttempt)
    .where(eq(v11InvocationAttempt.invocationId, invocationId))
    .orderBy(v11InvocationAttempt.attemptNo);
}
...
export async function updateAttemptState(
  ...
  const now = new Date();
  const updates: Partial<typeof v11InvocationAttempt.$inferInsert> = {
  ...
  await tx.update(v11InvocationAttempt).set(updates).where(eq(v11InvocationAttempt.id, attemptId));
...
export async function recordAttemptHeartbeat(
  attemptId: string,
  at: Date = new Date(),
): Promise<V11InvocationAttempt | null> {
  await db
    .update(v11InvocationAttempt)
