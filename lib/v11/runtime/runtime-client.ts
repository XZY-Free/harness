...
export interface StartInvocationRequestBody {
  ...
  attempt?: {
    attempt_no: number;
    /** 重调度时平台分配的 Attempt id（关联 V11InvocationAttempt 行）。 */
    attempt_id?: string;
    /** 重调度原因码（如 infra_error / runtime_lost / requires_redispatch）。 */
    retry_reason?: string;
    /** 重调度检查点引用（必须避开已确认副作用，事实源 §4.1 L755）。 */
    checkpoint_ref?: string;
    /** 重调度时 Runtime 的 producer_sequence 起点（整个 Invocation 内连续，事实源 §6.9 L500）。 */
...
export interface ResumeInvocationResponse {
  ...
  attempt_no: number;
  /**
   * Runtime 要求平台为同一 Invocation 创建新 Attempt 重调度（事实源 §4.5 L924-928）。
   *
