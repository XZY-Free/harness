 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.9（RuntimeEventIngress L486-500）、§9.1（事务边界）
...
async function processIngressBatch(
  ...
  const dedupResults: Array<{
    ...
    hash: string;
    existing: V11RuntimeEventIngress | null;
  }> = [];
  ...
  for (const event of safeEvents) {
    ...
    const [byEventId] = await tx
      ...
      .where(
        and(
          eq(v11RuntimeEventIngress.invocationId, params.invocationId),
          eq(v11RuntimeEventIngress.producerEventId, event.producer_event_id),
        ),
      )
      .limit(1);
    ...
    const [bySeq] = await tx
      ...
      .where(
        and(
          eq(v11RuntimeEventIngress.invocationId, params.invocationId),
          eq(v11RuntimeEventIngress.producerSequence, event.producer_sequence),
        ),
      )
      .limit(1);
  ...
  for (const result of dedupResults) {
    ...
    const now = new Date();
    await tx.insert(v11RuntimeEventIngress).values({
      id: ingressId,
  ...
  for (let i = 0; i < dedupResults.length; i++) {
    ...
    if (ingressId) {
      await tx
        ...
        .where(eq(v11RuntimeEventIngress.id, ingressId));
    }
...
async function getThreadSequenceForEvent(
  ...
  const [row] = await tx
    .select({ seq: v11ThreadEvent.eventSequence })
    .from(v11ThreadEvent)
    .where(eq(v11ThreadEvent.id, threadEventId))
    .limit(1);
  return row?.seq ?? null;
}
...
async function createThreadItem(
  ...
  const [row] = await tx.select().from(v11ThreadItem).where(eq(v11ThreadItem.id, id)).limit(1);
  if (!row) {
...
async function casUpdateTurn(
  ...
  const [current] = await tx.select().from(v11Turn).where(eq(v11Turn.id, params.turnId)).limit(1);
  if (!current) {
    throw new IngressInvocationNotFoundError(params.turnId);
  }

  const now = new Date();
  const updates: Partial<typeof v11Turn.$inferInsert> = {
  ...
  const result = await tx
    .update(v11Turn)
    .set(updates)
    .where(and(eq(v11Turn.id, params.turnId), eq(v11Turn.versionNo, current.versionNo)));

  if (result[0].affectedRows === 0) {
    // CAS 失败：并发或状态已变
    const [retry] = await tx.select().from(v11Turn).where(eq(v11Turn.id, params.turnId)).limit(1);
...
export async function getIngressByProducerEventId(
  ...
  invocationId: string,
  producerEventId: string,
): Promise<V11RuntimeEventIngress | null> {
  const [row] = await db
    .select()
    .from(v11RuntimeEventIngress)
    .where(
      and(
        eq(v11RuntimeEventIngress.tenantId, tenantId),
        eq(v11RuntimeEventIngress.invocationId, invocationId),
        eq(v11RuntimeEventIngress.producerEventId, producerEventId),
      ),
    )
    .limit(1);
  return row ?? null;
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(v11RuntimeEventIngress.tenantId, tenantId),
        eq(v11RuntimeEventIngress.invocationId, invocationId),
        eq(v11RuntimeEventIngress.producerSequence, producerSequence),
      ),
    )
    .limit(1);
  return row ?? null;
...
): Promise<V11RuntimeEventIngress[]> {
  ...
  return db
    ...
    .where(
      and(
        eq(v11RuntimeEventIngress.tenantId, tenantId),
        eq(v11RuntimeEventIngress.invocationId, invocationId),
        gt(v11RuntimeEventIngress.producerSequence, afterSeq),
      ),
    )
    .orderBy(asc(v11RuntimeEventIngress.producerSequence))
    .limit(limit);
}
