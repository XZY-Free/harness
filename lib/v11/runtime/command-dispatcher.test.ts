...
import { requestInterrupt } from "@/lib/v11/conversation/interrupt-queries";
...
} from "@/lib/v11/runtime/command-dispatcher";
...
import { v11InvocationCommand, v11ThreadEvent, v11Turn } from "@/lib/v11/schema/conversation";
...
async function acceptUserMessageTurnForCmd(params: {
  ...
  const { acceptUserMessageTurn } = await import("@/lib/v11/conversation/turn-queries");
...
async function seedRunningInvocationWithRunningTurn(
  ...
  const [turnRow] = await db.select().from(v11Turn).where(eq(v11Turn.id, ctx.turnId)).limit(1);
  if (!turnRow) throw new Error(`Turn 不存在: ${ctx.turnId}`);

  await updateTurnState(ctx.tenantId, ctx.turnId, "running", turnRow.versionNo);

  const [updatedTurn] = await db.select().from(v11Turn).where(eq(v11Turn.id, ctx.turnId)).limit(1);
...
async function getCommandRow(commandId: string): Promise<V11InvocationCommand | null> {
  const [row] = await db
    .select()
    .from(v11InvocationCommand)
    .where(eq(v11InvocationCommand.id, commandId))
    .limit(1);
  return row ?? null;
}
...
async function createResumeCommand(params: {
  ...
  const { computeInvocationCommandPayloadHash } = await import(
    "@/lib/v11/conversation/regenerate-queries"
  );
  ...
  await db.insert(v11InvocationCommand).values({
    id: commandId,
...
describe("S05-C04 dispatchResumeCommand", () => {
  ...
  it("requires_redispatch=true：触发重调度（创建新 Attempt + 调用 startInvocation + 写 turn.resumed + invocation.started）", async () => {
    ...
    const attempts = await db
      ...
      .where(eq(v11InvocationAttempt.invocationId, running.invocationId));
...
describe("S05-C04 端到端 + 投影器", () => {
  ...
  it("Steer 完整链路：queue → dispatch → ack + turn.steered 投影", async () => {
    const { rebuildProjectionsForThread } = await import("@/lib/v11/conversation/projector");
    const running = await seedRunningInvocationWithRunningTurn(ctx);
    ...
    const [dbEvent] = await db
      .select()
      .from(v11ThreadEvent)
      .where(eq(v11ThreadEvent.id, steeredEvent.id))
      .limit(1);
  ...
  it("Resume 完整链路：queue → dispatch → ack + Turn/Invocation 状态推进 + 投影", async () => {
    const { rebuildProjectionsForThread } = await import("@/lib/v11/conversation/projector");
    const { v11ThreadListProjection, v11TurnTimelineProjection } = await import(
      "@/lib/v11/schema/projection"
    );
    ...
    const [listProj] = await db
      ...
      .where(eq(v11ThreadListProjection.threadId, ctx.threadId))
      .limit(1);
    ...
    const [turnProj] = await db
      ...
      .where(eq(v11TurnTimelineProjection.turnId, ctx.turnId))
      .limit(1);
