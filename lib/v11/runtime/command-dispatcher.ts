 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.10（InvocationCommand 表）、§6.2-§6.4（Invocation/Binding/Attempt）
 * - ../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.7（Steer）、§3.8（Stop/Interrupt）、§3.9（Regenerate）、§3.10（Resume）
...
import { v11InvocationCommand, v11Thread, v11Turn } from "@/lib/v11/schema/conversation";
import type { V11ExecutionBinding, V11Invocation } from "@/lib/v11/schema/runtime";
...
async function loadCommandForDispatch(tenantId: string, commandId: string): Promise<LoadedCommand> {
  ...
  const [threadRow] = await db
    .select({ tenantId: v11Thread.tenantId })
    .from(v11Thread)
    .where(eq(v11Thread.id, commandRow.threadId))
    .limit(1);
...
async function transitionCommandToDispatched(
  ...
  const result = await tx
    ...
    .where(
      and(eq(v11InvocationCommand.id, commandId), eq(v11InvocationCommand.commandState, "queued")),
    );
...
async function transitionCommandToAcknowledged(tx: Tx, commandId: string): Promise<void> {
  ...
  await tx
    ...
    .where(
      and(
        eq(v11InvocationCommand.id, commandId),
        eq(v11InvocationCommand.commandState, "dispatched"),
      ),
    );
}
 ...
 *
 * 关键约束：
 * - Steer 不创建第二个 Turn（§3.7 行 366），将 user_guidance Item 加入当前 Turn。
...
export async function dispatchSteerCommand(params: {
  ...
  runtimeEndpointResolver: (
    binding: V11ExecutionBinding,
  ) => Promise<CommandRuntimeEndpointResolution>;
...
export async function dispatchResumeCommand(params: {
  ...
  const events = await db.transaction(async (tx) => {
    ...
    const writtenEvents: V11ThreadEvent[] = [];
 ...
 *
 * 事实源：../v11-agentkit-platform/11-api-and-event-boundaries.md §4.5 L924-928、
