/**
 * V11 ExecutionBinding 仓储（S05-C01）。
 *
 * 事实源：
 ...
 *
 * 职责：
 * - createExecutionBinding：INSERT ExecutionBinding（不可变，无 update 函数）。
 * - getExecutionBindingByInvocation：按 invocationId 查询（1:1）。
 * - computeBindingConfigHash：规范化字段后 SHA-256 计算 configHash。
 *
 * 关键约束：
...
/** 按 invocationId 获取 ExecutionBinding（1:1，跨租户隔离）。不存在返回 null。 */
export async function getExecutionBindingByInvocation(
  tenantId: string,
  invocationId: string,
): Promise<V11ExecutionBinding | null> {
  const [row] = await db
    .select()
    .from(v11ExecutionBinding)
    .where(
      and(
        ...
        eq(v11ExecutionBinding.invocationId, invocationId),
      ),
    )
    .limit(1);
  return row ?? null;
...
/** 按 invocationId 获取 ExecutionBinding（无租户隔离，内部 helper）。不存在抛错。 */
export async function getExecutionBindingByInvocationOrThrow(
  invocationId: string,
): Promise<V11ExecutionBinding> {
  const [row] = await db
    .select()
    .from(v11ExecutionBinding)
    .where(eq(v11ExecutionBinding.invocationId, invocationId))
    .limit(1);
  if (!row) {
    throw new ExecutionBindingNotFoundError(invocationId);
  }
  return row;
}

// 保留 randomUUID 引用避免被误删（未来扩展时可能需要生成额外 id）。
void randomUUID;
