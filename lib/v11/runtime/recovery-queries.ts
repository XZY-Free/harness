 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.2（Invocation 状态机含 lost 终态）、
 *   §6.9（producerSequence 在整个 Invocation 内连续）、§6.11（RuntimeSessionBinding lost）、
 *   §9.1（事务边界）、§13（Worker 失联恢复：不伪造完成）
 ...
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §4.5（Resume 与 requires_redispatch）、
 *   §6.6（JobEvent 不进员工 Thread SSE）
 ...
 * - invocation.lost Event 必须形成（§6.2：Invocation 终态必须形成公开 Event）。
 * - 跨租户隔离：所有查询按 tenantId 过滤。
 * - producerSequence 在整个 Invocation 内连续，不按 Attempt 从 1 重启（§6.9）。
 */
...
import { markSessionBindingLost } from "@/lib/v11/runtime/session-binding-queries";
import type { ThreadEventActorType } from "@/lib/v11/schema/conversation";
import { v11Thread } from "@/lib/v11/schema/conversation";
import type { InvocationExecutionState } from "@/lib/v11/schema/runtime";
import {
 ...
 *
 * 事实源：§6.2（lastHeartbeatAt 字段）、§13（Worker 失联恢复）。
 *
 * 选择条件：
 * - tenantId 匹配
...
export async function findStaleInvocations(
  ...
  const rows = await db
    .select({
      ...
      tenantId: v11Invocation.tenantId,
      threadId: v11Invocation.threadId,
      turnId: v11Invocation.turnId,
      jobId: v11Invocation.jobId,
      executionState: v11Invocation.executionState,
      lastHeartbeatAt: v11Invocation.lastHeartbeatAt,
      runtimeSessionBindingId: v11Invocation.runtimeSessionBindingId,
      runtimeExecutionRef: v11Invocation.runtimeExecutionRef,
    })
    .from(v11Invocation)
    .where(
      and(
        eq(v11Invocation.tenantId, params.tenantId),
        inArray(v11Invocation.executionState, [
          "queued",
          "running",
          "waiting_user",
        ] as InvocationExecutionState[]),
        isNotNull(v11Invocation.lastHeartbeatAt),
        lt(v11Invocation.lastHeartbeatAt, threshold),
      ),
    )
    .orderBy(asc(v11Invocation.lastHeartbeatAt))
    .limit(limit);
 ...
 * + 标记关联 RuntimeSessionBinding 为 lost。
 *
 * 事实源：§6.2（lost 终态）、§9.1（事务边界：Invocation 状态写入 + Event 同事务）、
  ...
  return db.transaction(async (tx) => {
    ...
    if (updatedInvocation.threadId) {
      ...
      const [thread] = await tx
        ...
        .from(v11Thread)
        .where(eq(v11Thread.id, updatedInvocation.threadId))
        .for("update")
        .limit(1);
      if (!thread) {
 ...
 *
 * 事实源：§6.9（producerSequence 在整个 Invocation 内连续，重调度时不能从 1 重启）。
 *
 * 返回值：
...
export async function getLatestProducerSequence(
  ...
  const [row] = await db
    .select({ maxSeq: sql<number>`COALESCE(MAX(${v11RuntimeEventIngress.producerSequence}), 0)` })
    .from(v11RuntimeEventIngress)
    .where(eq(v11RuntimeEventIngress.invocationId, invocationId));
  return row?.maxSeq ?? 0;
}
