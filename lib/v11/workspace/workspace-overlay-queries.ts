 ...
 * - ../v11-agentkit-platform/10-core-data-model.md §5.5（Event 只 INSERT）、§9（事务边界）
 ...
 * - Overlay 状态变化通过 workspace_overlay.created/merged/merge_conflict/discarded ThreadEvent 记录（§5.5）。
 * - 跨租户隔离：所有查询按 tenantId 过滤。
 * - 同事务内 Overlay 状态 + MergeConflict 行 + ThreadEvent（§9.1）。
