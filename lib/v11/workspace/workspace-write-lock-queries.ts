 ...
 * - 写锁状态变化通过 workspace_write_lock.acquired/released/revoked/conflict ThreadEvent 记录（§5.5）。
 * - 跨租户隔离：所有查询按 tenantId 过滤。
 * - 同事务内写锁状态 + ThreadEvent（§9.1）。
 ...
 * 3. 若存在 released/expired/revoked 历史行，新建一行 acquired（不修改历史行，§5.5 Event 只 INSERT）。
