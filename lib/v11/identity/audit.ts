 ...
 * - 从 V11Principal/V11WorkloadPrincipal 提取 actor（与幂等账本 caller 对齐）。
...
import { AUDIT_ACTION_TYPES, type AuditActorType } from "@/lib/v11/schema/audit";
import type { AuditEvent } from "@/lib/v11/schema/audit";
...
/** 从 V11Principal（员工 Session）提取审计执行者。 */
export function actorFromPrincipal(principal: V11Principal): AuditActor {
  return {
...
/**
 * 从 V11WorkloadPrincipal（runtime/gateway/admin Service/Workload Token）提取审计执行者。
 *
 ...
 */
export function actorFromWorkloadPrincipal(principal: V11WorkloadPrincipal): AuditActor {
