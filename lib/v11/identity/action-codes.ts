/**
 * V11 稳定 Action Code 目录。
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §8.2（稳定管理动作列表）
 * - ../v11-agentkit-platform/14-production-operations-security-and-retention.md §5.1（动作目录与资源 Scope）
 *
 * 服务端只依赖 action_code + resource_scope 判断；UI 菜单权限不能代替服务端 action_code。
 * 空 allowlist、未知 action、无法解析 scope 全部拒绝。
 */
import { ResourceScopeError, type ResourceScopeType } from "@/lib/v11/identity/resource-scope";

/** 稳定管理动作目录（方案 §8.2 + §5.1 最低动作集）。 */
...
/** 每个 action code 允许的 resource scope types（方案 §5.1）。 */
...
export type { ResourceScopeType } from "@/lib/v11/identity/resource-scope";
export { ResourceScopeError } from "@/lib/v11/identity/resource-scope";
