/**
 * S02-C05：V11 管理审计账本集成测试（真实 MySQL 8）。
 *
 * 覆盖：
...
describe("V11 computeContentHash", () => {
  it("相同内容 → 相同 hash", () => {
...
describe("V11 isKnownAuditActionType", () => {
  it("目录中所有动作类型均已知（RouteRevision 接入后共 29 种）", () => {
    expect(AUDIT_ACTION_TYPES.length).toBe(29);
...
describe("V11 assertAuditActionTypeKnown", () => {
  it("已知动作不抛错", () => {
...
describe("V11 actorFromPrincipal", () => {
  it("V11Principal → actorType=user, actorId=userIdentityId", () => {
...
describe("V11 actorFromWorkloadPrincipal", () => {
...
describe("V11 recordAuditEvent", () => {
  ...
  it("actor 从 V11Principal 提取（actorFromPrincipal）写入 user 类型", async () => {
  ...
  it("所有已知动作类型均可成功写入（RouteRevision 接入后共 29 种）", async () => {
    for (const actionType of AUDIT_ACTION_TYPES) {
...
describe("V11 recordSystemAuditEvent", () => {
  let tenantId: string;
