 ...
 *         ../v11-agentkit-platform-development-plan/02-identity-authorization-and-common-ledgers.md S02-W04。
 *
 * 行为：
...
/** 从 V11Principal（员工 Session）提取幂等调用方。 */
export function callerFromPrincipal(principal: V11Principal): IdempotencyCaller {
  return {
...
/**
 * 从 V11WorkloadPrincipal（runtime/gateway/admin Service/Workload Token）提取幂等调用方。
 *
 ...
 */
export function callerFromWorkloadPrincipal(principal: V11WorkloadPrincipal): IdempotencyCaller {
...
export function buildIdempotencyErrorResponse(params: {
  ...
  const code: V11ErrorCode = "IDEMPOTENCY_CONFLICT";
  const message =
  ...
  return v11Error(code, message, { requestId: rid, details });
}
