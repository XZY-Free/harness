/**
 * S02-C01：V11 身份模块集成测试（真实 MySQL 8）。
 *
 * 覆盖：
 ...
 * - resolver：dev/trusted-headers 双模式、缺身份/缺邮箱报错、v11AuthErrorResponse 401 映射。
 */
...
} from "@/lib/v11/identity/principal-binding-queries";
import {
  V11AuthError,
  getCurrentV11Principal,
  resolveV11Principal,
  v11AuthErrorResponse,
...
import {
  ...
  getTenantById,
  getTenantByKey,
} from "@/lib/v11/identity/tenant-queries";
import {
  getUserIdentityById,
  getUserIdentityBySubject,
  getUserIdentityForTenant,
  upsertUserIdentity,
} from "@/lib/v11/identity/user-identity-queries";
...
describe("V11 user-identity-queries", () => {
...
describe("V11 principal-binding-queries", () => {
...
describe("V11 resolver", () => {
  it("dev 模式返回默认身份并创建租户/身份/绑定", async () => {
    setAuthMode("dev");
    const principal = await resolveV11Principal(new Headers(), "employee");
  ...
  it("dev 模式二次调用复用同一 userIdentityId", async () => {
    setAuthMode("dev");
    const first = await resolveV11Principal(new Headers());
    const second = await resolveV11Principal(new Headers());
  ...
  it("trusted-headers 模式从 header 解析身份", async () => {
    ...
    const principal = await resolveV11Principal(headers, "admin");
  ...
  it("trusted-headers 模式缺 externalId → V11AuthError missing_identity", async () => {
    setAuthMode("trusted-headers");
    const headers = new Headers();
    headers.set("x-snow-user-email", "bob@example.com");
    await expect(resolveV11Principal(headers)).rejects.toThrow(V11AuthError);
    try {
      await resolveV11Principal(headers);
    } catch (e) {
      expect((e as V11AuthError).code).toBe("missing_identity");
    }
  });

  it("trusted-headers 模式缺 email → V11AuthError missing_email", async () => {
    setAuthMode("trusted-headers");
    const headers = new Headers();
    headers.set("x-snow-user-id", "sso-42");
    await expect(resolveV11Principal(headers)).rejects.toThrow(V11AuthError);
    try {
      ...
      expect((e as V11AuthError).code).toBe("missing_email");
    }
  });
  ...
  it("getCurrentV11Principal dev 模式可用", async () => {
    setAuthMode("dev");
    const principal = await getCurrentV11Principal("runtime");
    expect(principal.externalSubject).toBe(DEFAULT_USER_ID);
    expect(principal.audience).toBe("runtime");
  });

  it("getCurrentV11Principal trusted-headers 模式抛 V11AuthError", async () => {
    setAuthMode("trusted-headers");
    await expect(getCurrentV11Principal()).rejects.toThrow(V11AuthError);
  });

  it("v11AuthErrorResponse 把 V11AuthError 转 401", async () => {
    const error = new V11AuthError("missing_identity", "缺少 SSO 用户标识");
    const response = v11AuthErrorResponse(error, "req_test_1");
  ...
  it("v11AuthErrorResponse 非 V11AuthError 返回 null", () => {
    const response = v11AuthErrorResponse(new Error("other"), "req_test_2");
    expect(response).toBeNull();
  });

  it("v11AuthErrorResponse 缺 requestId 时自动生成", async () => {
    const error = new V11AuthError("missing_email", "缺少邮箱");
    const response = v11AuthErrorResponse(error);
    expect(response).not.toBeNull();
