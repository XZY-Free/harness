/**
 * S02-C03：V11 动作资源授权集成测试（真实 MySQL 8）。
 *
 * 覆盖：
...
import {
  ...
  requireActionScope,
} from "@/lib/v11/identity/authorization";
...
} from "@/lib/v11/identity/resource-scope";
import {
...
describe("V11 role-action-queries", () => {
  let tenantId: string;
...
describe("V11 authorization", () => {
  ...
  it("requireActionScope V11Principal 有权 → ok", async () => {
    await grantActionBinding({
      tenantId,
      principalBindingId,
      actionCode: "agent.publish",
  ...
  it("requireActionScope V11Principal 无权 → 403 ACTION_SCOPE_DENIED", async () => {
    const principal: V11Principal = {
      tenantId,
      tenantKey: "default",
      userIdentityId,
  ...
  it("requireActionScope V11WorkloadPrincipal service 有权 → ok", () => {
    const principal: V11WorkloadPrincipal = {
      tenantId,
      audience: "admin",
      callerType: "service",
      claims: {
  ...
  it("requireActionScope V11WorkloadPrincipal service 无权 → 403", () => {
    const principal: V11WorkloadPrincipal = {
      tenantId,
      audience: "admin",
      callerType: "service",
  ...
  it("requireActionScope V11WorkloadPrincipal workload（runtime/gateway）→ 403", () => {
    const principal: V11WorkloadPrincipal = {
      tenantId,
      audience: "runtime",
      callerType: "workload",
