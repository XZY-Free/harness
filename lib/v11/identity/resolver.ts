 ...
 * V11 身份解析器。
 *
 * 从请求 header 解析可信身份 → 映射到租户内稳定 userIdentity → 返回 V11Principal。
 *
 * 安全边界：
 ...
 *
 * audience=runtime/gateway/admin 的 Workload Token 验证见 resolveV11WorkloadPrincipal。
 */
import { authConfig } from "@/lib/config";
import { DEFAULT_USER_EMAIL, DEFAULT_USER_ID, DEFAULT_USER_NAME } from "@/lib/constants";
import { type V11Audience, generateRequestId, v11Error } from "@/lib/http";
...
/** V11 可信主体：四类 API 共用的身份信息。 */
export interface V11Principal {
  tenantId: string;
  tenantKey: string;
...
export class V11AuthError extends Error {
  constructor(
...
/**
 * 把 V11AuthError 转成 401 响应；非认证错误返回 null。
 * `requestId` 来自路由入口的 getRequestId(request)，保证可跟踪。
 */
export function v11AuthErrorResponse(
  error: unknown,
...
function resolveRawIdentity(headers: Headers): {
  ...
  if (!externalSubject) {
    throw new V11AuthError("missing_identity", "缺少 SSO 用户标识");
  }
  if (!email) {
    throw new V11AuthError("missing_email", "缺少 SSO 用户邮箱");
  }
...
/**
 * 从请求解析 V11 可信主体（HTTP route 入口用）。
 *
 * 流程：ensureDefaultTenant → resolveRawIdentity → upsertUserIdentity → upsertPrincipalBinding。
 * 返回的 V11Principal 包含 tenantId 和 userIdentityId，供后续授权和业务使用。
 */
export async function resolveV11Principal(
  headers: Headers,
  audience: V11Audience = "employee",
): Promise<V11Principal> {
  const tenant = await ensureDefaultTenant();
  if (tenant.status !== "active") {
    throw new V11AuthError("tenant_suspended", "租户已被暂停");
  }
...
/**
 * 无 Request 上下文时解析 V11 主体（仅 dev 模式可用）。
 * trusted-headers 模式下因无 header 会抛 V11AuthError。
 */
export async function getCurrentV11Principal(
  audience: V11Audience = "employee",
): Promise<V11Principal> {
  return resolveV11Principal(new Headers(), audience);
}

// ─── Workload / Service Identity（S02-C02）─────────────────────

/**
 * V11 Workload 主体：runtime/gateway/admin audience 的可信身份。
 *
 * 与 V11Principal 的区别：
 ...
 */
export interface V11WorkloadPrincipal {
  tenantId: string;
  audience: V11Audience;
...
/**
 * 从 Authorization header 解析 V11 Workload 主体（runtime/gateway/admin audience 用）。
 *
 * 流程：
 * 1. extractBearerToken：提取 Bearer token；缺失抛 V11AuthError missing_identity。
 ...
 * @throws V11AuthError 缺少 token
 * @throws WorkloadTokenError Token 解析/过期/audience 不匹配
 */
export function resolveV11WorkloadPrincipal(
  headers: Headers,
  expectedAudience: "runtime" | "gateway" | "admin",
): V11WorkloadPrincipal {
  const token = extractBearerToken(headers);
  if (!token) {
    throw new V11AuthError("missing_identity", `缺少 ${expectedAudience} Workload Token`);
  }
