import { generateRequestId, v11Error } from "@/lib/http";
/**
 * V11 动作资源授权守卫。
 *
 * 事实源：
 ...
 *
 * 身份分发：
 * - V11Principal（employee/admin audience）→ 查 role_action_binding。
 * - V11WorkloadPrincipal callerType=service → 查 CICD_SERVICE_ALLOWED_ACTIONS（无 resource_scope 绑定）。
 * - V11WorkloadPrincipal callerType=workload（runtime/gateway）→ 不走 action scope（由 ExecutionBinding 约束）→ 拒绝。
 */
...
import {
  ...
  isKnownActionCode,
} from "@/lib/v11/identity/action-codes";
import type { V11Principal, V11WorkloadPrincipal } from "@/lib/v11/identity/resolver";
import type { ResourceScope } from "@/lib/v11/identity/resource-scope";
import { scopeCovers } from "@/lib/v11/identity/resource-scope";
import {
  listActiveActionBindingsForUser,
  parseBindingScope,
} from "@/lib/v11/identity/role-action-queries";
import { isServiceActionAllowed } from "@/lib/v11/identity/workload-token";
 ...
 * ```
 *
 * @param principal V11Principal（employee/admin）或 V11WorkloadPrincipal（service/workload）
 * @param request action_code + resource
 * @param requestId 请求 id（来自 getRequestId），缺省自动生成
 */
export async function requireActionScope(
  principal: V11Principal | V11WorkloadPrincipal,
  request: ActionScopeRequest,
  ...
  const code: V11ErrorCode = "ACTION_SCOPE_DENIED";
  return { ok: false, response: v11Error(code, message, { requestId }) };
}
