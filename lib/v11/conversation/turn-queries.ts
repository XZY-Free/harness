 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §5.3（Turn 表）、§9.1（接纳事务边界）
 * - ../v11-agentkit-platform/02-agent-thread-and-runtime.md §7（Turn 接纳周期）
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §3.4（创建 Turn API）
 ...
 *
 * Runtime 暂不可用不回滚（§3.4 行 256）：Turn 从 accepted 开始，后续由阶段 5 接入 Runtime。
 */
export async function acceptUserMessageTurn(params: {
  ...
  const [turn] = await db.select().from(v11Turn).where(eq(v11Turn.id, turnId)).limit(1);
  if (!turn) throw new Error(`acceptUserMessageTurn: Turn 行未找到（id=${turnId}）`);

  const [item] = await db.select().from(v11ThreadItem).where(eq(v11ThreadItem.id, itemId)).limit(1);
  if (!item) throw new Error(`acceptUserMessageTurn: ThreadItem 行未找到（id=${itemId}）`);

  const events = await db
    .select()
    .from(v11ThreadEvent)
    .where(eq(v11ThreadEvent.turnId, turnId))
...
export async function acceptJobResultTurn(params: {
  ...
  actorId: string;
  idempotencyKey?: string;
}): Promise<{ turn: V11Turn; events: V11ThreadEvent[] }> {
  const turnId = randomUUID();
  const turnAcceptedEventId = randomUUID();
...
export async function getTurnById(tenantId: string, turnId: string): Promise<V11Turn | null> {
  const [row] = await db
    .select()
    .from(v11Turn)
    ...
    .limit(1);
  return row?.V11Turn ?? null;
}

/** 列出 Thread 的 Turn（按 turn_sequence 升序）。 */
export async function getTurnsByThread(tenantId: string, threadId: string): Promise<V11Turn[]> {
  const rows = await db
    .select({ turn: v11Turn })
    .from(v11Turn)
    .innerJoin(v11Thread, eq(v11Turn.threadId, v11Thread.id))
    .where(and(eq(v11Thread.tenantId, tenantId), eq(v11Turn.threadId, threadId)))
    .orderBy(asc(v11Turn.turnSequence));
...
/** Turn 是否处于终态。 */
export function isTerminalTurn(turn: V11Turn): boolean {
