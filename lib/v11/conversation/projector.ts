...
import { EventSequenceGapError, ProjectionFailureError } from "@/lib/v11/conversation/errors";
import {
...
async function projectToThreadList(
  ...
  switch (event.eventType) {
    case "thread.created": {
      ...
      await tx
        .insert(v11ThreadListProjection)
        .values({
          threadId: shardKey,
          tenantId,
          ownerUserId,
    ...
    case "thread.deleted": {
      ...
      await tx
        .update(v11ThreadListProjection)
        .set({
          lifecycleState,
          latestEventSequence: event.eventSequence,
          latestEventId: event.id,
        ...
        .where(eq(v11ThreadListProjection.threadId, shardKey));
      break;
    }
...
async function projectToTurnTimeline(
  ...
  switch (event.eventType) {
    case "turn.accepted": {
      ...
      await tx
        .insert(v11TurnTimelineProjection)
        .values({
          turnId,
          threadId,
          tenantId,
    ...
    case "turn.resumed": {
      ...
      await tx
        .update(v11TurnTimelineProjection)
        .set(updates)
        .where(eq(v11TurnTimelineProjection.turnId, turnId));
      break;
    }
 ...
 */
export async function projectThreadEvent(event: V11ThreadEvent): Promise<void> {
 ...
 */
export async function projectThreadEvents(events: V11ThreadEvent[]): Promise<void> {
...
export async function rebuildProjectionsForThread(
  ...
  await db.transaction(async (tx) => {
    // 1. 删除投影行
    await tx.delete(v11ThreadListProjection).where(eq(v11ThreadListProjection.threadId, threadId));
    await tx
      .delete(v11TurnTimelineProjection)
      .where(eq(v11TurnTimelineProjection.threadId, threadId));
  });
  ...
  // 由于 V11ProjectionCheckpoint 没有 delete helper，这里直接用 db.delete
  const { v11ProjectionCheckpoint } = await import("@/lib/v11/schema/projection");
  await db
    .delete(v11ProjectionCheckpoint)
    .where(and(eq(v11ProjectionCheckpoint.shardKey, threadId)));
