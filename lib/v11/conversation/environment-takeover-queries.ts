...
import { type ThreadEventActorType, v11Thread } from "@/lib/v11/schema/conversation";
/**
 ...
 */
import { type V11EnvironmentLease, v11EnvironmentLease } from "@/lib/v11/schema/environment";
import {
  type ExecutionOwnershipState,
  type V11ExecutionOwnership,
  v11ExecutionOwnership,
} from "@/lib/v11/schema/runtime";
import { v11Invocation } from "@/lib/v11/schema/runtime";
import type { ToolCallState } from "@/lib/v11/schema/tool-call";
...
export interface GetTakeoverConditionsInput {
  ...
  readonly activeOwnership: V11ExecutionOwnership | null;
  /** 当前 Desktop Lease（如有）；用于回退判断 owner 设备心跳。 */
  readonly activeLease: V11EnvironmentLease | null;
}
...
export async function performTakeover(input: PerformTakeoverInput): Promise<PerformTakeoverResult> {
  return db.transaction(async (tx) => {
    ...
    const [ownershipRow] = await tx
      ...
      .where(
        and(
          eq(v11ExecutionOwnership.id, input.activeOwnershipId),
          eq(v11ExecutionOwnership.invocationId, input.activeInvocationId),
        ),
      )
      .for("update")
      .limit(1);
    ...
    const [invocationRow] = await tx
      ...
      .where(
        and(
          eq(v11Invocation.tenantId, input.tenantId),
          eq(v11Invocation.id, input.activeInvocationId),
        ),
      )
      .limit(1);
    ...
    const [threadRow] = await tx
      ...
      .from(v11Thread)
      .where(and(eq(v11Thread.tenantId, input.tenantId), eq(v11Thread.id, input.threadId)))
      .for("update")
      .limit(1);
    if (!threadRow) {
      ...
      const leaseResult = await tx
        ...
        .where(
          and(
            ...
            eq(v11EnvironmentLease.id, input.activeLeaseId),
          ),
        );
