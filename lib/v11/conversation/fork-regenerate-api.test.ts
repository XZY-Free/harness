...
async function getTurnRow(turnId: string) {
  const [row] = await db.select().from(v11Turn).where(eq(v11Turn.id, turnId)).limit(1);
  return row;
}
...
async function getTurnCommands(turnId: string) {
  return db.select().from(v11InvocationCommand).where(eq(v11InvocationCommand.turnId, turnId));
}

/** 查询 Turn 的 ThreadItem 记录。 */
async function getTurnItems(turnId: string) {
  return db.select().from(v11ThreadItem).where(eq(v11ThreadItem.turnId, turnId));
}
