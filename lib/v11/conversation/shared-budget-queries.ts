 ...
 * - ../v11-agentkit-platform/10-core-data-model.md §5.2（ThreadRelation.budgetUsedJson）
...
import type { ChildThreadBudgetUsage } from "@/lib/v11/conversation/child-thread-queries";
...
export async function getSharedBudgetUsage(parentThreadId: string): Promise<SharedBudgetUsage> {
  const relations = await db
    .select({
      id: v11ThreadRelation.id,
      budgetUsedJson: v11ThreadRelation.budgetUsedJson,
      relationState: v11ThreadRelation.relationState,
    })
