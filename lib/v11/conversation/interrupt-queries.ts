 ...
 * - Stop 不撤销已发生的副作用（tool 副作用已生效，§3.8 行 393）。
...
import { computeInvocationCommandPayloadHash } from "@/lib/v11/conversation/regenerate-queries";
...
/** Interrupt 的 reason_code（与契约 §3.8 对齐，本阶段仅记录，不强制枚举）。 */
...
export interface RequestInterruptResult {
  ...
  /** 已完成副作用是否保留（固定 true，§3.8 行 393：Stop 不撤销已发生副作用）。 */
