 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §3.2（更新 Thread 默认设置）、§3.3（更换 Thread 主 Agent）
 * - ../v11-agentkit-platform/10-core-data-model.md §9.1（事务边界：当前状态更新和 Event 追加同事务）
...
import { ThreadNotFoundError, ThreadVersionConflictError } from "@/lib/v11/conversation/errors";
...
export async function updateThreadSettingsWithEvents(params: {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.update(v11Thread).set(setClause).where(eq(v11Thread.id, params.threadId));
