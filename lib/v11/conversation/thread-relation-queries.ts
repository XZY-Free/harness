 ...
 * - ../v11-agentkit-platform-development-plan/04-thread-turn-item-and-event-core.md S04-W07
 *
 * 职责：
...
export async function createThreadRelation(params: {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(v11ThreadRelation.parentThreadId, params.parentThreadId),
        eq(v11ThreadRelation.childThreadId, params.childThreadId),
        eq(v11ThreadRelation.relationType, params.relationType),
      ),
    )
    .limit(1);
  ...
  const [row] = await db
    .select()
    .from(v11ThreadRelation)
    .where(eq(v11ThreadRelation.id, id))
    .limit(1);
  if (!row) {
...
export async function getRelationById(relationId: string): Promise<V11ThreadRelation | null> {
  const [row] = await db
    .select()
    .from(v11ThreadRelation)
...
export async function getRelationsByParent(parentThreadId: string): Promise<V11ThreadRelation[]> {
  return db
    .select()
    .from(v11ThreadRelation)
    .where(eq(v11ThreadRelation.parentThreadId, parentThreadId));
}

/** 列出 child Thread 的所有关系。 */
export async function getRelationsByChild(childThreadId: string): Promise<V11ThreadRelation[]> {
  return db
    .select()
    .from(v11ThreadRelation)
    .where(eq(v11ThreadRelation.childThreadId, childThreadId));
}
...
export async function updateRelationState(
  ...
  const setValues: Partial<typeof v11ThreadRelation.$inferInsert> = {
  ...
  await db.update(v11ThreadRelation).set(setValues).where(eq(v11ThreadRelation.id, relationId));
