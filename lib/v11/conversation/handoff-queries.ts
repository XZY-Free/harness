 ...
 * - ../v11-agentkit-platform/09-unified-domain-model.md §4.6（父子 Thread；Handoff 不创建第二个 Thread）
 * - ../v11-agentkit-platform/10-core-data-model.md §5.1（thread.primary_agent_id）、
 *   §5.2（thread_relation：handoff 不创建 ThreadRelation）、§6.8（user_action_request.purpose）
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §3.3（更换主 Agent 命令）、
 *   §3.18（解析 UserActionRequest）、§5.5（发起 UserActionRequest）、§7.2（handoff.requested / handoff.completed Event）
 ...
 * - ../v11-agentkit-platform-development-plan/09-collaboration-jobs-and-recovery.md S09-W03、S09-C03
 *
 * 职责：
 ...
 *   不经过本模块（员工主动调用即是显式确认）。
 * - Handoff 不创建第二个 Thread，不创建 ThreadRelation（09 文档 §4.6 行 254）。
...
export async function requestHandoff(params: RequestHandoffParams): Promise<RequestHandoffResult> {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await tx.insert(v11UserActionRequest).values({
      id: requestId,
...
export interface ResolveHandoffResult {
  ...
  resumeCommand: V11InvocationCommand;
 ...
 * 8. INSERT V11InvocationCommand（commandType=resume，让 Runtime 恢复执行）
 *
 * deny 流程（同事务）：
 ...
 * 5. INSERT V11InvocationCommand（commandType=resume，让 Runtime 恢复执行）
...
export async function resolveHandoff(params: ResolveHandoffParams): Promise<ResolveHandoffResult> {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (params.resolution === "approve") {
      ...
      // 8. INSERT V11InvocationCommand (resume)
    ...
    // 7. INSERT V11InvocationCommand (resume)
    const resumeCommandId = randomUUID();
    const resumePayload = {
      request_id: request.id,
      resolution: "deny",
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(v11UserActionRequest.threadId, threadId),
        eq(v11UserActionRequest.purpose, HANDOFF_PURPOSE),
        eq(v11UserActionRequest.requestState, "pending"),
      ),
    )
