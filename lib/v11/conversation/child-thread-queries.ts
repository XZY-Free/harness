 ...
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §5.2（ThreadRelation 表）、
 *   §6.2（Invocation）、§6.10 行 504（InvocationCommand 表）
 * - ../v11-agentkit-platform/05-continuity-collaboration-and-reliability.md §9（Delegate 语义、状态机）、§16（取消）、§18（预算）
 * - ../v11-agentkit-platform/12-capability-and-collaboration-api.md §4（delegateChildThread 契约）、§4.2（结果投影）、§4.3（取消协议）
 * - ../v11-agentkit-platform-development-plan/09-collaboration-jobs-and-recovery.md S09-W01/W02
 *
 * 职责：
 ...
 * - 父 Turn 只得到一个 child_thread Item 和结构化摘要，不复制子会话全文（12 文档 §4.2 行 263）。
 * - 完成投影幂等：子 Runtime 不能直接回写父 Thread；投影由平台根据子 Thread 终态生成。
 * - unknown_effect 核对责任：子任务已产生 unknown effect 时不伪造无副作用取消（§16 行 333、12 文档 §4.3 行 294）。
...
/**
 * 子 Thread 运行时预算用量累积（12 文档 §4.2 行 245 budget_used）。
 * 存储于 V11ThreadRelation.budgetUsedJson。
 */
...
/**
 * 父 Thread 中 child_thread Item 的 contentJson 结构（12 文档 §4.2 行 263）。
...
export interface ChildThreadItemContent {
  ...
  state: V11ThreadRelation["relationState"];
...
export interface FinalizeCancellationResult {
  ...
  parentCancelledEvent: V11ThreadEvent;
  /** 子 Thread 的 child_thread.cancelled Event（payload from_parent=true）。 */
  childCancelledEvent: V11ThreadEvent;
}
...
export interface DelegateChildThreadResult {
  ...
  childThreadItem: V11ThreadItem;
  /** 父 Thread 流中的 item.created Event（item_type=child_thread）。 */
  itemCreatedEvent: V11ThreadEvent;
}
...
export async function delegateChildThread(
  ...
  const [parentInvocation] = await db
    ...
    .where(
      and(
        ...
        eq(v11Invocation.id, params.parentInvocationId),
      ),
    )
    .limit(1);
  ...
  const [childThreadItem] = await db
    .select()
    .from(v11ThreadItem)
    .where(eq(v11ThreadItem.id, childThreadItemId))
    .limit(1);
  if (!childThreadItem) {
  ...
  const [itemCreatedEvent] = await db
    .select()
    .from(v11ThreadEvent)
    .where(eq(v11ThreadEvent.id, itemCreatedEventId))
    .limit(1);
  if (!itemCreatedEvent) {
...
export async function computeDelegationDepth(threadId: string): Promise<number> {
  ...
  while (currentThreadId && !visited.has(currentThreadId)) {
    ...
    const [parentRel] = await db
      ...
      .where(
        and(
          eq(v11ThreadRelation.childThreadId, currentThreadId),
          eq(v11ThreadRelation.relationType, "delegate"),
        ),
      )
      .limit(1);
    if (!parentRel) break;
...
async function validateContextTransferPolicy(
  ...
  for (const itemId of policy.includeItemIds) {
    const [item] = await db
      .select()
      .from(v11ThreadItem)
      .where(and(eq(v11ThreadItem.threadId, parentThreadId), eq(v11ThreadItem.id, itemId)))
      .limit(1);
...
export async function getChildThreadRelation(
  relationId: string,
): Promise<V11ThreadRelation | null> {
  const [row] = await db
    .select()
    .from(v11ThreadRelation)
    .where(eq(v11ThreadRelation.id, relationId))
...
export async function getRelationsByParentInvocation(
  tenantId: string,
  parentInvocationId: string,
): Promise<V11ThreadRelation[]> {
  return db
    .select()
    .from(v11ThreadRelation)
    .where(
      and(
        eq(v11ThreadRelation.sourceInvocationId, parentInvocationId),
        eq(v11ThreadRelation.relationType, "delegate"),
      ),
    )
    .orderBy(asc(v11ThreadRelation.createdAt));
}
...
export async function getDelegateRelationsByParentThread(
  ...
  return db
    ...
    .where(
      and(
        eq(v11ThreadRelation.parentThreadId, parentThreadId),
        eq(v11ThreadRelation.relationType, "delegate"),
      ),
    )
    .orderBy(asc(v11ThreadRelation.createdAt));
...
export async function getChildThreadResult(relationId: string): Promise<{
  ...
  const [item] = await db
    .select()
    .from(v11ThreadItem)
    .where(eq(v11ThreadItem.id, relation.resultItemId))
    .limit(1);
...
export interface RequestCancellationResult {
  ...
  cancelRequestedEvent: V11ThreadEvent | null;
  /**
   ...
   */
  cancelCommand: V11InvocationCommand | null;
 ...
 * 6. INSERT V11InvocationCommand（commandType=cancel，commandPayloadJson={relation_id, reason_code}）
...
export async function requestChildThreadCancellation(params: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [relation] = await tx
      .select()
      .from(v11ThreadRelation)
      .where(eq(v11ThreadRelation.id, params.relationId))
      .for("update")
      .limit(1);
    if (!relation) {
    ...
    // 3. 校验 relation_state
    const terminalStates: V11ThreadRelation["relationState"][] = [
      "completed",
      "failed",
      "cancelled",
    ];
    ...
    const [childInvocation] = await tx
      ...
      .where(
        and(
          eq(v11Invocation.tenantId, params.tenantId),
          eq(v11Invocation.threadId, relation.childThreadId),
        ),
      )
      .orderBy(desc(v11Invocation.invocationSequence))
      .limit(1);
    ...
    if (childInvocation && !INVOCATION_TERMINAL_STATES.includes(childInvocation.executionState)) {
      // 6. INSERT V11InvocationCommand（commandType=cancel）
  ...
  let cancelCommand: V11InvocationCommand | null = null;
  if (result.cancelCommandId) {
    const [cmd] = await db
      .select()
      .from(v11InvocationCommand)
      .where(eq(v11InvocationCommand.id, result.cancelCommandId))
      .limit(1);
...
/**
 * 子 Thread 终态 → 父 Thread child_thread Item 投影 result（12 文档 §4.2 行 263）。
 *
 * 事实源：
 * - 05 文档 §9 行 412-417：子 Runtime 不能直接回写父 Thread；投影由平台根据子 Thread 终态生成。
 * - 12 文档 §4.2 行 263：父 Turn 只得到一个 child_thread Item 和结构化摘要，不复制子会话全文。
 * - 12 文档 §4.3 行 294：取消协议；relation_state 由 active/cancel_requested → completed。
 *
 * 流程：
...
export async function projectChildThreadResult(
  ...
  let finalItem: V11ThreadItem | null = params.finalItem ?? null;
  if (!finalItem) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [parent] = await tx
      ...
      .from(v11Thread)
      .where(
        and(eq(v11Thread.tenantId, params.tenantId), eq(v11Thread.id, relation.parentThreadId)),
      )
      .for("update")
      .limit(1);
    if (!parent) {
    ...
    const [childInvocation] = await tx
      .select()
      .from(v11Invocation)
      .where(eq(v11Invocation.threadId, relation.childThreadId))
      .orderBy(desc(v11Invocation.invocationSequence))
      .limit(1);
    if (childInvocation && !INVOCATION_TERMINAL_STATES.includes(childInvocation.executionState)) {
    ...
    await tx
      ...
      .where(eq(v11ThreadItem.id, relation.itemId));
    ...
    await tx
      .update(v11Thread)
      .set({ lastActivityAt: now, updatedAt: now })
      .where(eq(v11Thread.id, relation.parentThreadId));
  ...
  if (result.idempotent) {
    ...
    const [item] = await db
      .select()
      .from(v11ThreadItem)
      .where(eq(v11ThreadItem.id, result.relation.itemId))
      .limit(1);
    if (!item) {
      throw new ChildThreadResultProjectionError(
        params.relationId,
    ...
    return {
      relation: result.relation,
      item,
      completedEvent: null as unknown as V11ThreadEvent, // 幂等命中不返回新 Event
  ...
  const [item] = await db
    .select()
    .from(v11ThreadItem)
    .where(eq(v11ThreadItem.id, itemIdForRead))
    .limit(1);
  if (!item) {
    throw new ChildThreadResultProjectionError(
      params.relationId,
 ...
 * 不存在返回 null。
 */
async function findLatestCompletedChildItem(childThreadId: string): Promise<V11ThreadItem | null> {
  const [row] = await db
    .select()
    .from(v11ThreadItem)
    .where(and(eq(v11ThreadItem.threadId, childThreadId), eq(v11ThreadItem.itemState, "completed")))
    .orderBy(desc(v11ThreadItem.itemSequence))
    .limit(1);
...
/**
 * finalize Child Thread 取消终态落库（05 文档 §16 行 322-333；12 文档 §4.3 行 294）。
 *
 * 事实源：
 * - 05 文档 §9 行 412-417：取消请求 ≠ 已取消；最终 cancelled 由 Runtime/应用服务在执行确认后落库。
 * - 05 文档 §16 行 333：子任务已产生 unknown effect 时不伪造无副作用取消。
 * - 12 文档 §4.3 行 294：父子流分别写 child_thread.cancelled Event；子流 payload from_parent=true。
 *
 * 流程：
...
export async function finalizeChildThreadCancellation(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [child] = await tx
      .select()
      .from(v11Thread)
      .where(eq(v11Thread.id, relation.childThreadId))
      .for("update")
      .limit(1);
    if (!child) {
      throw new ChildThreadCancellationFinalizeError(
  ...
  if (result.idempotent) {
    ...
    return {
      relation: result.relation,
      parentCancelledEvent: null as unknown as V11ThreadEvent,
      childCancelledEvent: null as unknown as V11ThreadEvent,
    };
  }
...
export async function handleChildThreadTerminal(
  ...
  const [relation] = await db
    ...
    .where(
      and(
        eq(v11ThreadRelation.childThreadId, params.childThreadId),
        eq(v11ThreadRelation.relationType, "delegate"),
      ),
    )
    .limit(1);
  ...
  if (!relation) {
    return {
      action: "skipped",
      relation: null as unknown as V11ThreadRelation,
    };
  }
  ...
  if (params.terminalState === "failed") {
    ...
    await db
      ...
      .where(eq(v11ThreadRelation.id, relation.id));
...
/**
 * 累积子 Thread 预算用量到 budgetUsedJson（12 文档 §4.2 行 245 budget_used）。
 *
 * 事实源：
 * - 05 文档 §18 行 352-362：Child Thread 共享父任务总预算；耗尽由应用服务发出 cancel command。
 * - 12 文档 §4.2 行 245：budget_used 形状 { tokens, cost, tool_calls, wall_clock_ms, unknown_effect }。
 *
 * 流程：
