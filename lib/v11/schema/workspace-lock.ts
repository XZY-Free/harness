/**
 * V11 Workspace 并发控制 schema：workspace_write_lock / workspace_overlay /
...
import { workspaceBinding } from "@/lib/v11/schema/workspace";
 ...
 */
export const WORKSPACE_OVERLAY_STATES = [
  "active",
  "merged",
  "conflict",
  "discarded",
 ...
 */
export const WORKSPACE_MERGE_CONFLICT_STATES = [
  "reported",
  "resolved",
  "abandoned",
] as const;
...
export const workspaceWriteLock = mysqlTable(
  "V11WorkspaceWriteLock",
  {
    ...
    lockState: mysqlEnum("lockState", WORKSPACE_WRITE_LOCK_STATES)
      .notNull()
      .default("acquired"),
    /** 获取时间。 */
  ...
  (t) => ({
    ...
    tenantBindingPathIdx: uniqueIndex("V11WorkspaceWriteLock_tenant_binding_path_idx").on(
      t.tenantId,
      t.workspaceBindingId,
      t.pathFingerprint,
    ),
    tenantHolderIdx: index("V11WorkspaceWriteLock_tenant_holder_idx").on(
      t.tenantId,
      t.holderInvocationId,
    ),
    tenantStateIdx: index("V11WorkspaceWriteLock_tenant_state_idx").on(t.tenantId, t.lockState),
    tenantExpiryIdx: index("V11WorkspaceWriteLock_tenant_expiry_idx").on(t.tenantId, t.expiresAt),
  }),
);
...
export const workspaceOverlay = mysqlTable(
  "V11WorkspaceOverlay",
  {
    ...
    relationId: varchar("relationId", { length: 36 })
      .notNull()
      .references(() => v11ThreadRelation.id),
    ...
    overlayState: mysqlEnum("overlayState", WORKSPACE_OVERLAY_STATES)
      .notNull()
      .default("active"),
  ...
  (t) => ({
    ...
    tenantBindingRelationUq: uniqueIndex(
      "V11WorkspaceOverlay_tenant_binding_relation_uq",
    ).on(t.tenantId, t.parentWorkspaceBindingId, t.relationId),
    tenantStateIdx: index("V11WorkspaceOverlay_tenant_state_idx").on(t.tenantId, t.overlayState),
    tenantRelationIdx: index("V11WorkspaceOverlay_tenant_relation_idx").on(t.tenantId, t.relationId),
  }),
);
...
export const workspaceMergeConflict = mysqlTable(
  ...
  (t) => ({
    tenantOverlayIdx: index("V11WorkspaceMergeConflict_tenant_overlay_idx").on(
      t.tenantId,
      t.overlayId,
    ),
    tenantStateIdx: index("V11WorkspaceMergeConflict_tenant_state_idx").on(
      t.tenantId,
      t.conflictState,
    ),
  }),
...
export type V11WorkspaceWriteLock = InferSelectModel<typeof workspaceWriteLock>;
export type V11WorkspaceWriteLockInsert = InferInsertModel<typeof workspaceWriteLock>;

export type V11WorkspaceOverlay = InferSelectModel<typeof workspaceOverlay>;
export type V11WorkspaceOverlayInsert = InferInsertModel<typeof workspaceOverlay>;

export type V11WorkspaceMergeConflict = InferSelectModel<typeof workspaceMergeConflict>;
export type V11WorkspaceMergeConflictInsert = InferInsertModel<typeof workspaceMergeConflict>;
