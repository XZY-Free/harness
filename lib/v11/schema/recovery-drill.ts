/**
 * V11 schema：备份恢复演练与一致性核对（S12-W08）。
 *
 ...
 *
 * 表语义：
 * - V11RecoveryDrill：恢复演练主体。记录演练类型/RPO/RTO 目标/执行人/状态机/一致性汇总/审计事件 id。
 *   drillState 推进：scheduled → running → completed / failed / cancelled。
 *   演练必须在隔离环境执行，记录 environment_tag（如 isolated-staging-001）。
 * - V11RecoveryDrillCheck：每次演练下的一致性检查项。按 (drillId, checkType) 唯一。
 ...
 *     验证 V11 Runtime/Job 的恢复机制（recovery-queries / redispatch-queries）正确处理：
...
export const v11RecoveryDrill = mysqlTable(
  "V11RecoveryDrill",
  {
  ...
  (t) => ({
    tenantScheduledIdx: index("V11RecoveryDrill_tenant_scheduled_idx").on(
      t.tenantId,
      t.scheduledAt,
    ),
    tenantStateIdx: index("V11RecoveryDrill_tenant_state_idx").on(t.tenantId, t.drillState),
    tenantTypeIdx: index("V11RecoveryDrill_tenant_type_idx").on(t.tenantId, t.drillType),
    tenantExecutedByIdx: index("V11RecoveryDrill_tenant_executed_by_idx").on(
      t.tenantId,
      t.executedBy,
    ),
  }),
...
export type V11RecoveryDrill = InferSelectModel<typeof v11RecoveryDrill>;
export type NewV11RecoveryDrill = InferInsertModel<typeof v11RecoveryDrill>;

// ─── V11RecoveryDrillCheck 表 ──────────────────────────────

export const v11RecoveryDrillCheck = mysqlTable(
  "V11RecoveryDrillCheck",
  {
    ...
    drillId: varchar("drillId", { length: 36 })
      .notNull()
      .references(() => v11RecoveryDrill.id),
    /** 检查类型（9 类一致性维度）。 */
  ...
  (t) => ({
    drillCheckUq: uniqueIndex("V11RecoveryDrillCheck_drill_check_uq").on(t.drillId, t.checkType),
    tenantDrillIdx: index("V11RecoveryDrillCheck_tenant_drill_idx").on(t.tenantId, t.drillId),
    drillStateIdx: index("V11RecoveryDrillCheck_drill_state_idx").on(t.drillId, t.checkState),
  }),
);

export type V11RecoveryDrillCheck = InferSelectModel<typeof v11RecoveryDrillCheck>;
export type NewV11RecoveryDrillCheck = InferInsertModel<typeof v11RecoveryDrillCheck>;
