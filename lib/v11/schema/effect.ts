/**
 * V11 副作用核对账本 schema：EffectRecord + EffectTarget（阶段 8 S08-C05）。
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.7（effect_record 与 effect_target）、
 *   §6.6（tool_call.call_state 与 effect_state 同步）、§5.5（ToolCall、Effect 与 Credential）。
 * - ../v11-agentkit-platform/09-unified-domain-model.md §5.5、§10 第 9 条
 *   （unknown_effect 不自动重放）。
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §5.2（Gateway 即时核对）、
 *   §6.5（Admin 长期核对 + 同事务更新 tool_call.call_state + AuditEvent）。
 ...
 *   ThreadEvent/JobEvent + AuditEvent（§6.5；本阶段仅实现仓储层原子更新，
 ...
 */
export const v11EffectRecord = mysqlTable(
  "V11EffectRecord",
  {
    ...
    /** 所属 ToolCall id（逻辑外键 → V11ToolCall.id；一对一）。 */
  ...
  (t) => ({
    toolCallUq: uniqueIndex("V11EffectRecord_toolCall_uq").on(t.toolCallId),
    tenantToolCallIdx: index("V11EffectRecord_tenant_toolCall_idx").on(t.tenantId, t.toolCallId),
    tenantStateIdx: index("V11EffectRecord_tenant_state_idx").on(t.tenantId, t.effectState),
  }),
);

export type V11EffectRecord = InferSelectModel<typeof v11EffectRecord>;
export type NewV11EffectRecord = InferInsertModel<typeof v11EffectRecord>;

// ─── EffectTarget 表 ─────────────────────────────────────

/**
 * EffectTarget 表：EffectRecord 的逐目标明细（§6.7）。
 *
 * 关键约束：
 * - UNIQUE(effectRecordId, targetHash)：同 EffectRecord 内同目标不重复。
 * - effectRecordId 外键 → V11EffectRecord.id ON DELETE CASCADE。
 ...
 */
export const v11EffectTarget = mysqlTable(
  "V11EffectTarget",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    tenantId: varchar("tenantId", { length: 36 })
      .notNull()
      .references(() => tenant.id),
    /** 所属 EffectRecord id（外键 → V11EffectRecord.id ON DELETE CASCADE）。 */
    effectRecordId: varchar("effectRecordId", { length: 36 })
      .notNull()
      .references(() => v11EffectRecord.id),
  ...
  (t) => ({
    recordTargetHashUq: uniqueIndex("V11EffectTarget_record_targetHash_uq").on(
      t.effectRecordId,
      t.targetHash,
    ),
    tenantRecordIdx: index("V11EffectTarget_tenant_record_idx").on(t.tenantId, t.effectRecordId),
    tenantStateIdx: index("V11EffectTarget_tenant_state_idx").on(t.tenantId, t.targetState),
  }),
);

export type V11EffectTarget = InferSelectModel<typeof v11EffectTarget>;
export type NewV11EffectTarget = InferInsertModel<typeof v11EffectTarget>;
