 ...
 *
 * 表语义：
 * - V11DeletionRequest：删除请求主体。记录 subject/mode/reason/请求人/状态机/阻塞原因/审计事件 id。
 *   requestState 推进：planning → blocked_by_hold（Legal Hold 阻止）/ deleting → completed/partial/failed。
 * - V11DeletionStep：每个存储 Adapter 的删除步骤。按 (requestId, storeType, subjectRef) 唯一。
...
// ─── V11DeletionRequest 表 ─────────────────────────────────

export const v11DeletionRequest = mysqlTable(
  "V11DeletionRequest",
  {
  ...
  (t) => ({
    tenantSubjectIdx: index("V11DeletionRequest_tenant_subject_idx").on(
      t.tenantId,
      t.subjectType,
      t.subjectId,
    ),
    tenantStateIdx: index("V11DeletionRequest_tenant_state_idx").on(t.tenantId, t.requestState),
    tenantRequestedByIdx: index("V11DeletionRequest_tenant_requested_by_idx").on(
      t.tenantId,
      t.requestedBy,
    ),
  }),
...
export type V11DeletionRequest = InferSelectModel<typeof v11DeletionRequest>;
export type NewV11DeletionRequest = InferInsertModel<typeof v11DeletionRequest>;

// ─── V11DeletionStep 表 ────────────────────────────────────

export const v11DeletionStep = mysqlTable(
  "V11DeletionStep",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    requestId: varchar("requestId", { length: 36 })
      .notNull()
      .references(() => v11DeletionRequest.id),
    /** 存储类型（5 类 Adapter 之一）。 */
  ...
  (t) => ({
    requestStoreSubjectUq: uniqueIndex("V11DeletionStep_request_store_subject_uq").on(
      t.requestId,
      t.storeType,
      t.subjectRef,
    ),
    tenantRequestIdx: index("V11DeletionStep_tenant_request_idx").on(t.tenantId, t.requestId),
    requestStateIdx: index("V11DeletionStep_request_state_idx").on(t.requestId, t.stepState),
  }),
);

export type V11DeletionStep = InferSelectModel<typeof v11DeletionStep>;
export type NewV11DeletionStep = InferInsertModel<typeof v11DeletionStep>;
