/**
 * V11 会话域 schema：Thread、Turn、ThreadItem、Goal、ThreadRelation、ThreadEvent。
 *
 * 事实源：
 ...
 * - ../v11-agentkit-platform-development-plan/04-thread-turn-item-and-event-core.md S04-W01/W02/W03/W04
 *
 * 关键约束：
...
export const v11Thread = mysqlTable(
  "V11Thread",
  {
    ...
    /** 当前主 Agent 稳定身份（逻辑外键 → V11Agent.id）。 */
    ...
    /** 当前 active Goal id（逻辑外键 → V11Goal.id）。 */
  ...
  (t) => ({
    tenantOwnerLifecycleActivityIdx: index("V11Thread_tenant_owner_lifecycle_activity_idx").on(
      t.tenantId,
      t.ownerUserId,
      t.lifecycleState,
      t.lastActivityAt,
    ),
    tenantAgentActivityIdx: index("V11Thread_tenant_agent_activity_idx").on(
      t.tenantId,
      t.primaryAgentId,
      t.lastActivityAt,
    ),
...
export type V11Thread = InferSelectModel<typeof v11Thread>;
export type NewV11Thread = InferInsertModel<typeof v11Thread>;

// ─── Turn ───────────────────────────────────────────────────

export const v11Turn = mysqlTable(
  "V11Turn",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    /** 用户触发时指向 user_message Item（逻辑外键 → V11ThreadItem.id）。 */
    ...
    /** 当前正式回答或结果（逻辑外键 → V11ThreadItem.id）。 */
  ...
  (t) => ({
    threadSequenceUq: uniqueIndex("V11Turn_thread_sequence_uq").on(t.threadId, t.turnSequence),
    threadStateAcceptedIdx: index("V11Turn_thread_state_accepted_idx").on(
      t.threadId,
      t.turnState,
      t.acceptedAt,
    ),
...
export type V11Turn = InferSelectModel<typeof v11Turn>;
export type NewV11Turn = InferInsertModel<typeof v11Turn>;

// ─── ThreadItem ─────────────────────────────────────────────

export const v11ThreadItem = mysqlTable(
  "V11ThreadItem",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    threadId: varchar("threadId", { length: 36 })
      .notNull()
      .references(() => v11Thread.id),
    /** Item 必须属于 Turn（逻辑外键 → V11Turn.id）。 */
    ...
    /** 被替代关系（逻辑外键 → V11ThreadItem.id）；不得成环。 */
  ...
  (t) => ({
    threadSequenceUq: uniqueIndex("V11ThreadItem_thread_sequence_uq").on(
      t.threadId,
      t.itemSequence,
    ),
    threadTurnSequenceIdx: index("V11ThreadItem_thread_turn_sequence_idx").on(
      t.threadId,
      t.turnId,
      t.itemSequence,
    ),
    invocationIdx: index("V11ThreadItem_invocation_idx").on(t.invocationId),
  }),
);

export type V11ThreadItem = InferSelectModel<typeof v11ThreadItem>;
export type NewV11ThreadItem = InferInsertModel<typeof v11ThreadItem>;

// ─── ThreadEvent ────────────────────────────────────────────

export const v11ThreadEvent = mysqlTable(
  "V11ThreadEvent",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    /** 关联 Turn（逻辑外键 → V11Turn.id）。 */
    turnId: varchar("turnId", { length: 36 }),
    /** 关联 Item（逻辑外键 → V11ThreadItem.id）。 */
  ...
  (t) => ({
    threadSequenceUq: uniqueIndex("V11ThreadEvent_thread_sequence_uq").on(
      t.threadId,
      t.eventSequence,
    ),
    threadIdempotencyUq: uniqueIndex("V11ThreadEvent_thread_idempotency_uq").on(
      t.threadId,
      t.idempotencyKey,
    ),
    threadOccurredIdIdx: index("V11ThreadEvent_thread_occurred_id_idx").on(
      t.threadId,
      t.occurredAt,
      t.id,
    ),
    turnSequenceIdx: index("V11ThreadEvent_turn_sequence_idx").on(t.turnId, t.eventSequence),
    invocationSequenceIdx: index("V11ThreadEvent_invocation_sequence_idx").on(
      t.invocationId,
      t.eventSequence,
    ),
  }),
...
export type V11ThreadEvent = InferSelectModel<typeof v11ThreadEvent>;
export type NewV11ThreadEvent = InferInsertModel<typeof v11ThreadEvent>;

// ─── Goal ───────────────────────────────────────────────────

export const v11Goal = mysqlTable(
  "V11Goal",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    threadStateIdx: index("V11Goal_thread_state_idx").on(t.threadId, t.goalState),
  }),
);

export type V11Goal = InferSelectModel<typeof v11Goal>;
export type NewV11Goal = InferInsertModel<typeof v11Goal>;

// ─── ThreadRelation ─────────────────────────────────────────

export const v11ThreadRelation = mysqlTable(
  "V11ThreadRelation",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    parentChildTypeUq: uniqueIndex("V11ThreadRelation_parent_child_type_uq").on(
      t.parentThreadId,
      t.childThreadId,
      t.relationType,
    ),
    parentStateIdx: index("V11ThreadRelation_parent_state_idx").on(
      t.parentThreadId,
      t.relationState,
    ),
    childIdx: index("V11ThreadRelation_child_idx").on(t.childThreadId),
  }),
);

export type V11ThreadRelation = InferSelectModel<typeof v11ThreadRelation>;
export type NewV11ThreadRelation = InferInsertModel<typeof v11ThreadRelation>;
...
/**
 * V11PendingInput 表：Thread 内待接纳输入队列。
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §5.6（PendingInput 表，行 324-339）
 * - ../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.14（创建 PendingInput 不生成 user_message Item）
 * - ../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.17（删除 PendingInput 不生成 user_message Item）
 *
 * 关键约束：
 ...
 */
export const v11PendingInput = mysqlTable(
  "V11PendingInput",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    /** 队列排序位置；DECIMAL(20,10) 与契约 §5.6 行 327 对齐。 */
    ...
    /** 接纳后指向 Turn（逻辑外键 → V11Turn.id）。 */
    admittedTurnId: varchar("admittedTurnId", { length: 36 }),
    /** 接纳后指向 user_message/user_guidance Item（逻辑外键 → V11ThreadItem.id）。 */
  ...
  (t) => ({
    threadClientMessageUq: uniqueIndex("V11PendingInput_thread_client_message_uq").on(
      t.threadId,
      t.clientMessageId,
    ),
    threadStatePositionIdx: index("V11PendingInput_thread_state_position_idx").on(
      t.threadId,
      t.inputState,
      t.queuePosition,
    ),
...
export type V11PendingInput = InferSelectModel<typeof v11PendingInput>;
export type NewV11PendingInput = InferInsertModel<typeof v11PendingInput>;
 ...
 * - failed：Runtime 拒绝或执行失败（不能伪装成执行完成，§3.7 行 366）。
...
/**
 * V11InvocationCommand 表：员工命令入队（Steer/Interrupt/Regenerate）。
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §6.10 行 504（InvocationCommand 表）
 * - ../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.7（Steer）、§3.8（Stop/Interrupt）、§3.9（Regenerate）
 *
 * 关键约束：
 ...
 */
export const v11InvocationCommand = mysqlTable(
  ...
  (t) => ({
    threadTurnIdx: index("V11InvocationCommand_thread_turn_idx").on(t.threadId, t.turnId),
    invocationIdx: index("V11InvocationCommand_invocation_idx").on(t.invocationId),
    threadIdempotencyUq: uniqueIndex("V11InvocationCommand_thread_idempotency_uq").on(
      t.threadId,
      t.idempotencyKey,
    ),
  }),
...
export type V11InvocationCommand = InferSelectModel<typeof v11InvocationCommand>;
export type NewV11InvocationCommand = InferInsertModel<typeof v11InvocationCommand>;
