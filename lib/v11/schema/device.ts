/**
 * V11 设备 schema：从 V10 DesktopDevice 迁移的多端可信设备身份。
 *
 * 阶段 2（S02-W02）：建立 Desktop 请求的设备签名校验与租户/员工绑定。
 *
 * 与 V10 DesktopDevice 的差异（10-core-data-model.md §2.1、§10 迁移映射）：
 * - 新增 tenantId（V10 缺失，V11 必填）。
 * - deviceId → deviceKey，并改为 UNIQUE(tenantId, deviceKey)（V10 是全局 UNIQUE(deviceId)）。
 * - name → deviceName、version → appVersion。
 * - status → deviceState（V11 公共字段规则禁止裸 status 列，10-core-data-model.md:28）。
 * - userId 引用 UserIdentity 而非旧 User 表（V11 不再直接引用旧 User 表）。
 *
 * 私钥只在 Desktop Keychain，不写 DB（与 V10 一致）。
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §2.1。
 */
import { randomUUID } from "node:crypto";
import { tenant, userIdentity } from "@/lib/v11/schema/identity";
