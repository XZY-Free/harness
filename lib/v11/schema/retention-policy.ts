/**
 * V11 schema：数据保留策略与 Legal Hold（S12-W06）。
 *
 ...
 *
 * 表语义：
 * - V11RetentionPolicy：按 (tenantId, objectType) 唯一的保留策略。retentionDays 决定过期清理；
 *   legalHoldDays（可空）为 Legal Hold 解除后的额外保留窗口。dataClass 与 statutoryRequirements
 *   用于解析适用策略（不把一个天数硬编码到所有存储）。
 * - V11LegalHold：按 (tenantId, targetType, targetId) 唯一的 Legal Hold 记录。holdState=active
...
// ─── V11RetentionPolicy 表 ──────────────────────────────────

export const v11RetentionPolicy = mysqlTable(
  "V11RetentionPolicy",
  {
  ...
  (t) => ({
    tenantObjectUq: uniqueIndex("V11RetentionPolicy_tenant_object_uq").on(t.tenantId, t.objectType),
    tenantDataClassIdx: index("V11RetentionPolicy_tenant_data_class_idx").on(
      t.tenantId,
      t.dataClass,
    ),
  }),
...
export type V11RetentionPolicy = InferSelectModel<typeof v11RetentionPolicy>;
export type NewV11RetentionPolicy = InferInsertModel<typeof v11RetentionPolicy>;

// ─── V11LegalHold 表 ────────────────────────────────────────

export const v11LegalHold = mysqlTable(
  "V11LegalHold",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    tenantTargetUq: uniqueIndex("V11LegalHold_tenant_target_uq").on(
      t.tenantId,
      t.targetType,
      t.targetId,
    ),
    tenantStateIdx: index("V11LegalHold_tenant_state_idx").on(t.tenantId, t.holdState),
    validUntilIdx: index("V11LegalHold_valid_until_idx").on(t.validUntil),
  }),
);

export type V11LegalHold = InferSelectModel<typeof v11LegalHold>;
export type NewV11LegalHold = InferInsertModel<typeof v11LegalHold>;
