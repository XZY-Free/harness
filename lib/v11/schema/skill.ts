/**
 * V11 控制面 schema：Skill 与 SkillVersion（阶段 6 S06-C01）。
 *
 * 事实源：阶段 6 Skill/Capability 模型（参考 V11Agent / V11AgentRevision 结构）。
 *
...
export const v11Skill = mysqlTable(
  "V11Skill",
  {
    ...
    /** 当前生效 SkillVersion id（逻辑外键 → V11SkillVersion.id）；null 表示未发布。 */
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11Skill_tenant_skillKey_uq").on(t.tenantId, t.skillKey),
    tenantLifecycleUpdatedIdx: index("V11Skill_tenant_lifecycle_updated_idx").on(
      t.tenantId,
      t.lifecycleState,
      t.updatedAt,
    ),
...
export type V11Skill = InferSelectModel<typeof v11Skill>;
export type NewV11Skill = InferInsertModel<typeof v11Skill>;

// ─── SkillVersion ──────────────────────────────────────────

export const v11SkillVersion = mysqlTable(
  "V11SkillVersion",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    skillVersionNoUq: uniqueIndex("V11SkillVersion_skill_versionNo_uq").on(t.skillId, t.versionNo),
    skillStateIdx: index("V11SkillVersion_skill_state_idx").on(t.skillId, t.revisionState),
  }),
);

export type V11SkillVersion = InferSelectModel<typeof v11SkillVersion>;
export type NewV11SkillVersion = InferInsertModel<typeof v11SkillVersion>;
