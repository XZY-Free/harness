/**
 * V11 文件系统 Checkpoint schema：FilesystemCheckpoint（阶段 8 S08-C06）。
 *
 * 事实源：
 ...
 *
 * 与 lib/v11/schema/context-checkpoint.ts（V11ContextCheckpoint 上下文组装/压缩点）是不同表：
...
// ─── V11FilesystemCheckpoint 表 ──────────────────────────

/**
 * V11FilesystemCheckpoint 表：文件系统状态恢复点（§7.3）。
 *
 * 关键约束：
 * - workspaceBindingId 外键 → V11WorkspaceBinding(id) ON DELETE CASCADE。
 ...
 * - 写入后不可变。
 */
export const v11FilesystemCheckpoint = mysqlTable(
  ...
  (t) => ({
    tenantBindingIdx: index("V11FilesystemCheckpoint_tenant_binding_idx").on(
      t.tenantId,
      t.workspaceBindingId,
    ),
    tenantInvocationIdx: index("V11FilesystemCheckpoint_tenant_invocation_idx").on(
      t.tenantId,
      t.invocationId,
    ),
    tenantExpiresIdx: index("V11FilesystemCheckpoint_tenant_expires_idx").on(
      t.tenantId,
      t.expiresAt,
    ),
  }),
...
export type V11FilesystemCheckpoint = InferSelectModel<typeof v11FilesystemCheckpoint>;
export type NewV11FilesystemCheckpoint = InferInsertModel<typeof v11FilesystemCheckpoint>;
