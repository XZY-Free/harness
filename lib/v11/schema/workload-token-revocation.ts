/**
 * V11 schema：Workload Token 撤销表（S12-W05）。
 *
 * 事实源：../v11-agentkit-platform/14-production-operations-security-and-retention.md §5、
 *         ../v11-agentkit-platform/11-api-and-event-boundaries.md §9.1。
 *
...
export const v11WorkloadTokenRevocation = mysqlTable(
  ...
  (t) => ({
    tenantJtiUq: uniqueIndex("V11WorkloadTokenRevocation_tenant_jti_uq").on(t.tenantId, t.jti),
    tenantRevokedIdx: index("V11WorkloadTokenRevocation_tenant_revoked_idx").on(
      t.tenantId,
      t.revokedAt,
    ),
    expiresIdx: index("V11WorkloadTokenRevocation_expires_idx").on(t.expiresAt),
  }),
);

export type V11WorkloadTokenRevocation = InferSelectModel<typeof v11WorkloadTokenRevocation>;
export type NewV11WorkloadTokenRevocation = InferInsertModel<typeof v11WorkloadTokenRevocation>;
