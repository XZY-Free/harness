/**
 * V11 Memory schema：memory_candidate / memory_entry / memory_source / memory_index 表。
 *
 * 事实源：
 * - ../v11-agentkit-platform/10-core-data-model.md §7.5（Memory 与知识索引表）、§2（命名与公共字段）。
...
export const memoryCandidate = mysqlTable(
  ...
  (t) => ({
    ...
    candidateKeyUq: uniqueIndex("V11MemoryCandidate_candidateKey_uq").on(t.candidateKey),
    tenantInvocationIdx: index("V11MemoryCandidate_tenant_invocation_idx").on(
      t.tenantId,
      t.invocationId,
    ),
    tenantStateProposedIdx: index("V11MemoryCandidate_tenant_state_proposed_idx").on(
      t.tenantId,
      t.candidateState,
      t.proposedAt,
    ),
    tenantScopeIdx: index("V11MemoryCandidate_tenant_scope_idx").on(
      t.tenantId,
      t.proposedScopeType,
      t.proposedScopeRef,
    ),
    exactlyOneSourceCk: check(
      "V11MemoryCandidate_exactly_one_source_ck",
      sql`((${t.sourceItemId} IS NOT NULL) + (${t.sourceJobId} IS NOT NULL) + (${t.sourceArtifactId} IS NOT NULL)) = 1`,
    ),
    acceptedEntryCk: check(
      "V11MemoryCandidate_accepted_entry_ck",
      sql`(${t.candidateState} <> 'accepted' OR ${t.resolvedMemoryEntryId} IS NOT NULL)`,
    ),
    rejectedEntryCk: check(
      "V11MemoryCandidate_rejected_entry_ck",
...
export const memoryEntry = mysqlTable(
  ...
  (t) => ({
    entryKeyUq: uniqueIndex("V11MemoryEntry_entryKey_uq").on(t.entryKey),
    tenantScopeIdx: index("V11MemoryEntry_tenant_scope_idx").on(
      t.tenantId,
      t.scopeType,
      t.scopeRef,
    ),
    tenantStateUpdatedIdx: index("V11MemoryEntry_tenant_state_updated_idx").on(
      t.tenantId,
      t.memoryState,
      t.updatedAt,
    ),
    tenantContentHashIdx: index("V11MemoryEntry_tenant_contentHash_idx").on(
      t.tenantId,
      t.contentHash,
    ),
  }),
...
export const memorySource = mysqlTable(
  ...
  (t) => ({
    ...
    entrySourceUq: uniqueIndex("V11MemorySource_entry_type_id_hash_uq").on(
      t.memoryEntryId,
      t.sourceType,
      t.sourceId,
      t.sourceHash,
    ),
    candidateIdx: index("V11MemorySource_candidate_idx").on(t.memoryCandidateId),
  }),
);
...
export const memoryIndex = mysqlTable(
  ...
  (t) => ({
    ...
    entryProviderUq: uniqueIndex("V11MemoryIndex_entry_provider_uq").on(
      t.memoryEntryId,
      t.indexProvider,
    ),
  }),
...
export type V11MemoryCandidate = InferSelectModel<typeof memoryCandidate>;
export type NewV11MemoryCandidate = InferInsertModel<typeof memoryCandidate>;
export type V11MemoryEntry = InferSelectModel<typeof memoryEntry>;
export type NewV11MemoryEntry = InferInsertModel<typeof memoryEntry>;
export type V11MemorySource = InferSelectModel<typeof memorySource>;
export type NewV11MemorySource = InferInsertModel<typeof memorySource>;
export type V11MemoryIndex = InferSelectModel<typeof memoryIndex>;
export type NewV11MemoryIndex = InferInsertModel<typeof memoryIndex>;
