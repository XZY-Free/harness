/**
 * V11 控制面 schema：Connection / CredentialRef / ToolProvider / Tool / ToolSchemaRevision
 * （阶段 6 S06-C02）。
 *
 * 事实源：
...
export const v11Connection = mysqlTable(
  "V11Connection",
  {
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11Connection_tenant_connectionKey_uq").on(
      t.tenantId,
      t.connectionKey,
    ),
    tenantLifecycleUpdatedIdx: index("V11Connection_tenant_lifecycle_updated_idx").on(
      t.tenantId,
      t.lifecycleState,
      t.updatedAt,
    ),
...
export type V11Connection = InferSelectModel<typeof v11Connection>;
export type NewV11Connection = InferInsertModel<typeof v11Connection>;

// ─── CredentialRef ────────────────────────────────────────

export const v11CredentialRef = mysqlTable(
  "V11CredentialRef",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    connectionId: varchar("connectionId", { length: 36 }).references(() => v11Connection.id),
  ...
  (t) => ({
    tenantConnectionIdx: index("V11CredentialRef_tenant_connectionId_idx").on(
      t.tenantId,
      t.connectionId,
    ),
    tenantFingerprintIdx: index("V11CredentialRef_tenant_fingerprint_idx").on(
      t.tenantId,
      t.fingerprint,
    ),
  }),
...
export type V11CredentialRef = InferSelectModel<typeof v11CredentialRef>;
export type NewV11CredentialRef = InferInsertModel<typeof v11CredentialRef>;

// ─── ToolProvider ─────────────────────────────────────────

export const v11ToolProvider = mysqlTable(
  "V11ToolProvider",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11ToolProvider_tenant_providerKey_uq").on(t.tenantId, t.providerKey),
    tenantTypeLifecycleIdx: index("V11ToolProvider_tenant_providerType_lifecycle_idx").on(
      t.tenantId,
      t.providerType,
      t.lifecycleState,
    ),
...
export type V11ToolProvider = InferSelectModel<typeof v11ToolProvider>;
export type NewV11ToolProvider = InferInsertModel<typeof v11ToolProvider>;

// ─── Tool ─────────────────────────────────────────────────

export const v11Tool = mysqlTable(
  "V11Tool",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    /** 当前 SchemaRevision id（逻辑外键 → V11ToolSchemaRevision.id）；null 表示未发布。 */
  ...
  (t) => ({
    tenantProviderKeyUq: uniqueIndex("V11Tool_tenant_providerId_toolKey_uq").on(
      t.tenantId,
      t.providerId,
      t.toolKey,
    ),
    tenantLifecycleRiskIdx: index("V11Tool_tenant_lifecycle_riskClass_idx").on(
      t.tenantId,
      t.lifecycleState,
      t.riskClass,
    ),
...
export type V11Tool = InferSelectModel<typeof v11Tool>;
export type NewV11Tool = InferInsertModel<typeof v11Tool>;

// ─── ToolSchemaRevision ───────────────────────────────────

export const v11ToolSchemaRevision = mysqlTable(
  ...
  (t) => ({
    toolRevisionNoUq: uniqueIndex("V11ToolSchemaRevision_tool_revisionNo_uq").on(
      t.toolId,
      t.revisionNo,
    ),
    toolStateIdx: index("V11ToolSchemaRevision_tool_state_idx").on(t.toolId, t.revisionState),
  }),
);

export type V11ToolSchemaRevision = InferSelectModel<typeof v11ToolSchemaRevision>;
export type NewV11ToolSchemaRevision = InferInsertModel<typeof v11ToolSchemaRevision>;
