/**
 * V11 Context Checkpoint schema：context_checkpoint 表。
 *
 * 事实源：
...
export const contextCheckpoint = mysqlTable(
  ...
  (t) => ({
    ...
    tenantInvocationTypeRangesUq: uniqueIndex(
      "V11ContextCheckpoint_tenant_invocation_type_ranges_uq",
    ).on(t.tenantId, t.invocationId, t.checkpointType, t.sourceRangesHash),
    tenantInvocationCreatedIdx: index("V11ContextCheckpoint_tenant_invocation_created_idx").on(
      t.tenantId,
      t.invocationId,
      t.createdAt,
    ),
    tenantExpiresIdx: index("V11ContextCheckpoint_tenant_expires_idx").on(t.tenantId, t.expiresAt),
  }),
);

export type V11ContextCheckpoint = InferSelectModel<typeof contextCheckpoint>;
export type NewV11ContextCheckpoint = InferInsertModel<typeof contextCheckpoint>;
