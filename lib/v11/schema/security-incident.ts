 ...
 *
 * 表语义：
 * - V11SecurityIncident：安全事件主体。记录严重程度/状态机/目标类型/检测来源/审计事件 id。
 *   incidentState 推进：open → investigating → contained → resolved / escalated。
 * - V11IncidentContainment：每次事故下的隔离止损动作。按 (incidentId, actionType) 唯一。
...
// ─── V11SecurityIncident 表 ───────────────────────────────

export const v11SecurityIncident = mysqlTable(
  "V11SecurityIncident",
  {
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11SecurityIncident_tenant_key_uq").on(t.tenantId, t.incidentKey),
    tenantStateIdx: index("V11SecurityIncident_tenant_state_idx").on(t.tenantId, t.incidentState),
    tenantTargetIdx: index("V11SecurityIncident_tenant_target_idx").on(
      t.tenantId,
      t.targetType,
      t.targetId,
    ),
    tenantSeverityIdx: index("V11SecurityIncident_tenant_severity_idx").on(t.tenantId, t.severity),
    tenantDetectedIdx: index("V11SecurityIncident_tenant_detected_idx").on(
      t.tenantId,
      t.detectedAt,
    ),
  }),
...
export type V11SecurityIncident = InferSelectModel<typeof v11SecurityIncident>;
export type NewV11SecurityIncident = InferInsertModel<typeof v11SecurityIncident>;

// ─── V11IncidentContainment 表 ────────────────────────────

export const v11IncidentContainment = mysqlTable(
  "V11IncidentContainment",
  {
    ...
    incidentId: varchar("incidentId", { length: 36 })
      .notNull()
      .references(() => v11SecurityIncident.id),
    /** 隔离动作类型（9 类）。 */
  ...
  (t) => ({
    incidentActionUq: uniqueIndex("V11IncidentContainment_incident_action_uq").on(
      t.incidentId,
      t.actionType,
    ),
    tenantIncidentIdx: index("V11IncidentContainment_tenant_incident_idx").on(
      t.tenantId,
      t.incidentId,
    ),
    incidentStateIdx: index("V11IncidentContainment_incident_state_idx").on(
      t.incidentId,
      t.actionState,
    ),
  }),
...
export type V11IncidentContainment = InferSelectModel<typeof v11IncidentContainment>;
export type NewV11IncidentContainment = InferInsertModel<typeof v11IncidentContainment>;
