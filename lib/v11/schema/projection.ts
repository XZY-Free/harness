 ...
 * - ../v11-agentkit-platform/14-production-operations-security-and-retention.md §2.1（投影消费协议七条规则）、§2.2（SSE 背压与 cursor_expired）
 * - ../v11-agentkit-platform/11-api-and-event-boundaries.md §7.4（顺序与去重）、§7.5（恢复规则）
 *
 * 关键约束：
 ...
 *
 * 幂等键至少包含 consumer、event_id 和 projection target（§2.1 规则 2）。
 */
 ...
 * 投影检查点：记录消费者已完成的流内位置。
 *
 * 事实源：§8.1 行 584-586、§9.2 行 639。
 * - PRIMARY KEY(consumer_name, stream_type, shard_key)。
 * - 只在投影写入成功的同一事务后前移（§2.1 规则 3）。
 * - 不允许在投影失败前移（§0 README ProjectionCheckpoint 域对象）。
 */
export const v11ProjectionCheckpoint = mysqlTable(
  ...
  (t) => ({
    ...
    consumerStreamShardIdx: uniqueIndex("V11ProjectionCheckpoint_consumer_stream_shard_uq").on(
      t.consumerName,
      t.streamType,
      t.shardKey,
    ),
...
export type V11ProjectionCheckpoint = InferSelectModel<typeof v11ProjectionCheckpoint>;
export type NewV11ProjectionCheckpoint = InferInsertModel<typeof v11ProjectionCheckpoint>;
 ...
 * 事件交付失败记录。
 *
 * 事实源：§8.1 行 587-589、§2.1 规则 5-7。
 * - UNIQUE(consumer_name, stream_type, stream_id, event_id)。
 * - state 为 retrying/quarantined/resolved。
 * - 达到重试上限进入 quarantined，后续同流事件不得越序生效（§8.1 行 592）。
 */
export const v11EventDeliveryFailure = mysqlTable(
  ...
  (t) => ({
    consumerStreamEventUq: uniqueIndex("V11EventDeliveryFailure_consumer_stream_event_uq").on(
      t.consumerName,
      t.streamType,
      t.streamId,
      t.eventId,
    ),
    consumerStreamSequenceIdx: index("V11EventDeliveryFailure_consumer_stream_sequence_idx").on(
      t.consumerName,
      t.streamType,
      t.streamId,
      t.eventSequence,
    ),
    stateRetryIdx: index("V11EventDeliveryFailure_state_retry_idx").on(
      t.failureState,
      t.nextRetryAt,
    ),
  }),
...
export type V11EventDeliveryFailure = InferSelectModel<typeof v11EventDeliveryFailure>;
export type NewV11EventDeliveryFailure = InferInsertModel<typeof v11EventDeliveryFailure>;
 ...
 * - Last-Event-ID < earliest_available_sequence 返回 EVENT_CURSOR_EXPIRED（§2.2 行 42）。
 */
export const v11EventStreamFloor = mysqlTable(
  "V11EventStreamFloor",
  {
    streamType: mysqlEnum("streamType", STREAM_TYPES).notNull(),
  ...
  (t) => ({
    streamTypeStreamIdUq: uniqueIndex("V11EventStreamFloor_stream_type_stream_id_uq").on(
      t.streamType,
      t.streamId,
    ),
    tenantIdx: index("V11EventStreamFloor_tenant_idx").on(t.tenantId),
  }),
);

export type V11EventStreamFloor = InferSelectModel<typeof v11EventStreamFloor>;
export type NewV11EventStreamFloor = InferInsertModel<typeof v11EventStreamFloor>;
 ...
 */
export const v11ThreadListProjection = mysqlTable(
  ...
  (t) => ({
    tenantOwnerActivityIdx: index("V11ThreadListProjection_tenant_owner_activity_idx").on(
      t.tenantId,
      t.ownerUserId,
      t.lastActivityAt,
    ),
    tenantAgentActivityIdx: index("V11ThreadListProjection_tenant_agent_activity_idx").on(
      t.tenantId,
      t.primaryAgentId,
      t.lastActivityAt,
    ),
    tenantLifecycleIdx: index("V11ThreadListProjection_tenant_lifecycle_idx").on(
      t.tenantId,
      t.lifecycleState,
    ),
  }),
...
export type V11ThreadListProjection = InferSelectModel<typeof v11ThreadListProjection>;
export type NewV11ThreadListProjection = InferInsertModel<typeof v11ThreadListProjection>;
 ...
 * - 可重建，非权威写表；投影器按 event sequence 幂等更新。
 */
export const v11TurnTimelineProjection = mysqlTable(
  ...
  (t) => ({
    threadSequenceUq: uniqueIndex("V11TurnTimelineProjection_thread_sequence_uq").on(
      t.threadId,
      t.turnSequence,
    ),
    tenantThreadSequenceIdx: index("V11TurnTimelineProjection_tenant_thread_sequence_idx").on(
      t.tenantId,
      t.threadId,
      t.turnSequence,
    ),
    threadStateIdx: index("V11TurnTimelineProjection_thread_state_idx").on(t.threadId, t.turnState),
  }),
);

export type V11TurnTimelineProjection = InferSelectModel<typeof v11TurnTimelineProjection>;
export type NewV11TurnTimelineProjection = InferInsertModel<typeof v11TurnTimelineProjection>;
