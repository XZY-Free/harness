/**
 * V11 控制面 schema：Agent 与 AgentRevision。
 *
 ...
 *
 * 旧 `Agent` 表（Phase 4-4 B1 只读档案）保持兼容，最终迁移安排在阶段 13。
 */
...
export const v11Agent = mysqlTable(
  "V11Agent",
  {
    ...
    /** 当前发布修订 id（逻辑外键 → V11AgentRevision.id）；null 表示未发布。 */
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11Agent_tenant_agentKey_uq").on(t.tenantId, t.agentKey),
    tenantLifecycleUpdatedIdx: index("V11Agent_tenant_lifecycle_updated_idx").on(
      t.tenantId,
      t.lifecycleState,
      t.updatedAt,
    ),
...
export type V11Agent = InferSelectModel<typeof v11Agent>;
export type NewV11Agent = InferInsertModel<typeof v11Agent>;

// ─── AgentRevision ──────────────────────────────────────────

export const v11AgentRevision = mysqlTable(
  "V11AgentRevision",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    agentRevisionNoUq: uniqueIndex("V11AgentRevision_agent_revisionNo_uq").on(
      t.agentId,
      t.revisionNo,
    ),
...
export type V11AgentRevision = InferSelectModel<typeof v11AgentRevision>;
export type NewV11AgentRevision = InferInsertModel<typeof v11AgentRevision>;
