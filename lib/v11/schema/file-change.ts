/**
 * V11 文件变更 schema：FileChange（阶段 8 S08-C06）。
 *
 * 事实源：
 ...
 * - artifactId 可选关联到 V11Artifact（变更结果被上传为 Artifact 时填）。
 * - 写入后不可变（无状态机、无 versionNo 乐观锁）。
 * - 跨租户隔离：所有查询按 tenantId 过滤；tenantId 外键 → Tenant(id) ON DELETE CASCADE。
 * - workspaceBindingId 外键 → V11WorkspaceBinding(id) ON DELETE CASCADE（路径解释依赖）。
 * - toolCallId 逻辑外键 → V11ToolCall.id（不加 DB FK 避免跨阶段耦合）。
 * - artifactId 逻辑外键 → V11Artifact.id（不加 DB FK 避免跨阶段耦合）。
 */
...
/**
 * V11FileChange 表：Desktop/Tool 执行产生的文件变更记录（§7.4）。
 *
 * 关键约束：
 * - workspaceBindingId 外键 → V11WorkspaceBinding(id) ON DELETE CASCADE。
 ...
 * - 写入后不可变。
 */
export const v11FileChange = mysqlTable(
  "V11FileChange",
  {
    ...
    /** 产生此变更的 ToolCall id（逻辑外键 → V11ToolCall.id；必填）。 */
    toolCallId: varchar("toolCallId", { length: 36 }).notNull(),
    /** 路径解释所必需的 Binding（DB 级 FK → V11WorkspaceBinding.id ON DELETE CASCADE）。 */
    ...
    /** 变更结果被上传为 Artifact 时关联（逻辑外键 → V11Artifact.id；可为 null）。 */
  ...
  (t) => ({
    tenantToolCallIdx: index("V11FileChange_tenant_toolCall_idx").on(t.tenantId, t.toolCallId),
    tenantBindingIdx: index("V11FileChange_tenant_binding_idx").on(
      t.tenantId,
      t.workspaceBindingId,
    ),
    tenantArtifactIdx: index("V11FileChange_tenant_artifact_idx").on(t.tenantId, t.artifactId),
  }),
);

export type V11FileChange = InferSelectModel<typeof v11FileChange>;
export type NewV11FileChange = InferInsertModel<typeof v11FileChange>;
