/**
 * V11 Usage / Capacity / SLI schema：UsageRecord / CostAggregate / CapacitySnapshot / ServiceLevelIndicator（S11-W07）。
 *
 * 事实源：
...
/**
 * SLI 指标 key（S12-W03 扩展为覆盖 spec §7.2 八类能力）：
...
export const v11UsageRecord = mysqlTable(
  "v11_usage_record",
  {
...
export type V11UsageRecord = typeof v11UsageRecord.$inferSelect;
export type V11UsageRecordInsert = typeof v11UsageRecord.$inferInsert;

// ─── V11CostAggregate ────────────────────────────────────

export const v11CostAggregate = mysqlTable(
  "v11_cost_aggregate",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    tenantId: varchar("tenant_id", { length: 36 })
      .notNull()
      .references(() => tenant.id),
    /** 用量维度（与 v11_usage_record.dimension 对齐）。 */
    dimension: varchar("dimension", { length: 32 }).notNull(),
    /** 维度切分 scope 类型（与 v11_usage_record.scope_type 对齐）。 */
    scopeType: varchar("scope_type", { length: 32 }).notNull(),
    /** scope 引用标识。 */
    scopeRef: varchar("scope_ref", { length: 128 }),
    /** 窗口起点（含）。 */
...
export type V11CostAggregate = typeof v11CostAggregate.$inferSelect;
export type V11CostAggregateInsert = typeof v11CostAggregate.$inferInsert;

// ─── V11CapacitySnapshot ──────────────────────────────────

export const v11CapacitySnapshot = mysqlTable(
  "v11_capacity_snapshot",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    tenantId: varchar("tenant_id", { length: 36 })
      .notNull()
...
export type V11CapacitySnapshot = typeof v11CapacitySnapshot.$inferSelect;
export type V11CapacitySnapshotInsert = typeof v11CapacitySnapshot.$inferInsert;

// ─── V11ServiceLevelIndicator ─────────────────────────────

export const v11ServiceLevelIndicator = mysqlTable(
  "v11_service_level_indicator",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    tenantId: varchar("tenant_id", { length: 36 })
      .notNull()
    ...
    /** 告警错误码（S12-W03）：breach=true 时填入触发的 V11 错误码（如 STREAM_BACKPRESSURE）。 */
...
export type V11ServiceLevelIndicator = typeof v11ServiceLevelIndicator.$inferSelect;
export type V11ServiceLevelIndicatorInsert = typeof v11ServiceLevelIndicator.$inferInsert;
