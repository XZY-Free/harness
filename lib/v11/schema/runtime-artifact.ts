/**
 * V11 运行产物 schema：Artifact（阶段 8 S08-C06）。
 *
 * 事实源：
 ...
 *
 * 与 lib/v11/schema/artifact.ts（ArtifactAttestation 控制面供应链证明）是不同概念：
 * - ArtifactAttestation：对 AgentRevision/RuntimeRevision/Skill/Tool/Policy 等制品做签名/SBOM 验证。
 * - V11Artifact（本表）：运行时产物（ToolCall/Job 生成的 Excel、报告、图片等）。
 *
 * 关键不变量：
 ...
 */
export const v11Artifact = mysqlTable(
  "V11Artifact",
  {
  ...
  (t) => ({
    itemIdUq: uniqueIndex("V11Artifact_itemId_uq").on(t.itemId),
    tenantInvocationIdx: index("V11Artifact_tenant_invocation_idx").on(t.tenantId, t.invocationId),
    tenantThreadIdx: index("V11Artifact_tenant_thread_idx").on(t.tenantId, t.threadId),
    tenantJobIdx: index("V11Artifact_tenant_job_idx").on(t.tenantId, t.jobId),
    tenantExpiresIdx: index("V11Artifact_tenant_expires_idx").on(t.tenantId, t.expiresAt),
  }),
);

export type V11Artifact = InferSelectModel<typeof v11Artifact>;
export type NewV11Artifact = InferInsertModel<typeof v11Artifact>;
