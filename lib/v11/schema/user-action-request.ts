/**
 * V11 用户操作请求 schema：UserActionRequest（阶段 8 S08-C04）。
 *
 * 事实源：
 ...
 * - grant：用户授权（approve/deny，成功后写 V11Grant）。
 ...
 * - grant_id：grant 类型 approve 后指向 V11Grant.id。
 * - 只能从 pending 解析一次（应用层原子更新 + 乐观锁）。
 */
export const v11UserActionRequest = mysqlTable(
  "V11UserActionRequest",
  {
    ...
    /** 所属 Thread（逻辑外键 → V11Thread.id）。 */
    threadId: varchar("threadId", { length: 36 }).notNull(),
    /** 所属 Turn（逻辑外键 → V11Turn.id）。 */
    turnId: varchar("turnId", { length: 36 }).notNull(),
    /** 所属 Invocation，必须为 Turn Invocation（逻辑外键 → V11Invocation.id）。 */
    invocationId: varchar("invocationId", { length: 36 }).notNull(),
    /** 引发请求的 ToolCall（可空；逻辑外键 → V11ToolCall.id）。 */
    toolCallId: varchar("toolCallId", { length: 36 }),
    /** 员工可见 ThreadItem 投影外键（可空但唯一；逻辑外键 → V11ThreadItem.id）。 */
    ...
    /** grant 类型 approve 后指向 V11Grant.id（逻辑外键）。 */
  ...
  (t) => ({
    itemIdUq: uniqueIndex("V11UserActionRequest_item_id_uq").on(t.itemId),
    tenantInvocationStateIdx: index("V11UserActionRequest_tenant_invocation_state_idx").on(
      t.tenantId,
      t.invocationId,
      t.requestState,
    ),
    tenantToolCallIdx: index("V11UserActionRequest_tenant_toolCall_idx").on(
      t.tenantId,
      t.toolCallId,
    ),
    tenantStateExpiresIdx: index("V11UserActionRequest_tenant_state_expires_idx").on(
      t.tenantId,
      t.requestState,
      t.expiresAt,
    ),
    authStateHashIdx: index("V11UserActionRequest_auth_state_hash_idx").on(t.authStateHash),
  }),
);

export type V11UserActionRequest = InferSelectModel<typeof v11UserActionRequest>;
export type NewV11UserActionRequest = InferInsertModel<typeof v11UserActionRequest>;
