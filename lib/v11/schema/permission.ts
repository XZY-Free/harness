/**
 * V11 受控工具执行链 schema：PermissionDecision + Grant（阶段 8 S08-C03）。
 *
 * 事实源：
 ...
 * - credentialRefId 引用 V11CredentialRef；Grant 不保存 Credential 原值。
...
/**
 * V11 PermissionDecision 决策结果（§5.5）。
 * - allow：允许执行，无需用户操作。
 ...
 *   Agent 只能收紧不能放宽平台策略。
 *
 * 命名加 V11_ 前缀避免与旧 lib/db/schema.ts PERMISSION_DECISIONS（["allow","deny","ask"]）冲突。
 */
export const V11_PERMISSION_DECISIONS = ["allow", "pause", "block"] as const;
export type V11PermissionDecisionValue = (typeof V11_PERMISSION_DECISIONS)[number];
 ...
 * - toolCallId 引用 V11ToolCall（逻辑外键，不加 DB 级 FK 避免跨阶段耦合）。
 ...
 */
export const v11PermissionDecision = mysqlTable(
  "V11PermissionDecision",
  {
    ...
    /** 所属 ToolCall id（逻辑外键 → V11ToolCall.id）。 */
    ...
    decision: mysqlEnum("decision", V11_PERMISSION_DECISIONS).notNull(),
  ...
  (t) => ({
    toolCallSequenceUq: uniqueIndex("V11PermissionDecision_toolCall_sequence_uq").on(
      t.toolCallId,
      t.decisionSequence,
    ),
    tenantToolCallIdx: index("V11PermissionDecision_tenant_toolCall_idx").on(
      t.tenantId,
      t.toolCallId,
    ),
    tenantDecisionIdx: index("V11PermissionDecision_tenant_decision_idx").on(
      t.tenantId,
      t.decision,
    ),
  }),
...
export type V11PermissionDecision = InferSelectModel<typeof v11PermissionDecision>;
export type NewV11PermissionDecision = InferInsertModel<typeof v11PermissionDecision>;
...
export const v11Grant = mysqlTable(
  "V11Grant",
  {
    ...
    credentialRefId: varchar("credentialRefId", { length: 36 })
      .notNull()
      .references(() => v11CredentialRef.id),
  ...
  (t) => ({
    tenantUserStateIdx: index("V11Grant_tenant_user_state_idx").on(
      t.tenantId,
      t.userId,
      t.grantState,
    ),
    tenantCredentialIdx: index("V11Grant_tenant_credential_idx").on(t.tenantId, t.credentialRefId),
    tenantStateExpiresIdx: index("V11Grant_tenant_state_expires_idx").on(
      t.tenantId,
      t.grantState,
      t.expiresAt,
    ),
...
export type V11Grant = InferSelectModel<typeof v11Grant>;
export type NewV11Grant = InferInsertModel<typeof v11Grant>;
 ...
 */
export const v11PolicySet = mysqlTable(
  "V11PolicySet",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    /** 当前生效修订 id（逻辑外键 → V11PolicyRevision.id）；null 表示未发布。 */
    currentRevisionId: varchar("currentRevisionId", { length: 36 }),
    lifecycleState: mysqlEnum("lifecycleState", POLICY_LIFECYCLE_STATES)
      .notNull()
      .default("draft"),
    /** 乐观并发版本号。 */
    versionNo: bigint("versionNo", { mode: "number" }).notNull().default(1),
  ...
  (t) => ({
    tenantKeyUq: uniqueIndex("V11PolicySet_tenant_policySetKey_uq").on(t.tenantId, t.policySetKey),
    tenantLifecycleUpdatedIdx: index("V11PolicySet_tenant_lifecycle_updated_idx").on(
      t.tenantId,
      t.lifecycleState,
      t.updatedAt,
    ),
...
export type V11PolicySet = InferSelectModel<typeof v11PolicySet>;
export type NewV11PolicySet = InferInsertModel<typeof v11PolicySet>;

/**
 * PolicyRevision 表：不可变策略修订（§4.4）。
 *
 * 关键约束：
 ...
 */
export const v11PolicyRevision = mysqlTable(
  "V11PolicyRevision",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    revisionState: mysqlEnum("revisionState", POLICY_REVISION_STATES)
      .notNull()
      .default("draft"),
  ...
  (t) => ({
    setRevisionNoUq: uniqueIndex("V11PolicyRevision_set_revisionNo_uq").on(
      t.policySetId,
      t.revisionNo,
    ),
    setStateIdx: index("V11PolicyRevision_set_state_idx").on(t.policySetId, t.revisionState),
  }),
);

export type V11PolicyRevision = InferSelectModel<typeof v11PolicyRevision>;
export type NewV11PolicyRevision = InferInsertModel<typeof v11PolicyRevision>;

/**
 * V11Policy 表：单条持久化策略规则（ToolPermissionRule 迁入）。
 *
 * 关键约束：
 * - policySetId 引用 V11PolicySet（逻辑外键，标识所属策略集）。
 * - policyRevisionId 引用 V11PolicyRevision（逻辑外键，标识所属修订）。
 * - decision 与 V11PermissionDecision.decision 对齐（allow/pause/block）。
 * - scopeJson 保存映射后的 scope 表达（如 { type: "tenant" }）。
 */
export const v11Policy = mysqlTable(
  "V11Policy",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    tenantId: varchar("tenantId", { length: 36 })
      .notNull()
      .references(() => tenant.id),
    /** 所属策略集 id（逻辑外键 → V11PolicySet.id）。 */
    policySetId: varchar("policySetId", { length: 36 }).notNull(),
    /** 所属修订 id（逻辑外键 → V11PolicyRevision.id）。 */
    policyRevisionId: varchar("policyRevisionId", { length: 36 }),
    ...
    /** 决策结果（allow/pause/block，与 V11PermissionDecision.decision 对齐）。 */
    decision: mysqlEnum("decision", V11_PERMISSION_DECISIONS).notNull(),
  ...
  (t) => ({
    tenantSetIdx: index("V11Policy_tenant_set_idx").on(t.tenantId, t.policySetId),
    tenantDecisionIdx: index("V11Policy_tenant_decision_idx").on(t.tenantId, t.decision),
  }),
);

export type V11Policy = InferSelectModel<typeof v11Policy>;
export type NewV11Policy = InferInsertModel<typeof v11Policy>;
