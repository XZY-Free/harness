/**
 * V11 控制面 schema：CatalogEntry 与 CatalogRevision（阶段 6 S06-C03）。
 *
 * 事实源：
...
export const v11CatalogEntry = mysqlTable(
  "V11CatalogEntry",
  {
    ...
    projectedAt: datetime("projectedAt", { mode: "date", fsp: 3 })
      .notNull()
      .$defaultFn(() => new Date()),
    /** 当前所属租户目录的修订号（与 V11CatalogRevision.currentRevision 对齐）。 */
    catalogRevision: bigint("catalogRevision", { mode: "number" }).notNull(),
  },
  (t) => ({
    tenantResourceUq: uniqueIndex("V11CatalogEntry_tenant_resourceType_resourceId_uq").on(
      t.tenantId,
      t.resourceType,
      t.resourceId,
    ),
    tenantTypeLifecycleIdx: index("V11CatalogEntry_tenant_resourceType_lifecycle_idx").on(
      t.tenantId,
      t.resourceType,
      t.lifecycleState,
    ),
    tenantRevisionIdx: index("V11CatalogEntry_tenant_catalogRevision_idx").on(
      t.tenantId,
      t.catalogRevision,
    ),
  }),
...
export type V11CatalogEntry = InferSelectModel<typeof v11CatalogEntry>;
export type NewV11CatalogEntry = InferInsertModel<typeof v11CatalogEntry>;

// ─── CatalogRevision ──────────────────────────────────────

export const v11CatalogRevision = mysqlTable(
  "V11CatalogRevision",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    tenantAudienceUq: uniqueIndex("V11CatalogRevision_tenant_audience_uq").on(
      t.tenantId,
      t.audience,
    ),
  }),
...
export type V11CatalogRevision = InferSelectModel<typeof v11CatalogRevision>;
export type NewV11CatalogRevision = InferInsertModel<typeof v11CatalogRevision>;
