 ...
 * - reconcile 同事务更新：effect_record + effect_target + tool_call.call_state（§6.5）。
...
/**
 * 核对 ToolCall 副作用（§5.2 Gateway 即时核对 / §6.5 Admin 长期核对）。
 *
 * 关键规则：
 ...
 * - 同事务更新：effect_record + effect_target + tool_call.call_state（§6.5）。
 ...
 * 调用方应在 Tool 执行超时时调用本函数，避免直接置为 failed（§6.4）。
 */
