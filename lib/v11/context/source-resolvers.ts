...
/**
 * Skill 指令片段解析器（§3.2：按需加载 Skill 完整指令）。
 *
...
/**
 * Workspace 文件地图解析器（§3.1：可用资料和能力的简短索引）。
 *
...
/**
 * Memory 源解析器（§3.2：按需加载某一作用域的 Memory；§8 作用域；§9 挂载与检索）。
 *
...
/**
 * Knowledge 源解析器（§3.2：按需加载 Knowledge 文档与图谱证据；§13 加载规则）。
 *
...
export class KnowledgeResolver implements SourceResolver {
  ...
  async resolve(ctx: ContextQueryContext): Promise<SourceQueryResult> {
    ...
    // 无 query 时返回 empty（§13.1：Agent 先看到目录，需要时提交查询）
