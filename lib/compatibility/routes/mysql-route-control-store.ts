      ...
      operation({
        async lockRouteSet(tenantId, routeSetId) {
          const [row] = await tx
            ...
            .where(
              and(
                eq(v11DeploymentRouteSet.id, routeSetId),
        ...
        async resolveRouteIdentity(params) {
          const conditions = params.routeId
            ? and(
                eq(v11DeploymentRoute.id, params.routeId),
                eq(v11DeploymentRoute.routeSetId, params.routeSetId),
              )
            : and(
                eq(v11DeploymentRoute.routeSetId, params.routeSetId),
                eq(v11DeploymentRoute.agentRevisionId, params.content.agentRevisionId),
                eq(v11DeploymentRoute.runtimeRevisionId, params.content.runtimeRevisionId),
              );
          ...
          const id = randomUUID();
          await tx.insert(v11DeploymentRoute).values({
            id,
          ...
          const [created] = await tx
            .select()
            .from(v11DeploymentRoute)
            .where(eq(v11DeploymentRoute.id, id))
            .limit(1);
        ...
        async updateRouteProjection(params) {
          await tx
            ...
            .where(eq(v11DeploymentRoute.id, params.routeId));
          const [row] = await tx
            .select()
            .from(v11DeploymentRoute)
            .where(eq(v11DeploymentRoute.id, params.routeId))
          ...
          const result = await tx
            ...
            .where(
              and(
                ...
                eq(v11DeploymentRouteSet.versionNo, params.expectedVersionNo),
              ),
            );
