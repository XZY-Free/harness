  ...
  create: (input) =>
    db.transaction(async (tx) => {
      const [invocation] = await tx
        ...
        .from(v11Invocation)
        .where(
          and(eq(v11Invocation.id, input.invocationId), eq(v11Invocation.tenantId, input.tenantId)),
        )
        .limit(1)
        .for("update");
      ...
      const [existing] = await tx
        ...
        .where(eq(v11ExecutionBinding.invocationId, input.invocationId))
        .limit(1);
      ...
      await tx.insert(v11ExecutionBinding).values({
...
async function lockAndValidateRoute(tx: Transaction, input: StoreExecutionBindingInput) {
  const [routeRow] = await tx
    .select({ route: v11DeploymentRoute, routeSet: v11DeploymentRouteSet })
    .from(v11DeploymentRoute)
    .innerJoin(v11DeploymentRouteSet, eq(v11DeploymentRouteSet.id, v11DeploymentRoute.routeSetId))
    .where(
      and(
        eq(v11DeploymentRoute.id, input.deploymentRouteId),
  ...
  const [agentRevision, runtimeRevision] = await Promise.all([
    tx
      ...
      .where(eq(v11AgentRevision.id, input.agentRevisionId))
      .limit(1)
      .for("update")
    ...
    tx
      ...
      .where(eq(v11RuntimeRevision.id, input.runtimeRevisionId))
      .limit(1)
      .for("update")
      .then((rows) => rows[0] ?? null),
  ]);
  ...
  const [agent, runtime] = await Promise.all([
    tx
      .select({ id: v11Agent.id, lifecycleState: v11Agent.lifecycleState })
      .from(v11Agent)
      .where(and(eq(v11Agent.id, agentRevision.agentId), eq(v11Agent.tenantId, input.tenantId)))
      .limit(1)
      .for("update")
      .then((rows) => rows[0] ?? null),
    tx
      .select({ id: v11Runtime.id, lifecycleState: v11Runtime.lifecycleState })
      .from(v11Runtime)
      .where(
        and(eq(v11Runtime.id, runtimeRevision.runtimeId), eq(v11Runtime.tenantId, input.tenantId)),
      )
      .limit(1)
      .for("update")
      .then((rows) => rows[0] ?? null),
  ...
  if (input.policyRevisionId) {
    const [policy] = await tx
      ...
      .where(eq(v11PolicyRevision.id, input.policyRevisionId))
      .limit(1)
      .for("update");
...
async function lockAndValidateConformance(
  ...
  revision: typeof v11RuntimeRevision.$inferSelect,
): Promise<void> {
  const [run] = await tx
    .select()
...
function toExecutionBinding(row: typeof v11ExecutionBinding.$inferSelect): ExecutionBinding {
  if (
    !row.routeRevisionId ||
