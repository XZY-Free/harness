...
/**
 * Phase 4-2：Analytics 聚合查询层（**纯只读**）。
 *
 * 把 Phase 2 事件流（`thread_events`）+ `tool_runs` + Phase 3 skill 绑定，聚合成运营指标。
 * 跨 thread 聚合，与 `lib/thread-events/projector.ts`（单 thread 投影）同源不同向。
 *
 * **头号纪律（§4.5）**：本模块只 `select` 不 mutate——不 import 任何写函数
 *（`updateThreadStatus` / `appendThreadEvent` / `finishToolRun*` / `insert` / `update`）。
 * Stage E 用 grep 把关。任何 mutation import 都破坏「零回归」承诺。
 *
 * 指标口径以本文件函数注释为准（§4.4），防 P4-4 UI 误读。
 *
 * Phase 4-3：所有聚合支持 `userId` scope。`/api/analytics` 默认只聚合当前用户数据，
...
// ─── preview 成功率（§4.1 reportReady 探活） ────────────────
 ...
 *
 * - startMs：`agent.started` 事件 createdAt；缺则退回 `thread.createdAt`（§4.4.3）。
 ...
 * 量小实时聚合可接受（§7）；量大再优化。
 *
 * Phase 4-3：thread 首次 select 按 scope.userId 限定归属。
 */
type Lifespan = {
  threadId: string;
  skillId: string | null;
...
export type ToolFailureBreakdown = {
   ...
   * **不给 tool_runs 加冗余列**（那是写路径变更，违反零回归，§4.4.2）。
   */
 ...
 *
 * Phase 4-3：userId scope 通过 threadIdsForScope 限定（skills.matched 事件无 userId 列,
