...
import { seedInvocation, seedTenant, validBindingConfig } from "@/lib/agents/calls/test/agent-call-test-fixtures";
...
async function seedRunningCall(threadId?: string) {
  ...
  await mysqlAgentCallStore.updateState({
    callId: call.id, tenantId, from: "queued", to: "running", now: NOW,
    lifecycle: { startedAt: NOW },
  });
  return { tenantId, parentId, call };
}

function ev(o: Partial<AgentCallCandidateEvent> & { type: AgentCallCandidateEvent["type"] }): AgentCallCandidateEvent {
  return {
...
describe("ingestAgentCallEvents 原子应用与生命周期边界", () => {
  it("started+completed 原子绑定 task/context/session，结果 text+data+digest，attempt 终态，ingress 全 mapped，parent/runtime 不变", async () => {
    ...
    await ingestAgentCallEvents({
      tenantId, callId: call.id,
      events: [
        ev({ type: "call.started", producer_event_id: "evt-start", producer_sequence: 1,
             payload: { task_id: "task-1", context_id: "ctx-1" } }),
        ev({ type: "call.completed", producer_event_id: "evt-done", producer_sequence: 2,
    ...
    const [attempt] = await db.select().from(agentCallAttemptTable).where(eq(agentCallAttemptTable.callId, call.id));
    ...
    const [parentRow] = await db.select().from(invocationTable).where(eq(invocationTable.id, parentId));
  ...
  it("input_required/failed/cancelled/lost 各自仅子状态，input prompt/schema 保留，parent 不变", async () => {
    for (const [type, state, terminal] of [
      ...
      const payload = type === "call.input_required"
      ...
      const [ing] = await db.select().from(agentCallEventIngressTable).where(eq(agentCallEventIngressTable.producerEventId, `evt-${type}`));
      if (type === "call.input_required") expect(ing?.payloadJson).toMatchObject({ prompt: "need more", schema: { kind: "form" } });
      const [parentRow] = await db.select().from(invocationTable).where(eq(invocationTable.id, parentId));
      expect(parentRow?.executionState).toBe("running");
      expect(parentRow?.finishedAt).toBeNull();
    }
  ...
  it("重复回放幂等；同 key 改 payload/type 冲突且无状态变更；异租户同 key 不透出", async () => {
    ...
    const batch = {
      tenantId, callId: call.id,
      events: [ev({ type: "call.completed", producer_event_id: "evt-done", producer_sequence: 1, payload: { text: "x" } })],
    };
    ...
    expect(await rejects(() => ingestAgentCallEvents({ ...batch, events: [{ ...first, payload: { text: "DIFFERENT" } }] }))).toBe(true);
    expect(await rejects(() => ingestAgentCallEvents({ ...batch, events: [{ ...first, type: "call.failed" }] }))).toBe(true);
    expect(await rejects(() => ingestAgentCallEvents({ tenantId: otherTenant, callId: call.id, events: batch.events }))).toBe(true);
  ...
  it("task/context 关联不匹配、中途非法事件 -> call/session/ingress/attempt 整批回滚", async () => {
    ...
    expect(await rejects(() => ingestAgentCallEvents({ tenantId, callId: call.id, events: [
      ev({ type: "call.started", producer_event_id: "e1", producer_sequence: 1, payload: { task_id: "task-1", context_id: "c" } }),
      ev({ type: "call.completed", producer_event_id: "e2", producer_sequence: 2, payload: { task_id: "task-9", context_id: "c", text: "x" } }),
    ] }))).toBe(true);
    ...
    expect(await rejects(() => ingestAgentCallEvents({ tenantId, callId: call.id, events: [
      ev({ type: "call.started", producer_event_id: "e3", producer_sequence: 3, payload: { task_id: "t", context_id: "c" } }),
      ev({ type: "call.unsupported" as AgentCallCandidateEvent["type"], producer_event_id: "e4", producer_sequence: 4, payload: {} }),
    ] }))).toBe(true);
  ...
  it("session thread 只来自 parent；parent 无 thread 不建会话；同 refs 异 owner 拒绝", async () => {
    ...
    await ingestAgentCallEvents({ tenantId, callId: call.id, events: [
      ev({ type: "call.started", producer_event_id: "e1", producer_sequence: 1, payload: { task_id: "task-job", context_id: "ctx-job" } }),
    ] });
    const [callRow] = await db.select().from(agentCallTable).where(eq(agentCallTable.id, call.id));
    ...
    expect(await rejects(() => ingestAgentCallEvents({ tenantId: otherTenant, callId: call2.id, events: [
      ev({ type: "call.started", producer_event_id: "e9", producer_sequence: 1, payload: { task_id: "task-job", context_id: "ctx-job" } }),
    ] }))).toBe(true);
  });
  ...
  it("终态后迟到事件不可改状态/结果", async () => {
    const { tenantId, call } = await seedRunningCall("thread-1");
    await ingestAgentCallEvents({ tenantId, callId: call.id, events: [
      ev({ type: "call.completed", producer_event_id: "evt-done", producer_sequence: 1, payload: { text: "final" } }),
    ] });
    expect(await rejects(() => ingestAgentCallEvents({ tenantId, callId: call.id, events: [
      ev({ type: "call.failed", producer_event_id: "evt-late", producer_sequence: 2, payload: { error: "late" } }),
    ] }))).toBe(true);
    const [callRow] = await db.select().from(agentCallTable).where(eq(agentCallTable.id, call.id));
    expect(callRow?.state).toBe("completed");
