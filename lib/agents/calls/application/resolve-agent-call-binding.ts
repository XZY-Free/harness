...
/**
 * resolveAgentActionBinding — 解析 Agent target Route 并装配 AgentCallBinding candidate
 ...
 *
 * Hosted Harness 与 Gateway capability-actions（HTTP）
...
/** Harness 已提交的 Agent action 无法解析为可执行 AgentCall（fail closed）。 */
export class AgentActionUnavailableError extends Error {
  readonly code = "AGENT_ROUTE_UNAVAILABLE";

  constructor(agentId: string, reason: string) {
    super(`Agent action ${agentId} 无法执行：${reason}`);
    this.name = "AgentActionUnavailableError";
  }
}

export interface ResolveAgentActionBindingParams {
  tenantId: string;
  agentId: string;
...
export interface ResolvedAgentActionBinding {
 ...
 * 失败即抛 AgentActionUnavailableError（fail closed）。
 */
export async function resolveAgentActionBinding(
  params: ResolveAgentActionBindingParams,
): Promise<ResolvedAgentActionBinding> {
  ...
  if (routeOutcome.status !== "resolved") {
    throw new AgentActionUnavailableError(
      agentId,
  ...
  if (resolution.target.kind !== "agent") {
    throw new AgentActionUnavailableError(agentId, "解析结果不是 agent Route");
  }
  const agentRevisionId = resolution.target.agentRevisionId;
  if (!agentRevisionId) {
    throw new AgentActionUnavailableError(agentId, "agent Route 缺少 AgentRevision");
  }
  ...
  if (resolution.controlPlaneEvidence.kind !== "agent") {
    throw new AgentActionUnavailableError(agentId, "Agent Route 缺少 exact Agent Contract 证据");
  }
  ...
  if (!snapshot) {
    throw new AgentActionUnavailableError(agentId, "AgentContractSnapshot 不存在");
  }
