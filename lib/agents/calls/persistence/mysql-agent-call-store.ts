 ...
 *
 * 统一事务（create / createIdempotent）：
 * 1. 校验 parent Invocation 存在且属于同 tenant（cross-tenant fail-closed）。
 * 2. 幂等：查 (parentInvocationId, logicalCallKey)；已存在 → 返回已存在 call。
 * 3. 插入 AgentCall + AgentCallBinding + 初始 Attempt(1)。
 *
...
export const mysqlAgentCallStore: AgentCallStore = {
  create: (input) =>
    db.transaction(async (tx) => {
      return doCreate(tx, input);
    }),
    ...
    db.transaction(async (tx) => {
      ...
      if (input.logicalCallKey) {
        ...
        if (existing) {
          ...
          return { call: toAgentCall(existing), binding, created: false };
        }
      }
      const call = await doCreate(tx, input);
      return { call, binding: input.binding, created: true };
    }),
  ...
  updateState: (input) =>
    db.transaction(async (tx) => {
      ...
      const updated = {
        ...
        ...(input.externalTaskRef !== undefined ? { externalTaskRef: input.externalTaskRef } : {}),
...
async function doCreate(tx: Transaction, input: StoreAgentCallInput): Promise<AgentCall> {
  ...
  const [invocation] = await tx
    .select({ id: invocationTable.id, tenantId: invocationTable.tenantId })
    ...
    .limit(1);
  if (!invocation) {
    throw new AgentCallParentInvocationError(input.parentInvocationId, input.tenantId);
  }

  // 2. 幂等：同一 call 已绑定 → 禁止重复。
  ...
  // 3. 插入 AgentCall + Binding + 初始 Attempt(1)。
...
export class AgentCallParentInvocationError extends Error {
  constructor(
    public readonly parentInvocationId: string,
    public readonly tenantId: string,
  ) {
    super(`AgentCall 的 parent Invocation ${parentInvocationId} 不存在或不属于租户 ${tenantId}`);
    this.name = "AgentCallParentInvocationError";
  }
}
