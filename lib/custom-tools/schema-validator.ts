/**
 * P1 修复（01 AI Core P1-7 完整化）：轻量 JSON Schema 子集校验器。
 *
 ...
 * - enum: 值在枚举内
 *
 * V6-M3-2（B6）fail-closed：遇不支持的 keyword（$ref/oneOf/anyOf/pattern/format/minLength/maxLength）
...
/** V6-M3-2（B6）：不支持的 JSON Schema keyword，遇则报错（fail-closed）。 */
...
export function validateJsonSchema(value: unknown, schema: JsonSchema, path = ""): string[] {
  ...
  // V6-M3-2（B6）：不支持的 keyword → fail-closed（报错而非静默跳过）
