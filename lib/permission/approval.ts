/**
 * V3.1：审批 fingerprint 计算、scope 匹配与 CRUD 查询封装（蓝图 §5.3 / §12 V3.1）。
 *
...
// V3.2：追加 runBuild / installDependencies——installDependencies ask 后模型重试时 input 可能
// 变化（packageManager 等），取首 token（npm/pnpm/yarn）稳定，复用 V3.1 fingerprint 规则（plan §12）。
...
/**
 * P1 修复（07 Permission P1-2）：取 command 首 N 个 token,用于 fingerprint 细化。
 *
...
export function summarizeArgs(toolName: string, input: Record<string, unknown>): string {
  ...
  if (toolName === "applyPatch" && typeof input.patch === "string") {
    ...
    // P1 修复（07 P1-2）：审批展示完整命令(原只展示首 token,审批者看不到完整命令)。
 ...
 * - always → 全局复用
 * - thread → 仅同 threadId
 * - project → V3.1 thread 无 projectId 维度，按 thread 收敛（后续补 project 后放宽）
...
export function isApprovalApplicable(
  ...
  switch (approval.approvedScope) {
    ...
    case "thread":
      return approval.threadId === ctx.threadId;
    // V6-M3-5（C4）：project scope 按 projectId 匹配，允许跨 thread 复用
    case "project":
      return (
        approval.projectId !== null && ctx.projectId != null && approval.projectId === ctx.projectId
      );
    case "session":
      // S1（07-P1-6）：session scope = 同 thread + 未过期（短 TTL 由 expiresAt 控制，区别于 thread 永久）
      return approval.threadId === ctx.threadId;
    default:
      return false;
  }
