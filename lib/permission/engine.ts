...
/**
 * V3.1：allow/deny/ask 三态权限引擎纯函数（蓝图 §5.3 / §12 V3.1）。
 *
 * 单一收口：`executeToolRun` 用本引擎替换 deny-only 的 `beforeTool`。所有工具（含 V3.1 新增 8 个）
...
export interface EvaluatePermissionArgs {
  ...
  /**
   * S1（07-P1-1）：当前 thread 绑定的 skillId(供 skill-scope 权限规则匹配)。
  ...
  skillId?: string | null;
  /**
   * V3.4：覆盖 permissionKey（默认 `tool.${toolName}`）。
...
/**
 * V3.4 支持的 permissionKey 前缀（多前缀分发）。
 ...
 *
 * 裸名（无任一已知前缀）仍等价 `tool.<name>`（V3.1 零回归）。
 */
 ...
 * - "web.*"/"docs.*"/"custom.*" 同理
 *
 * V3.4 修复：原实现 `pattern.startsWith("tool.") ? pattern : \`tool.${pattern}\``
 ...
 * - risk：V3.1 不在引擎层解释（规则层 ask 表达），存在即忽略
 */
function matchArg(matcher: PermissionRule["argMatcher"], input: Record<string, unknown>): boolean {
  ...
  if (matcher.pathRegex !== undefined) {
    ...
    // S1（07-P1-3）：ReDoS 防护——限输入长度 + 源码风险检测（嵌套量词拒绝）
...
/** S1（07-P1-3）：ReDoS 源码风险检测（嵌套量词 / 重叠交替量词）。 */
 ...
 *
 * S1（07-P1-1）：skill-scope 真解释(原注释自承"不解释,空壳")。
...
function isScopeApplicable(
  ...
  switch (rule.scope) {
    case "global":
    case "tenant": // V3.1 无 tenant 维度，按全局处理
      return true;
    case "thread":
    ...
    case "skill":
      // S1（07-P1-1）：scopeRef=null 放行(全局 skill 规则);否则仅绑定该 skill 的 thread 匹配
