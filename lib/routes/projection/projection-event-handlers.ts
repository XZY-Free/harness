 ...
 *
 * : 所有事件使用 RouteEligibilitySourceReader 从权威事实发现受影响 Route，
...
export interface ProjectionEventHandlerDeps {
  ...
  /** : 权威事实 SourceReader — 替代 findProjectionsByXxx。 */
 ...
 * 创建 Outbox 事件处理器。
 *
 * : 所有事件通过 SourceReader 从权威事实发现 Route。
...
export function createProjectionEventHandler(deps: ProjectionEventHandlerDeps): OutboxEventHandler {
  return async function handleOutboxEvent(event: ControlPlaneOutboxEvent): Promise<void> {
    ...
    switch (event.eventType) {
      ...
      // ─── Revision 事件 — : 通过 SourceReader 从权威事实发现 Route ───
      ...
      // ─── Attestation 事件 — : 通过 PublicationRecord → RouteRevision 发现 ───
      ...
      // ─── Conformance 事件 — : 通过 RuntimeRevision 发现 ───
      ...
      // ─── : 生命周期事件 — 通过 SourceReader 从权威事实发现 ───
      ...
      // ─── : Policy 事件 — 通过 SourceReader 从权威事实发现 ───
      ...
      // ─── : RouteSet 激活事件 — 通过 SourceReader 从权威事实发现 ───
      case "route_set.activated": {
        const routeSetId = payload.route_set_id as string;
        // : payload 中可能包含 route_ids，优先使用；否则按 RouteSet 查权威 Route
  ...
  /**
   * : 从权威事实发现受影响 Route 并重建 Projection。
  ...
  /**
   * : 从权威事实发现受影响 Route 并标记 Ineligible。
   */
