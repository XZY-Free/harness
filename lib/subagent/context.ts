...
/**
 * V3.5 Stage B：子代理 scoped context package 派生。
 *
 * 命门（计划执行规矩 §3）：**子代理不看父历史**。本函数只构造裁剪后的合成消息
 *（goal + contextHints + 相关 snippet + 可选 plan 片段 / 工具证据），**不传父 Message 表
 * 原始历史**。复用 V3.3a buildContextPackage，裁剪在 input 层，builder 零改动。
 *
 ...
 * ContextSummary 表写摘要行。子代理上下文是预裁剪的合成消息（短），无需 V3.3a 压缩；
...
export async function buildSubagentContextPackage(
  ...
  // S1（04-G16）：token 限流。原 tokenBudget=Infinity 无上限，长 goal/多 snippet 可能把子代理
