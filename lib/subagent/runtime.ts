...
export type SubagentModelRunner = (args: {
 ...
 /** 用 SDK 的 ToolSet 类型替代 Record<string, unknown>，消除 as never。 */
 tools: ToolSet;
 maxSteps: number;
...
const defaultTranscriptWriter: TranscriptWriter = async ({ parentThreadId, runId, data }) => {
 ...
 // transcript 体积上限（默认 2MB），超限截断 messages 等大字段防磁盘膨胀。
...
const defaultModelRunner: SubagentModelRunner = async (args) => {
 ...
 // tools 用 ToolSet 类型直接传入，消除 as never。
...
/** 用 ToolSet 类型保留 SDK 工具类型信息，消除 as never。 */
...
export async function executeSubagent(
 ...
 timeoutMs?: number;
 abortSignal?: AbortSignal;
 /** per-run maxSteps 覆盖（默认 SUBAGENT_MAX_STEPS）。 */
 maxSteps?: number;
 },
 ...
 // 3. 嵌套执行（带超时）。在子代理 scope 内执行，子代理内工具调用自动带 subagentScope 审计标记。
...
export async function waitForSubagent(
 ...
 // detached run 轮询间隔 500→250ms，降低 join 延迟（本地 run 走 activeRuns.promise 已无轮询）
