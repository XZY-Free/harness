...
/**
 * V3.5 Stage D：subagent 摘要构建（填充 V3.3a subagentSummary slot）。
 *
 ...
 *
 * 实现委托给 lib/context/summary-types.ts 的 extractSubagentSummary（V3.3a slot filler，纯函数），
...
export function renderSubagentSummaries(summaries: SubagentSummary[]): string {
  ...
  return `[子代理结果汇总（V3.5，只含 summary 不含 transcript）]\n${body}`;
}
