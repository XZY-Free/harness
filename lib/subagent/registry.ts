...
/**
 * V3.5 Stage A：子代理 registry——SubagentDefinition/Run CRUD + 生命周期编排。
 *
 * 子代理执行模型（蓝图 §5.5）：进程内嵌套 streamText 循环 + SubagentRun DB 行做审计/状态
 * + transcript 落 artifact。本模块只管数据层与生命周期约束（状态机 / writeScope 互斥 / 并发上限 /
 * outputSchema 校验），**不接执行**（Stage C 的 executeSubagent 才跑 streamText）。
 *
 * 四个命门（计划执行规矩 §3）在此落地：
...
/** 单父 thread 并发子代理软上限（蓝图 §5.5 / 计划 §1 决策）。 */
...
/**
 * S1（04-G17）：子代理定义不得包含「再 spawn」能力工具（嵌套深度=1，defense-in-depth 早期拒绝）。
...
export async function createDefinition(params: {
  ...
  // S1（04-G17）：早期拒绝含 spawn 能力的定义（不依赖运行时剥离）
 ...
 * S1（04-G18）：展开 brace `{a,b}` 为多个前缀取最宽（fail-closed：brace 视为多前缀，
...
export async function updateRunStatus(
  ...
  // P1-12: CAS——把读到的 status 作为 expectedStatus,UPDATE 仅在该值未变时生效,防并发覆盖。
  ...
  if (!updated) {
    // P1-12: CAS 冲突——状态已被并发改写(canTransition 在调用前已校验,此处直接放弃)。
