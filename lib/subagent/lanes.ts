 ...
 *
 * 退化策略（计划 §1）：
 * - researcher：无 V3.4 时退化为 workspace+文档只读；V3.4 已落地 → 含 web 工具（webFetch/webSearch/searchDocs）。
 * - verifier：无 V3.6 时退化为测试结果核查（无浏览器截图）；V3.6 未落地 → 不含截图工具。
 *
 * executor 角色在 enum 内,S1（04-G7）起预置默认 lane（写能力,含写工具 + runBuild/runTests 验证）。
...
/**
 * S1（04-G7）：executor lane outputSchema。
 ...
 * researcher 含 V3.4 web 工具；verifier 不含 V3.6 截图工具（未落地）。
 */
export const DEFAULT_LANES: Record<SubagentRole, SubagentLaneSpec | null> = {
  ...
  // S1（04-G7）：executor 默认 lane——写能力（实现/修改代码），含写工具 + runBuild/runTests 验证。
 ...
 * 供 spawnSubagent 的 role 入参解析用（Stage D 起 spawn 支持 definitionId | role）。
 */
