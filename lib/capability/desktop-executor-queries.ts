import { type CreateFileChangesInput, createFileChanges } from "@/lib/capability/artifact-queries";
import {
...
/**
 * 校验 Desktop 执行上下文一致性（W07-2 + 四重校验）。
 *
 * 校验规则：
 * - lease.tenantId === binding.tenantId === toolCall.tenantId（租户一致）
 * - lease.deviceId === binding.deviceId（Desktop 设备一致；L543）
...
export async function validateDesktopExecutionContext(
  ...
  // 设备一致（L543：binding.device_id 必须等于 lease.device_id）
...
/**
 * 记录 Desktop 命令产生的文件变更（W07 + + S08-C06）。
 *
