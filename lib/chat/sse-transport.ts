/**
 * V4 Phase B-1 + V7 S5-3：SSE 订阅式 ChatTransport（B-4 断连自动重连 + sequence 续传）。
 *
 ...
 * V7 S5-3：后端为每个 chunk 附带 sequence；前端记录 lastSeq，重连时传 afterSeq，
...
export interface SseChatTransportInitOptions {
  ...
  backoffMs?: number[];
  /** V6-M3-6：POST 成功后的回调（用于清理编辑重发状态等）。 */
  onSendSuccess?: () => void;
}
  ...
  implements ChatTransport<UI_MESSAGE>
{
  ...
  async sendMessages(
    ...
    // V6-M3-6：POST 成功后回调（用于清理编辑重发状态）
    this.onSendSuccess?.();

    // Step 2：GET SSE 订阅 runId，返回带断连重连的韧性流（B-4 + S5-3）
  ...
  /**
   * V7 S3-2：预设目标 runId，供后续 useChat.resumeStream() → reconnectToStream() 使用。
   *
   ...
   *
   * V7 S5-3：请求带 afterSeq，后端先从 DB 补 chunk，再接 broadcaster 实时流。
  ...
  /**
   * B-4 + V7 S5-3：构造带断连重连 + sequence 去重续传的韧性流，对 useChat 透明。
   *
      ...
      try {
        ...
        try {
          for (;;) {
            ...
            if (!value) continue;
            // V7 S5-3：按 sequence 去重（兼容无 sequence 的旧格式，用 -1 兜底）
