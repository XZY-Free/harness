 ...
 *
 * Runtime 责任面（蓝图 §8.2）：本模块是 `WorkspaceStore` 责任面的当前唯一实现——
 * 负责工作区路径解析与文件读写。Phase 2 Stage D 按「从轻、不造空壳」处理：
 * 当前仅单一实现，抽 TS interface 纯属 indirection 噪音（§13.3），故接口抽象
 * 推迟到 Phase 5——待第二个 runtime 实现（dev-server / 容器）出现时再抽稳定接口。
 *
 * Phase 4-4 切片 B2：补 read / delete / stat，并硬化 write 的安全边界。
...
export async function writeWorkspaceFile(
  ...
  // V6-M2-3: 原子写入（B3）—— 先写临时文件再 rename，防崩溃留下半写文件
...
/**
 * V6-M2-3: per-path 互斥锁（B3）—— 防并发 editFile 丢失改动。
...
/**
 * V10 Phase 7-1：从 ReadableStream 流式写入 workspace 文件。
 *
