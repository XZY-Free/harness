/**
 * §03.2: Revision 执行资格唯一跨领域 Reader Port。
 *
 * 只提供两种明确语义：
 ...
 * 不得创建第三套 Reader。
 *
 * 参见：SnowHarness专题01最终差距整改与正式链路收口实施方案 §03.2
 */
 ...
 *
 * 实现必须：
 * - 真实读取所有证据字段（§03.6: 禁止硬编码）
 * - 使用调用方传入的 DB Session 或 Transaction（§03.3: 禁止内部使用全局 db）
 * - Policy 为 null 仅当 Route 未引用 Policy（§03.4）
 */
