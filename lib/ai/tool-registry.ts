/**
 * V3.0 Stage B：内置工具统一元数据 registry。
 *
 * 目的：为后续权限引擎、context manifest、Studio 展示和审计提供稳定的工具元数据来源。
 * V3.0 只登记现有内置工具，**不新增 agent 可见工具**，不改变 `buildTools` 的可见集合
...
export type ToolMetadata = {
  ...
  /**
   * S1 修复（01-P2-7）：工具默认超时（ms）。execute 类工具的硬编码超时统一收口到元数据 +
   * `resolveToolTimeoutMs`，运维可经 env 覆盖。0/undefined=用全局默认。
   */
  defaultTimeoutMs?: number;
  /** S1 修复（01-P2-7）：工具超时硬上限（ms），resolveToolTimeoutMs 取 min(caller, default, max)。 */
  maxTimeoutMs?: number;
};
...
const REGISTRY: readonly ToolMetadata[] = [
  ...
  // ─── V3.7 Stage B/C：git / delivery 工具组（plan §6/§7） ────
  {
    name: "gitStatus",
    displayName: "Git 状态",
  ...
  // ─── V3.4 Stage B：web / docs 工具（外部资料访问，蓝图 §5.4） ────
  ...
  // V3.5：子代理并行工作流（蓝图 §5.5）。category 复用 command（不扩 ToolCategory enum）。
  ...
  // ─── V3.6 Stage B/C：浏览器 QA 工具五件套（plan §6/§7） ────
  ...
  // V3.8：部署工具（CI/CD webhook 交接，默认 ask，prod 强制 ask）
  {
