...
// probePreviewUrl 抽到 lib/runtime/preview-probe.ts(Phase 5 Stage C,避免 DevServerPreviewRuntime 循环 import)
...
export async function reportThreadReady(
  ...
  // beforeDelivery（Phase 4-1）：交付前必跑验证。未过 → fail-closed 拒绝交付，
  ...
  // S1（05-P2-8）：探活带静态预览 token（静态 server 现要求鉴权）
  const probe = await probePreviewUrl(url, { token });

  if (!probe.ok) {
    // §8.4：探活失败保持 previewUrl=null、status=executing；
  ...
  // V3.6 Stage D：QA gate——probe 通过后、ready_for_review 前跑确定性浏览器质量门。
  ...
  // S1（05-P2-8）：静态预览 server 要求 token 鉴权，QA gate 通过请求头带 token，避免 token 落 artifact。
...
export async function finalizeThreadRun(threadId: string): Promise<DataArtifact> {
  ...
  // 收尾切换状态：追加本轮最后一个 agent.status_changed，replay 据此判定最终态（§8.5）
  await appendThreadEvent(threadId, "agent.status_changed", {
    from,
    to: status,
    reason: status === "failed" ? "run_failed" : "run_idle",
  });
  // P1-13: CAS——仅当当前 status 仍为读取时的 from 才切换,防 cancel 并发把 cancelled 覆盖成 idle/failed
  await Promise.all([
    updateThreadPreviewUrl(threadId, null),
