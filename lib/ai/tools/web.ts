...
/**
 * V3.4 Stage B：web / docs 工具（蓝图 §5.4）。
 *
 ...
 *   大网页若抽取后仍超 toolOutputThreshold，由 V3.3a oversized 机制自动摘要。
      ...
      execute: async ({ query }) => {
        try {
          return await executeToolRun(threadId, "webSearch", { query }, async (signal) => {
            ...
            if (!r.ok) return r;
            // S1（06-P1-4）：追加 external.searched 事件供 Studio external 审计（fail-open）
      ...
      execute: async ({ query }) => {
        try {
          return await executeToolRun(threadId, "searchDocs", { query }, async (signal) => {
            ...
            if (!r.ok) return r;
            // S1（06-P1-4）：追加 external.docs_searched 事件供 Studio external 审计（fail-open）
