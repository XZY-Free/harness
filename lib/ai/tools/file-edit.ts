 ...
 * - deleteFile：复用 workspace.delete；默认 ask（Stage A rules.ts）。
 *
      ...
      execute: async ({ path, oldString, newString }) => {
        try {
          return await executeToolRun(
            ...
            async (signal) => {
              // V6-M2-3: per-path 互斥锁（B3）—— read-modify-write 整体加锁，防并发丢失
      ...
      execute: async ({ path, edits }) => {
        try {
          return await executeToolRun(
            ...
            { path, edits },
            async (signal) => {
              // V6-M2-3: per-path 互斥锁（B3）—— 批量 read-modify-write 整体加锁
              return withPathLock(`${threadId}:${path}`, async () => {
                try {
...
function parseAndCollectPaths(patch: string): { paths: string[] } {
  // S1 修复（01-P2-8）：复用 apply.ts 的 parsePatch，消除两份路径解析逻辑。
