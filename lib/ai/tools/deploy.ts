...
/**
 * V3.8 Stage D：部署工具（deployToEnvironment / deployStatus / rollback）。
 *
 * 生产级 CI/CD 交接（plan §8）：
 ...
 * @param commitSha V3.7 finalizeThreadRun 的来源 commit（可空）
 */
      ...
      execute: async ({ environment, imageTag }) => {
        return executeToolRun(
          ...
          async (signal) => {
            ...
            // S1（09-P1-5）+ P1-8: 原子占用 deploying 槽位——事务内 FOR UPDATE 锁 thread 行,
            ...
            try {
              ...
              // V6-M2-6: 读取并解密 per-thread CI/CD token（向后兼容明文）
        ...
        return executeToolRun(
          ...
          async (signal) => {
            ...
            if (deployment.cicdJobId && deployment.status === "deploying") {
              try {
                // V6-M2-6: 传入解密后的 per-thread token 给 queryStatus
      ...
      execute: async ({ deploymentId }) => {
        return executeToolRun(
          ...
          async () => {
            ...
            // P1-8: 回滚也原子占用 deploying 槽位,防并发回滚/部署触发多次 CI/CD
            ...
            try {
              ...
              // P1-18: 回滚也用 per-thread token(与部署一致),不能回退全局 token
