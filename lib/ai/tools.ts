 ...
 *
 * 工具：writeFile / readFile / listFiles / runCommand / runTests / reportReady
 * 全部经 executeToolRun 统一包裹，落 tool_runs + tool.* 事件。
 ...
 *
 * 文件读写经 `WorkspaceStore`、命令执行经 `ExecutionRuntime`（resolveRuntimes
...
function allTools(
  ...
  /**
   * 注入的 runtime handle（供子代理用 ScopedWorkspaceStore 收窄写范围）。
  ...
  return {
    ...
    // Stage C：后台任务四件套（startBackgroundTask/readTaskLogs/stopBackgroundTask/listBackgroundTasks）
    ...buildCommandTaskTools(threadId, rt, runtimeType),
    // Stage D：工程命令工具（runBuild/installDependencies）
    ...buildCommandBuildTools(threadId, rt, runtimeType),
    // Stage B/C：git / delivery 工具组（gitStatus/gitDiff/gitCheckpoint/gitRestoreCheckpoint/gitCreateBranch/gitCommit/gitPush + createPullRequest/deliverySummary）
    ...buildGitTools(threadId),
    // 长期记忆工具（rememberFact，经 executeToolRun 收口）
    ...buildMemoryTools(threadId),
    // Stage B：web / docs 工具（webFetch/webSearch/searchDocs，经 executeToolRun + 域名治理）
    ...buildWebTools(threadId),
    // Stage C：MCP 通用入口（listMcpTools/callMcpTool，经 executeToolRun + mcpEvaluate 默认 ask）
    ...buildMcpTools(threadId),
    // 自定义工具（webhook/script executor，经 executeToolRun + customEvaluate 默认 ask）
    // declarations 由调用方异步预加载后传入（保持 buildTools 同步、零回归）；空 → 无自定义工具
    ...buildCustomTools(threadId, customDeclarations),
    // Stage C：子代理控制工具（spawnSubagent/joinSubagent，父 agent 派生并行子代理用）
    ...buildSubagentControlTools(threadId),
    // Stage B/C：浏览器 QA 工具五件套（capturePreview/runBrowserCheck/runResponsiveCheck/
    // runAccessibilitySmoke/visualVerdict），Playwright 驱动，证据落 .snow/runtime artifact
    ...buildQaTools(threadId, rt, runtimeType),
    // Stage D：部署工具（deployToEnvironment/deployStatus/rollback），经 CI/CD webhook 交接
    ...
    // Stage E：预览控制三工具（蓝图 全集补齐）。经 executeToolRun 包裹，调 PreviewRuntime。
    startPreview: tool({
      description:
 ...
 *
 * 实现层每个工具仍经 executeToolRun 包裹（不回归）。
 */
