...
/**
 * V3.6 Stage A：QA 证据落盘 + 事件 payload 构造（plan §4.1 / §5）。
 *
 * 证据**不落 DB blob**，落文件（对齐 V3.2 后台任务日志的 artifact 模式）：
 ...
 * 不被静态 preview server 服务）。这与 V3.2 的 container→workspace 限制不同——V3.2 那样是因为
...
export async function readQaArtifact(threadId: string, fileName: string): Promise<Buffer | null> {
  try {
    ...
    // P1-16: 防 symlink 越界——realpath 解析符号链接后再校验在 QA 目录内,
...
/**
 * S1 修复（05-P1-5）：清理 thread 的全部 QA 证据文件（截图 + 报告 JSON）。
 *
...
// ─── 事件 payload 构造 ───────────────────────────────────────
// V10 Phase 7-5：QaFailure / QaCheckPassedPayload / QaCheckFailedPayload 类型
