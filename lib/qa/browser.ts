/**
 * V3.6 Stage A：Playwright 浏览器生命周期（蓝图 §8 预览与 QA / plan §5）。
 *
 * 设计取舍（plan §1 决策）：
...
/**
 * V10 Phase 7-5：QA schema 类型从 `lib/desktop/qa-schema` 统一导出（Server/Desktop 共享）。
 *
 ...
 *
 * S1（05-P1-4）：经 maxBrowserConcurrency 信号量限流，超限排队等待；page.close() 释放信号量。
 */
export async function openQaPage(viewport: QaViewport, hooks: QaPageHooks = {}): Promise<QaPage> {
  ...
  if (hooks.onConsole) {
    page.on("console", (msg: never) => {
      ...
      // V10 Phase 7-5：`level` 对齐 ConsoleEntry（Playwright msg.type() 返回
  ...
  if (hooks.onResponse) {
    page.on("response", (res: never) => {
      // V10 Phase 7-5：补充 method/statusText/mimeType 富信息（best-effort，缺失不阻断）。
      const r = res as {
        url(): string;
        status(): number;
        statusText?(): string;
...
// V10 Phase 2：QA storageState 派生已移除。
// 原 V9 从 UserBrowserProfile 解密登录态供 QA 隐藏 context 继承，V10 删除服务端
// 用户浏览器链路后无数据来源。QaPageHooks.storageState 字段保留（Phase 7 Desktop
