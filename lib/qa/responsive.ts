...
/**
 * V3.6 Stage C：runResponsiveCheck——多 viewport 响应式布局断言（plan §7）。
 *
...
interface LayoutProbe {
  ...
  textLength: number;
  hasMedia: boolean;
  /** S1（05-P2-2）：可见文本/交互元素互相重叠的对数（getBoundingClientRect 相交）。 */
  overlapCount: number;
  /** S1（05-P2-2）：text-overflow:ellipsis 且 scrollWidth > clientWidth 的截断元素数。 */
  truncatedCount: number;
}

// S1（05-P2-2）：追加重叠检测（可见元素 getBoundingClientRect 两两相交）+ 文本截断检测
