...
/**
 * Phase 2 Stage E：事件回放 / projector（方案 §9.2）。
 *
 * 纯函数：输入某 thread 的 append-only 事件序列，输出当前投影状态。
 * 只负责 thread 状态解释，不重建完整消息流（§9.2.2）。
 *
 * 投影规则（§11 Stage E）：
 ...
 *
 * V3.1：`tool.approval_requested` / `tool.approval_resolved` 不参与 status / artifact
 * 投影（审批状态由 ToolApprovalRequest 表承载，见 lib/permission/）。未知事件类型一律忽略。
 *
 * V3.5：`subagent.spawned` / `subagent.joined` / `subagent.failed` 不参与 status / artifact
...
export type ThreadProjection = {
  ...
  /**
   * S1（04-G10）：子代理投影。从 subagent.spawned/joined/failed 事件推导每个 run 的最近状态,
...
export function projectThread(events: ThreadEvent[]): ThreadProjection {
  ...
  // S1（04-G10）：subagent run → 最近状态（spawned=running, joined=completed/cancelled, failed=failed/timed_out）
