...
/**
 * Phase 4-4：Agent Studio 后台只读 / 聚合查询。
 *
...
/**
 * Studio 列表行：Skill + 同步映射摘要（02 文档 §7.1）。
  ...
  return (
    db
      .select({
        ...
        // S1（09-P2-3）：per-thread CI/CD token（同上，select 漏选补齐）
...
/**
 * 跨 thread 聚合某用户可见的最近 artifact 事件（Phase 4-4 切片 B2，owner-scoped）。
 *
