...
// 12-P1-3：事件总线广播——appendThreadEvent 成功后广播到进程内 hub，供 SSE 端点即时推送。
...
/**
 * P1-7: 事务客户端类型(db 或 db.transaction 的 tx),供函数可选接入事务。
...
// ─── Owner-scoped Thread Queries (Phase 4-3) ────────────────
 ...
 *
 * S1（08-P1-2）：retention 明细清理与彻底物理删除共用此清单,避免两套删表逻辑漏表。
...
const THREAD_CHILD_TABLES: ReadonlyArray<{
  ...
  // P1-1: V7/V8 新增三表对 threadId 建了 DB FK(noAction),必须先删,否则 delete thread 触发 FK 违反。
  ...
  // V10 Phase 2：V9 浏览器表（browserSession/browserDownload/userBrowserProfile）
  // 已由 migration 0059 删除，物理删除级联清单不再包含它们。
];

/**
 * S1（08-P1-2）：物理删除 thread + 全部子表数据。
 *
 ...
 * 更新 thread status。
 *
 * P1-13: 可选 `expectedFrom` 做 CAS——SQL 加 `WHERE status = expectedFrom`,
...
export async function reapStaleThreads(maxAgeMs = 10 * 60_000): Promise<string[]> {
  ...
  // P1-10: 多实例部署下,排除有"新鲜活跃 run"(他实例正在跑)的 thread,防误杀。
...
/**
 * S1 修复（03-P2-3）：pinned facts 原子化读-改-写。
 *
...
export async function mutateThreadPinnedFacts(
  ...
  return db.transaction(async (tx) => {
    ...
    // S1（08-P2-3）：json 列 zod 校验（fail-closed，mutator 返回非法结构抛错不落库）
...
/**
 * P1 修复（05 QA P1-1）：更新 thread 的 reviewState。
 *
...
/**
 * V6-M2-6（G3）：更新 thread 的 per-thread CI/CD API token。
 ...
 *
 * P1-24: 加 limit + before 游标,防长会话全量加载 OOM。默认 500 条(覆盖绝大多数会话),
 * 上限 5000;export 等需全量的场景显式传大 limit。before 游标用于向前翻页(取游标之前的消息)。
 *
 * P0 修复（08 DB P1-3）：orderBy 加 id 作 tie-breaker,防同毫秒消息乱序。
 */
...
export async function saveMessages(
  ...
  // S1（08-P2-3）：json 列轻量校验。parts 须为数组且每项有 type 字符串（防脏数据写入）。
...
export async function deleteMessagesFromId(threadId: string, messageId: string): Promise<number> {
  ...
  if (!target) return 0;
  // S1（08-P1-4）：(createdAt, id) 复合比较，避免同毫秒消息误删（原 createdAt >= 删同秒 id 更大的）。
  const result = await db
    .delete(message)
    .where(
      and(
...
// ─── Thread Event Queries (Phase 2 append-only 事件流) ───────

/**
 * 分配下一个 sequence 值（§13.2）。
 * 取当前 thread 最大 sequence + 1；无历史记录时从 1 开始。
 * S1（08-P1-7）：原子性由 appendThreadEvent 的 unique 冲突重试保证（非此函数）。
 */
...
export async function appendThreadEvent(
  ...
  /** V7 S2-3：归属 ThreadRun（nullable 兼容历史事件和纯 thread 管理事件）。 */
  runId?: string | null,
): Promise<ThreadEvent> {
  ...
  return db.transaction(async (tx) => {
    ...
    // 12-P1-3：成功落库后广播到进程内事件总线，供 SSE 端点即时推送各面板。
...
/**
 * V3.6：查询某 thread 的 QA 检查事件（qa.check_passed / qa.check_failed），按 sequence 降序
...
/**
 * P1 修复（05 QA P1-1）：统计 thread 最近连续 QA gate 失败次数。
 *
...
// ─── Tool Run Queries (Phase 2 结构化工具执行记录) ───────────

/**
 * 创建一个 tool run 记录（默认状态 running）。
 *
 * V3.1：`status` 可选，ask 暂停时传 `awaiting_approval`，以区分「被治理暂停」与「业务失败」。
 *
...
export async function createToolRun(params: {
  ...
  /** V7 S2-2：归属 ThreadRun（nullable 兼容历史记录）。 */
  runId?: string | null;
}): Promise<ToolRun> {
  // S1（08-P2-3）：json 列 zod 校验（fail-closed，脏数据抛错不落库）
...
export async function finishToolRunSuccess(
  ...
  // S1（08-P2-3）：json 列 zod 校验（fail-closed）
...
export async function failRunningToolRunsForThread(
  ...
  // P1-22: 限定 runId 防误杀——cancel/markFailed 后用户立即发新消息触发新 run,
...
export async function createSkillVersion(
  params: {
    skillId: string;
    version: number;
    /** 迁移期保留兼容；目录形态下新版本留空，内容由 skills/<name>/SKILL.md 承载 */
...
/**
 * 切换 skill 当前版本。
 * P1-14: 可选 `expectedCurrentVersionId` 做 CAS——仅当当前 currentVersionId 匹配时才切换,
 ...
 * P1-14: CAS——仅 status != archived 时归档(防并发 unarchive/archive 竞态)。
 */
...
// ─── SkillSyncMapping（02 文档 §4.3 同步映射）────────────────
...
// ─── Settings 用户/角色管理 Queries (Phase 4-4 切片 B3) ──────
//
...
// ─── Policy Config 写入 (Phase 4-4 切片 B3) ──────────────────
...
/**
 * V6-M3-4（C5）：记录 policy 配置变更历史（before/after 快照 + changedKeys）。
...
// ─── Agent / Provider Profile Queries (Phase 4-4 切片 B1: 只读档案) ────
//
// 仅档案存储 + 只读展示，**不接入 lib/ai/ runtime**（§2.2 / 非目标）。
// 列表查询无 owner 范围——档案是全局只读，非租户隔离数据。

/**
 * 列全部 agent 档案，按 createdAt asc。
 *
 * S1（08-P2-2）：默认过滤 deletedAt isNull(软删不展示);includeDeleted=true 看全部(含已软删)。
 */
 ...
 *
 * S1（08-P2-2）：默认过滤 deletedAt isNull;includeDeleted=true 含已软删(seed 幂等判定用,
 * 避免软删后重复建同名 agent)。
 */
...
/**
 * S1（08-P2-2）：软删 agent(设 deletedAt)。身份层保留,可恢复;listAgents 默认不展示。
...
// ─── Admin Audit Queries (Phase 4-4 切片 C: append-only 审计) ──
//
...
// ─── Admin Audit + 业务写 事务封装 (Phase 4-4 切片 C) ─────────
//
// 用户角色覆盖属敏感写操作，按 §4.3 必须「角色替换 + audit」同事务，audit 写失败则
...
// ─── Context Snapshot Queries (V3.0 Stage C) ────────────────
//
// 每次模型调用前构建的 context manifest 落库。manifest 只记来源与摘要，调用方负责
// 不塞完整 prompt / 用户消息正文 / 完整工具输出（§6.2 隐私约束）。
...
export async function saveContextSnapshot(params: {
  ...
  /** V3.3b Stage 0：本轮是否压缩装配（与真实模型输入一致）。 */
  compressed?: boolean;
  /** V3.3b Stage 0：本轮装配后真实模型输入 token（nullable 兼容旧快照）。 */
  ...
  /** V7 S2-1：归属 ThreadRun（nullable 兼容历史快照）。 */
  runId?: string | null;
...
export async function cleanupOldSubagentRuns(retainDays = 14): Promise<number> {
  ...
  // P1-18: 先 select 待删 run 的 transcriptPath,删行前 unlink 文件,防孤儿 transcript 累积
  const rows = await db
...
// ─── Thread Plan / Todo Queries (V3.0 Stage D) ──────────────
//
// thread 级计划容器与条目。V3.0 不要求 agent 自动写计划，仅提供数据层与查询接口，
...
// ─── Tool Permission Rule / Approval Request Queries (V3.1) ──
//
// V3.1 ask/deny/ask 权限引擎的数据层。规则默认从 PolicyConfig 派生（lib/permission/rules.ts），
...
/**
 * 创建一条持久化权限规则（DB 覆盖默认规则）。V3.1 无 UI 编辑入口，供 seed/测试用。
 *
 * S1（07-P2-5）：actorUserId 非空时同事务落 permission_rule.created 审计行（input 经脱敏）。
...
export async function requestApprovalAtomic(params: {
  ...
  return db.transaction(async (tx) => {
    ...
    const approval: ToolApprovalRequest = {
      ...
      status: "pending",
      approvedScope: null,
      // V6-M3-5（C4）：审批请求记录 projectId，供 project scope 跨 thread 匹配
      projectId: params.projectId ?? null,
      resolvedBy: null,
      resolvedAt: null,
      createdAt: now,
...
export async function resolveApprovalRequest(params: {
  ...
  // session scope（07-P1-6）：决议时把 expiresAt 收紧到短 TTL，
...
export async function findMatchingApprovals(params: {
  ...
  // V6-M3-5（C4）：scopeFilter 增加 project scope 跨 thread 匹配
...
// ─── Background Task Queries (V3.2) ─────────────────────────
//
// V3.2 命令治理：可恢复后台任务的数据层。DB 行作可审计/可列表/进程重启标记孤儿的真实来源；
...
export async function createMemoryRow(params: {
  ...
  // S1（08-P2-3）：json 列 zod 校验（fail-closed，provenance 必须非空防孤儿记忆）
...
export async function updateMemoryRow(
  ...
  patch: {
    ...
    provenance?: MemoryProvenanceEntry[];
    expiresAt?: Date | null;
    /** S1（06-P1-5）：text 更新（同步 textHash）。 */
    text?: string;
    textHash?: string;
  },
): Promise<MemoryEntry | null> {
...
// ─── Memory Embedding Queries (V3.3b Stage B) ──────────────
//
...
export async function upsertEmbeddingRow(params: {
  ...
  // P1-27: 改 INSERT ... ON DUPLICATE KEY UPDATE(依赖 memoryId+provider 唯一索引),
...
/**
 * S1 修复（06-P1-2）：批量取多条 memory 的 active embedding（单查询替代 N+1）。
...
/**
 * S1 修复（06-P2-3）：provider 切换后老 embedding fallback。
...
/**
 * V6-M2-7：批量取任意 provider 的 active embedding（替代 N+1 循环调用 getActiveEmbeddingRowAnyProvider）。
...
/**
 * S1 修复（06-P2-4）：清理过期记忆。物理删除 expiresAt < now 的 active 记忆（含其 embedding 行）。
 * 由 retention 定时任务调用。@returns 删除条数
 */
...
/**
 * S1（08-P2-5）：清理超期 ContextSnapshot（全 thread，不限终态）。
...
// ─── MCP Server Config Queries (V3.4) ───────────────────────
//
...
export async function createMcpServerConfig(params: {
  ...
  const row: McpServerConfig = {
    ...
    // S1（10-P2-5）：协商字段建时为 null,连接成功后 recordMcpServerHandshake 回写
...
/**
 * S1（10-P2-5）：记录 MCP server 连接协商结果(server 版本 + 能力)到 DB。
 *
...
// ─── External Source 审计查询 (V3.4) ────────────────────────
//
// external.fetched 事件复用 ThreadEvent payload（不单独建表，§4.3 决策）。
...
// ─── Subagent Definition / Run Queries (V3.5) ───────────────
//
// V3.5 子代理数据层。Definition = 可派生模板；Run = 一次执行的审计/状态行。
...
export async function updateSubagentRun(
  ...
  patch: {
    ...
    startedAt?: Date | null;
    finishedAt?: Date | null;
    /** P1-12: CAS——仅当当前 status 匹配时才更新,防并发状态机覆盖。 */
  ...
  if (!existing) return null;
  // P1-12: CAS 校验——若调用方传了 expectedStatus 且与当前不符,视为并发冲突返回 null。
  ...
  // P1-12: UPDATE 也带 status 守卫,防 select 与 update 之间的 TOCTOU。
...
/**
 * P1-8: 原子占用 deploying 槽位。事务内 SELECT ... FOR UPDATE 锁 thread 行,
...
/**
 * S1（09-P1-4）：列所有 deploying 状态的 deployment（跨 thread，供后台轮询）。
 */
...
export async function completeThreadRun(
  ...
  const now = new Date();
  // P1-1:CAS 守卫——仅活跃态可迁 completed,防 cancel/markFailed/stale reaper 并发覆盖终态。
  await db
    .update(threadRun)
    .set({
      status: "completed",
...
export async function failThreadRun(runId: string, error: string): Promise<void> {
  const now = new Date();
  // P1-1:CAS 守卫——仅活跃态可迁 failed,防已终态 run 被覆盖。
  await db
    .update(threadRun)
...
export async function cancelThreadRun(runId: string, cancelReason: string): Promise<void> {
  const now = new Date();
  // P1-1:CAS 守卫——仅活跃态可迁 cancelled,防已终态 run 被覆盖。
  await db
    .update(threadRun)
 ...
 * V7 S3-3：同时更新对应 Thread.status（executing → failed）+ 写 ThreadEvent 审计事件。
...
export async function markStaleThreadRuns(cutoff: Date): Promise<number> {
  ...
  // P1-26: CAS——UPDATE 加 status='running' 守卫,防 select 后 run 被 cancelRun 标 cancelled
  ...
  // V7 S3-3：逐 run 写 ThreadEvent 审计事件 + 更新 Thread.status
...
/**
 * V7 S4-1：获取单次 run 的完整证据链（run + messages + events + toolRuns + contextSnapshots）。
...
// ─── V7 S5-1: RunTranscriptChunk Queries ────────────────────

/**
 * 追加一个 RunTranscriptChunk。
 *
 * 未传入 sequence 时自动分配 run 内递增 sequence（先查当前 max，再 +1）。
 * V7 S5-3：由 broadcaster 前置分配 sequence，保证内存流与 DB 的 sequence 一致，
 * 避免并发写入导致顺序错乱。
 */
