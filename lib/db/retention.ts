 ...
 * P1-11: 扩展到全部执行过程明细——原 5 张(threadEvent/toolRun/contextSnapshot/
...
/**
 * P2 修复（08 DB P2-4）：已结束 thread 明细数据 TTL 清理。
 *
...
export type PurgeResult = {
  ...
  /** S1(08-P1-2):物理删除主记录条数(软删超 hardDeleteRetentionDays 的 thread)。 */
...
export async function purgeExpiredThreadDetails(
  ...
  // S1(08-P1-2):明细按表删除,复用 RETENTION_DETAIL_TABLES 与 deleteThreadRecursive 对齐,防漏表。
  ...
  // P1-11: 串行删除(按 FK 依赖顺序),防 threadRunSkill/runTranscriptChunk 引用 threadRun 时并行删触发 FK 违反
