#!/usr/bin/env tsx
/**
 * P2 修复（08 DB P2-4）：已结束 thread 明细数据 TTL 清理 CLI 入口。
 *
 * 用法:
...
async function main() {
  ...
  // S1（03-P2-7）：supersede 链 GC —— 物理删除 7 天前已 supersede 的旧 summary（全 thread）
  ...
  // S1（04-G14）：SubagentRun TTL —— 物理删除 14 天前终态 run
  ...
  // S1（06-P2-4）：清理过期记忆（expiresAt < now）
