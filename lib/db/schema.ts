 ...
 * - User：单租户固定用户（Phase 4 接 SSO）
 */

// ─── Thread Status ───────────────────────────────────────────

// V3.0：扩展为完整 agent 生命周期状态。新增值追加在尾部，保留既有 enum 存储顺序，
// 现有 chat 路径仍只写 idle/executing/ready_for_review/failed，不影响行为（§0.3）。
...
// ─── Thread Event Types (§6.1.1 权威表) ─────────────────────

/**
 * Phase 2 事件类型——取自蓝图 §7.3 词表。
 * 本枚举是全文事件名的唯一权威定义。
 *
 * V3.7 激活：delivery.succeeded / delivery.failed（交付终态）+ git.checkpoint_*。
...
export const THREAD_EVENT_TYPES = [
  ...
  "artifact.created",
  "artifact.updated",
  // V3.0：context manifest 与 plan/todo 事件（只追加，不改旧事件含义）
  "context.snapshot_created",
  // V3.3a：上下文压缩（只追加，不改旧事件含义）
  ...
  "task.stopped",
  "task.failed",
  // V3.7：交付生命周期事件（从注释态激活）+ checkpoint 事件
  "delivery.succeeded",
  "delivery.failed",
  ...
  "memory.created",
  "memory.revoked",
  // V3.3b Stage B：embedding 索引重建（payload 含 memoryId/provider/model/status/dimension?/errorCode?）。
  // 纯应用层 const 追加——事件 type 在 DB 是 varchar（非 enum 约束），故零 migration SQL。
  "memory.reindexed",
  // V3.4：外部资料与 MCP 调用（只追加，不改旧事件含义）
  // external.fetched：webFetch/webSearch/searchDocs 一次外部资料访问（payload 含来源/hash/artifact）
  // external.searched：webSearch 一次搜索（S1 06-P1-4，payload 含 query/resultCount）
  // external.docs_searched：searchDocs 一次文档搜索（S1 06-P1-4）
  ...
  "subagent.joined",
  "subagent.failed",
  // V3.6：浏览器 QA gate 生命周期（只追加，不改旧事件含义）
  ...
  "qa.check_passed",
  "qa.check_failed",
  // V3.8：部署与 secret 生命周期（只追加，不改旧事件含义）
  ...
  "secret.rotated",
  "secret.revoked",
  // P1 修复(01 AI Core P1-3): agent 每轮 token 用量审计(供计费/用量分析)。
...
// V3.1：追加 awaiting_approval——ask 暂停时 tool_runs.status 设为该值（非 failed），
...
export const user = mysqlTable(
  "User",
  {
    ...
    // 公司用户中心 subject / employee id（Phase 4-3：SSO 身份映射键）
...
export const thread = mysqlTable(
  "Thread",
  {
    ...
    // P1-10:fsp=3 毫秒精度,SSE since 游标 gt(updatedAt) 不再丢同秒事件。
    ...
    // V6-M2-6（G3）：per-thread CI/CD API token，AES-256-GCM 加密存储（JSON: keyId + ciphertext）。
...
export const message = mysqlTable(
  ...
  (t) => ({
    // P0 修复（08 DB P1-1）：message 表加 threadId+createdAt 复合索引。
...
export const threadEvent = mysqlTable(
  "ThreadEvent",
  {
    ...
    threadId: varchar("threadId", { length: 36 })
      .notNull()
      .references(() => thread.id),
    // thread 内单调递增序号（§13.2：单 thread/低并发语境下用简单递增）
    ...
    runId: varchar("runId", { length: 36 }),
    // P1-10:fsp=3 毫秒精度,SSE since 游标 gt(createdAt) 不再丢同秒事件。
    createdAt: datetime("createdAt", { mode: "date", fsp: 3 })
      .notNull()
      .$defaultFn(() => new Date()),
  },
  (t) => ({
    // (threadId, sequence) 唯一：thread 内序号不重复（§13.2 应用层简单递增 + DB 兜底，真并发冲突 fail-loud 而非静默错乱）
...
export const toolRun = mysqlTable(
  ...
  (t) => ({
    // 高频查询维度（§6.2.1）：某 thread 的工具执行、按状态/时间筛失败
...
export const skill = mysqlTable(
  "Skill",
  {
    ...
    // S1（11-P2-6）：skill 所有者（创建者 userId），用于权限隔离（owner 可写自己的 skill）
    ownerUserId: varchar("ownerUserId", { length: 36 }),
    // 02 文档 §4.2：skill 来源。local=本地自建(可编辑)，capability-market=同步镜像(只读)。
    source: mysqlEnum("source", SKILL_SOURCES).notNull().default("local"),
    createdAt: datetime("createdAt", { mode: "date" })
      .notNull()
      .$defaultFn(() => new Date()),
    // S1（08-P2-2）：软删除（原物理删除不可恢复）
    deletedAt: datetime("deletedAt", { mode: "date" }),
  },
  (t) => ({
...
export const skillVersion = mysqlTable(
  "SkillVersion",
  {
    ...
    // agent system prompt（迁移期保留兼容；目录形态下新版本不再写入，留空。
...
/**
 * capability-market 远端资产与本地 Skill 的同步映射（02 文档 §4.3）。
 *
 ...
 *
 * 不用远端 asset_id 替代本地主键（02 文档 §4.1）：本地主键已被 Studio / Resolver /
...
// ─── RBAC (Phase 4-4: role → permission, user → role) ────────
...
// ─── Policy Config (Phase 4-4: policy DB 化) ────────────────
...
/**
 * V6-M3-4（C5）：Policy 配置变更历史表（版本管理 + before/after 快照）。
 *
...
// ─── Agent / Provider Profiles (Phase 4-4 切片 B1: 只读档案,不接 runtime) ─

/**
 * agent 档案（蓝图 §11.3）。
 *
 * Phase 4-4 B1 只做档案存储 + 只读展示，**不接入 lib/ai/ runtime 执行链**：
 ...
 *   不依赖 MySQL JSON default（§2.2 / 约束 6）。
 */
 ...
 * 不做运行时切换（§2.2 / 非目标）。apiKeyRef 存 env 引用名（如 "LLM_API_KEY"），
 * **不落明文 secret**（约束 6）。
 */
// ─── Admin Audit Log (Phase 4-4 切片 C: 后台敏感写操作审计) ────
...
export const ADMIN_AUDIT_ACTIONS = [
  ...
  "skills.deleted",
  "skills.matched",
  // 02 文档 §7.3：capability-market 同步审计
  "skills.synced",
  "skills.unsynced",
  "workspace.file.written",
  "workspace.file.deleted",
  // S1（07-P2-4）：高危工具执行审计；S1（07-P2-5）：权限规则变更审计
  "tool.high_risk.executed",
  "permission_rule.created",
  "permission_rule.updated",
  "permission_rule.deleted",
  // S1（08-P1-2）：admin 彻底删除 thread(物理删主记录 + 子表)
  "thread.purged",
  // P1-9:审批决议独立 action(原复用 tool.high_risk.executed/policies.updated 语义错乱)
  "approval.resolved",
...
// ─── V3.0 Agent Kernel: Context Snapshot / Plan / Todo ─────────
//
// V3.0 只铺数据地基与只读观测，不接入 agent 自动决策（§0.2）。
// - ContextSnapshot：每次模型调用前的上下文来源清单，只记来源与摘要，
//   不落完整 prompt / 用户消息正文 / 完整工具输出（§6.2 隐私约束）。
// - ThreadPlan / ThreadPlanItem：thread 级计划容器与条目，V3.0 不要求 agent 自动生成，
...
/** ThreadPlan 来源。agent/user/system。V3.0 暂不由 agent 自动写入。 */
...
export const contextSnapshot = mysqlTable(
  "ContextSnapshot",
  {
    ...
    // 粗略 token 估算（layers 之和；V3.3b Stage 0：有 packageManifest 时为真实 afterTokens）
    estimatedTokens: int("estimatedTokens").notNull(),
    // V3.3b Stage 0：本轮是否压缩装配（与真实模型输入一致，非静态推断）
    compressed: boolean("compressed").notNull().default(false),
    // V3.3b Stage 0：本轮装配后真实模型输入 token（压缩后 ≠ estimatedTokens；nullable 兼容旧快照）
    afterTokens: int("afterTokens"),
    // V8 Skill Run Resolver：本轮 Resolver 输入摘要（availableSkillCount / uiSelectedSkillIds）。
    // 不含完整 SKILL.md（懒加载约束）。nullable 兼容历史快照。
...
/** 摘要类型。subagent 为前置空 slot，V3.5 填充。 */
...
/**
 * Thread 计划容器（V3.0 §7.2）。一个 thread 可有多个 plan，但同时只有一个 active。
 */
...
/**
 * Plan/Todo 条目（V3.0 §7.2）。parentId 预留轻量层级；position 排序；
 * evidence 存测试/toolRun/artifact 引用，V3.0 可为空。
 */
...
// ─── V3.1: Tool Permission Rule / Approval Request ───────────
//
// V3.1 把 deny-only 的 beforeTool 升级为 allow/deny/ask 三态权限引擎（蓝图 §5.3 / §12 V3.1）。
// - ToolPermissionRule：持久化规则。默认规则从 PolicyConfig 派生（protectedPaths/commandDenyList
//   → deny；deleteFile/applyPatch/multiEditFile → ask），DB 行作覆盖。V3.1 不做完整 UI 编辑（留后续）。
// - ToolApprovalRequest：一次 ask 暂停产生的待审批记录。批准复用语义由
//   status=approved + approvedScope + argFingerprint 表达，不单建 ToolApproval 表（§4.3）。
...
export const toolApprovalRequest = mysqlTable(
  "ToolApprovalRequest",
  {
    ...
    // V6-M3-5（C4）：project scope 审批跨 thread 复用——记录审批时的 projectId
    projectId: varchar("projectId", { length: 36 }),
  ...
  (t) => ({
    ...
    // V6-M3-5（C4）：project scope 跨 thread 查询索引
...
// ─── V3.2: Background Task ──────────────────────────────────
//
// V3.2 命令治理：可恢复、可审计的后台长跑任务（蓝图 §12 V3.2）。
...
export const gitCheckpoint = mysqlTable(
  "GitCheckpoint",
  {
    ...
    // S1（09-P2-4）：变更文件列表（git diff --stat 摘要），供回滚前快速判断 checkpoint 内容
...
// 不自动写入：只能经 rememberFact 工具或 Studio curate 写入，provenance 必填（蓝图 §6.5/§14）。
...
// MemoryEmbedding：一条 memory 的向量（Stage B 混合检索 semantic rerank 用）。
...
// ─── V3.4: MCP Server Registry / Custom Tools ───────────────
//
// V3.4 外部能力接入（蓝图 §5.4）。MCP server 与自定义工具都是「外部不可信能力」，
...
export const mcpServerConfig = mysqlTable(
  "McpServerConfig",
  {
    ...
    // S1（10-P2-5）：连接时协商到的 server 版本/能力（best-effort 回写，审计兼容性）
...
// ─── V3.5: Subagent Definition / Run ─────────────────────────
//
// V3.5 子代理与并行工作流（蓝图 §5.5 / §12 V3.5）。
...
// ─── V3.8: Secret Mount（加密存储的 secret 挂载）──────────────
//
// 生产级 secret at rest 保护（plan §1/§4.1）：
...
export const deployment = mysqlTable(
  "Deployment",
  {
    ...
    // 来源 commit（V3.7 finalizeThreadRun）
...
// V10 Phase 2：V9 内置浏览器表（UserBrowserProfile / BrowserSession /
...
export * from "@/lib/v11/schema/audit";
export * from "@/lib/v11/schema/agent";
export * from "@/lib/v11/schema/runtime";
export * from "@/lib/v11/schema/artifact";
export * from "@/lib/v11/schema/deployment-route";
export * from "@/lib/v11/schema/conversation";
export * from "@/lib/v11/schema/projection";
export * from "@/lib/v11/schema/skill";
export * from "@/lib/v11/schema/tool";
export * from "@/lib/v11/schema/catalog";
export * from "@/lib/v11/schema/capability-use";
export * from "@/lib/v11/schema/tool-call";
export * from "@/lib/v11/schema/workspace";
export * from "@/lib/v11/schema/workspace-lock";
