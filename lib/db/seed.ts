 ...
 * allowedTools 含全部 6 个工具（**必须含 reportReady**，否则 Phase 2 预览闸门开不了预览，
 * 见方案 §4.4 / §10 风险）。行为与 Phase 2 完全一致，不回归。
 *
 * reviewMode / artifactPolicy 仅存储，行为留 Phase 4（§6.3）。
 */
...
function buildDefaultSkillMd(): string {
  return [
    ...
    "description: 从想法到上线：示例全栈生成 skill（Phase 2 行为基线）",
...
// ─── 默认 policy seed（Phase 4-4 policy DB 化） ──────────────
...
// ─── 默认 provider / agent seed（Phase 4-4 切片 B1: 只读档案） ──

/**
 * 默认 provider 档案名（唯一幂等键）。
 *
 * provider 表只镜像当前 env 配置供后台只读展示，**不接 runtime**（runtime 仍走 env
 * aiConfig，见 §2.2 / 非目标）。apiKeyRef 存 env 引用名 `LLM_API_KEY`，**不落明文 secret**。
 */
...
export async function seedDefaultAgents(): Promise<{ created: boolean; skipped: boolean }> {
  // S1（08-P2-2）：includeDeleted 查全部(含软删),软删的 default agent 也算已存在,
