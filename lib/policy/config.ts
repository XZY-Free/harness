 ...
 *
 * policy 与 Phase 3 allowedTools 正交（§7）：allowedTools 控「工具可见性」（粗粒度），
...
export type PolicyConfig = {
  ...
  verifyBeforeDelivery: {
    ...
    /**
     * S1（07-P2-3）：e2e 测试文件 pattern（detect 命中 e2e 时跳过验证——e2e 慢，交付前只跑 unit）。
...
export function makeTestFileDetect(
  ...
  return (files: string[]) => {
    ...
    // S1（07-P2-3）：排除 e2e 测试文件——交付前只跑 unit
...
export const defaultPolicyConfig: PolicyConfig = {
  ...
  verifyBeforeDelivery: {
    ...
    // S1（07-P2-3）：e2e 测试 pattern——detect 命中 e2e 时不算「有可验证项」（e2e 慢，交付前只跑 unit）
...
// S1（07-P1-5）：TTL 刷新时间戳。多实例部署下其他实例改 policy 后，本实例在 TTL 内自动刷新。
let cachedAt = 0;
 ...
 * - 否则返回 cached。S1（07-P1-5）：超 TTL 时标记 stale（异步刷新，不阻塞同步返回）。
 */
 ...
 * - DB 空 / 读取异常 → fail-open 沿用 defaultPolicyConfig（policy 是治理便利，不是治理目的，§4.5）。
 */
export async function refreshPolicyConfigFromDB(): Promise<void> {
  ...
  try {
    ...
    cachedAt = Date.now(); // S1（07-P1-5）：更新刷新时间戳
  } catch (error) {
