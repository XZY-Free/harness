...
/**
 * Phase 4-1：hook 引擎——挂载在 Phase 2 的统一工具入口（executeToolRun）与交付闸门
 ...
 *
 * fail-open / fail-closed 边界（§4.5，两类不可混淆）：
 ...
 *
 * @deprecated P2 清理(07 Permission P2-1): V3.1 executeToolRun 已改用 evaluatePermission
 ...
 * 不是治理目的（§4.5）。command 为空则 no-op（未配置 formatter）。
 */
// S1（07-P2-2）：formatter 可用性缓存。首次失败（npx 退出非零，如 workspace 无 prettier）后
...
export async function runFormatOnWrite(
  ...
  // S1（07-P2-2）：formatter 不可用则跳过（避免每次写入 fork npx 失败的性能损耗）
  ...
  // P1-21 同类:拒 shell 元字符,防 `prettier --write x && rm -rf /` 链式注入
  const noMetachar = !/[;&|$\n`\\]/.test(cmd);
  const isAllowed =
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      // S1（07-P2-2）：formatter 不可用（如 ENOENT 127 / prettier 未装）→ 缓存 false，后续跳过
    ...
    // S1（07-P2-2）：spawn ENOENT → formatter 不可用，缓存 false
 ...
 */
/**
 * P1-21: verifyBeforeDelivery.command 白名单(纯函数,可单测)。
...
export async function runVerifyBeforeDelivery(
  ...
  // P1-21: verifyBeforeDelivery.command 白名单校验(与 formatOnWrite 对齐)。
