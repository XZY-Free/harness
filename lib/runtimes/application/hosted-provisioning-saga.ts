 ...
 * §08.3: Runtime 拆为 4 步 Gateway。
 * §08.6: Worker 每次只执行一个步骤，完成后保存 Checkpoint + 清除 Lease。
 * §08.10: Route Activation 返回路由详情。
 * §08.11: verify_route 验证精确 ID，非仅非空。
 */
...
export interface SagaConfig {
  ...
  /** 最大重试次数。 */
  maxAttempts: number;
  /** §08.8: 当前 Worker ID，用于 lease owner 校验。 */
  workerId: string;
}

/**
 * §08.4: 创建 Hosted 供应 Saga 执行器。
 *
...
export function createHostedProvisioningSaga(config: SagaConfig) {
  ...
  async function advanceStep(
    ...
    // §08.6: 成功非终态 → 保存 Checkpoint → currentStep=下一步 → state=pending → 清除 Lease
    await config.store.updateState({
      requestId: request.id,
      workerId: config.workerId,
      state: "pending",
  ...
  /** §08.4: validate_request — 校验请求冻结值完整性。 */
  ...
  /**
   * §08.2: ensure_agent_publication — Agent 发布，验证 revision 一致性。
   */
  ...
  /**
   * §08.3: prepare_runtime_revision — 准备 Runtime Revision。
   */
  ...
  /**
   * §08.3: verify_runtime_artifact — Runtime Artifact Attestation。
   */
  ...
  /**
   * §08.3: record_runtime_conformance — 记录 Runtime Conformance。
   */
  ...
  /**
   * §08.3: publish_runtime_revision — 发布 Runtime Revision。
   */
  ...
  /**
   * §08.10: activate_route — Route 激活，返回路由详情。
   */
