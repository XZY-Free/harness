...
/**
 * V3.7 Stage C：deliverySummary 聚合（plan §7 / §4.2）。
 *
 * 把 thread 的交付证据聚合成结构化 deliverySummary，对应 `delivery.succeeded` 事件 payload。
 * 不单独建 DeliverySummary 表（§4.2 决策：复用 ThreadEvent payload）。
 *
 * 数据来源：
 ...
 * - blindCommit：最近 gitCommit 之前是否读过 gitStatus/gitDiff（§9.2「commit 前必须读」软约束审计）。
 */
 ...
 *
 * S1（09-P2-2）：优先解析结构化 JSON（vitest --json / jest --json），回退正则。
 * 原仅正则解析，不同测试框架输出差异导致脆弱。
 */
function parseTestResults(stdout: unknown): DeliveryTestResults {
  const text = typeof stdout === "string" ? stdout : "";

  // S1（09-P2-2）：优先尝试解析结构化 JSON（vitest/jest --json 输出）
  try {
