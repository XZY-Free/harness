 ...
 *
 * 对每个 result，UPSERT 到 runtimeConformanceResultTable：
...
async function upsertOneResult(
  ...
  const id = randomUUID();
  await tx
    .insert(runtimeConformanceResultTable)
    .values({
      id,
...
export async function listConformanceResultsByRevision(
  ...
  return db
    .select()
    .from(runtimeConformanceResultTable)
    .where(eq(runtimeConformanceResultTable.runtimeRevisionId, runtimeRevisionId))
    .orderBy(asc(runtimeConformanceResultTable.caseId));
}
...
export async function getConformanceResult(
  runtimeRevisionId: string,
  caseId: string,
): Promise<RuntimeConformanceResult | null> {
  const [row] = await db
    .select()
    .from(runtimeConformanceResultTable)
    .where(
      and(
        eq(runtimeConformanceResultTable.runtimeRevisionId, runtimeRevisionId),
        eq(runtimeConformanceResultTable.caseId, caseId),
      ),
    )
    .limit(1);
  return row ?? null;
...
export async function deleteConformanceResultsByRevision(
  ...
  const result = await db
    .delete(runtimeConformanceResultTable)
    .where(eq(runtimeConformanceResultTable.runtimeRevisionId, runtimeRevisionId));
...
export type { RuntimeConformanceResult } from "@/lib/persistence/schema/runtime";
