 ...
 *   docker network 治理 + cgroup 硬配额（Stage B 网络策略实现后生效）。
 */
 ...
 * @param opts.secretMount 是否支持 secret 挂载（Stage C 启用前 false）。
...
export function buildCapability(opts: {
  ...
  imageVersion?: string;
  quota?: ResourceQuota;
  /** V3.8 Stage B：已解析的 per-thread 网络策略。 */
