...
import {
  appendInvocationStateEvent,
  appendTurnStateEvent,
  closeInvocationTerminalInTransaction,
} from "@/lib/executions/application/close-invocation-terminal";
    ...
    const state =
      event.type === "execution.completed"
        ...
        : event.type === "execution.failed"
          ...
          : "cancelled";
    if (!INVOCATION_TERMINAL_STATES.includes(invocation.executionState)) {
      // R05/R06 的终态收口链（Attempt / Turn + canonical 事件 / Ownership / Session /
      // Invocation + recoveryVersion 水位 / Job 桥）已抽成单一实现，与 Hosted Cancel
      // 共用同一条正式收口边界——见 `close-invocation-terminal.ts` 的模块注释。
      return closeInvocationTerminalInTransaction(tx, {
        invocation,
        attemptId,
        ownershipId,
        sessionBindingId,
        state,
        now,
        resultRef: typeof event.payload.resultRef === "string" ? event.payload.resultRef : null,
        resultDigest:
          typeof event.payload.resultDigest === "string" ? event.payload.resultDigest : null,
        errorCode: typeof event.payload.errorCode === "string" ? event.payload.errorCode : null,
