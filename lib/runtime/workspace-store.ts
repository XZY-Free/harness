...
/**
 * Phase 5 Stage A：HostWorkspaceStore——`WorkspaceStore` 的宿主实现。
 *
 ...
 *
 * V3.1：glob/grep 优先走 `rg`（host 直接 execa；container 经 docker exec seam）。
...
/**
 * Phase 5 Stage B：ContainerWorkspaceStore——容器模式的 WorkspaceStore。
 *
 ...
 *
 * V3.1：glob/grep override 为 `docker exec` 进容器跑 `rg`（经 docker-cli seam，复用
