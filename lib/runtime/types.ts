/**
 * Phase 5 Stage A：三层 runtime 稳定 interface（蓝图 §8.2）。
 *
 * Phase 2 Stage D 把三层 runtime 职责分散到三个文件，但按 §13.3「单一实现不抽」显式拒绝
 * 抽 interface。Phase 5 第二个 runtime 实现（容器）出现，该前提失效，抽象不再空壳——
 * 本文件定义三个稳定 interface，host / container 实现都满足之。
 *
 * 设计取舍（Stage A 零行为变更）：
 * - 方法签名贴合现有 `lib/workspace` / `lib/preview/manager` / `tools.ts` 的 execa 逻辑，
 *   而非方案 §2.2 草案签名（read→Buffer / write→{bytes} 等）。零回归优先于签名理想化；
...
export interface ExecutionRuntime {
   ...
   * 执行命令。
   *
   * V3.8：增 `logCapBytes`——per-thread 日志体积配额(只能收紧)。
  ...
  exec(
    ...
    opts?: {
      timeoutMs?: number;
      logCapBytes?: number;
      /** S1（02-P1-8）：流式回写回调，逐块接收 stdout/stderr（同时仍返回完整缓冲结果）。 */
...
export interface PreviewHandle {
  ...
  port: number;
  kind: PreviewKind;
  /** S1（05-P2-8）：静态预览鉴权 token（内部探活/反代转发时带回）。 */
  token?: string;
}
...
export interface PreviewStatus {
  ...
  port?: number;
  kind?: PreviewKind;
  /** S1（05-P2-8）：静态预览鉴权 token（反代转发时带回，防 host 内进程直读 workspace）。 */
  token?: string;
}
 ...
 *
 * Stage A 零行为变更：`start` 仍返回 `http://localhost:{port}/`（相对化在 Stage D）。
 */
...
export interface RuntimeHandle {
  ...
  /** V3.8：runtime 能力上报（审计 + UI 可见 + 部署决策依据）。 */
  capability: RuntimeCapability;
}

// ─── V3.8: Runtime Capability / Network Policy / Resource Quota ──
...
export interface ResourceQuota {
  ...
  /**
   * S1 修复（02-P1-6）：容器 rootfs 磁盘配额（bytes）。
 ...
 * - secretMount: 是否支持 secret 挂载注入(Stage C 启用)。
