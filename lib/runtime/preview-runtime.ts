...
/**
 * Phase 5 Stage A：StaticPreviewRuntime——`PreviewRuntime` 的静态文件实现。
 *
 ...
 *
 * Stage A 仍返回 `http://localhost:{port}/`（相对化在 Stage D）。
 * DevServerPreviewRuntime（容器内 dev server）在 Stage C 提供第二实现。
 */
...
function createStaticServer(threadId: string, token: string): Server {
  return createServer(async (req, res) => {
    // S1（05-P2-8）：鉴权——?token= / X-Preview-Token 头 / preview-token cookie 任一匹配即可。
    ...
    // S1 修复（02-P2-4）：复用 workspace.safeJoin（lstat+realpath symlink 防护）替代自做词法检查 +
...
// ─── DevServerPreviewRuntime（Phase 5 Stage C）─────────────────────────
...
/**
 * Phase 5 Stage C：DevServerPreviewRuntime——容器内 spawn `npm run dev` + ready 探测。
 *
 * 补齐 Phase 2 未达成的「static + dev-server 双实现」。经同一 PreviewRuntime interface 调用。
 *
 * start 流程：
 * 1. 已 ready → 复用返回
...
export class DevServerPreviewRuntime implements PreviewRuntime {
  ...
  async start(threadId: string): Promise<PreviewHandle> {
    ...
    // S1（05-P1-6）：dev server 输出重定向到 bind mount 日志文件，启动失败时可 tail 诊断
  ...
  async stop(threadId: string): Promise<void> {
    ...
    // S1 修复（02-P1-4）：先回收该 thread 的后台任务（dev server 是后台任务之一），再停删容器，
...
/**
 * S1 修复（02-P2-1）：删除未使用的 `devServerPreviewRuntime` 单例导出。
 *
