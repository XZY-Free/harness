...
export interface RunParams {
  threadId: string;
  /**
   * V7 S1-3：预创建的 ThreadRun id（由 DB 事实源生成）。
  ...
  /**
   * P1-2 完整化:streamText 失败时标记当前 endpoint 失败(熔断计数)。
...
interface LiveRun {
  ...
  /**
   * V7 S1-4：streamText onFinish 捕获的 token 用量，markDone 写回 ThreadRun 时消费。
...
/**
 * RunBroadcaster：一个 run 的多播 + 历史回放 + V7 S5-2/S5-3 持久化与续传。
 *
...
class RunBroadcaster {
  ...
  publish(chunk: UIMessageChunk): void {
    ...
    // V7 S5-2/S5-3：异步落 DB（best-effort，失败不阻断），sequence 由 broadcaster 前置分配，
    // 保证内存流顺序与 DB 一致。
...
export function enqueue(params: RunParams): string {
  const { threadId, modelMessages, system, modelId, tools, getChatModel, markEndpointFailed } =
    params;
  // V7 S1-3：runId 优先用 DB 预创建的 ThreadRun id，未传时回退生成（向后兼容）。
...
async function driveRun(run: LiveRun, params: RunParams): Promise<void> {
  ...
  // S1（07-P1-1）：读 thread.projectId，注入 per-thread project scope
  ...
  // V7 S2-2：注入 runId scope，executeToolRun 创建 ToolRun 时自动归属 ThreadRun。
    ...
    execute: ({ writer }) => {
      const result = streamText({
        ...
        onError: ({ error }) => {
          ...
          // P1-2 完整化:标记当前 endpoint 失败(熔断计数,超阈值切备用 endpoint)
          markEndpointFailed?.();
          // S1（04-G9）：streamText 失败时取消该 thread 活跃子代理，防子代理继续跑到 120s 超时
          //（onFinish 可能不触发，子代理无收尾）。best-effort 不阻塞 error 处理。
          stopAllSubagents(threadId).catch(() => {});
          // P1-9: 直接落终态——onFinish 在 onError 后不保证触发,若不主动 markFailed,
        ...
        onFinish: async ({ totalUsage }) => {
          ...
          if (totalUsage) {
            ...
            // V7 S1-4：捕获 token 用量到 LiveRun，供 markDone 写回 ThreadRun。
            run.totalUsage = {
  ...
  try {
    ...
    run.finalizeResolve?.();
    // S1（07-P1-1）：清除 per-thread project/skill scope
    clearThreadProjectScope(threadId);
    clearThreadSkillScope(threadId);
    // V7 S2-2：清除 per-thread runId scope
...
async function markDone(run: LiveRun): Promise<void> {
  ...
  // V7 S1-4：写回 ThreadRun 终态（completed + token 用量）。fail-open 不阻塞流。
...
async function markFailed(run: LiveRun, error: unknown): Promise<void> {
  ...
  // V7 S1-4：写回 ThreadRun 终态（failed + error 信息）。fail-open 不阻塞流。
  await failThreadRun(
    run.runId,
...
export async function cancelRun(runId: string, reason = "user_cancelled"): Promise<void> {
  ...
  // S1（04-G9）：用户停止/abort 时取消该 thread 活跃子代理（防 orphan 继续跑到 120s）
...
/** 订阅 run 的 SequencedChunk 流（带历史回放 + V7 S5-3 sequence）。run 不存在或已结束 → null。 */
...
/** P1-6: 查询 run 当前 SSE 订阅者数（0 表示无活跃视图,可考虑回收）。 */
