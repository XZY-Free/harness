 ...
 * - identityMode 当前只允许 none/bearer（external）；workload_token 仅限 Hosted /
...
/**
 * 协议中立认证语义。Transport 负责把 auth 映射为 HTTP header：
...
export type OutboundRuntimeAuthErrorKind =
  | "identity_mode_invalid" // identityMode 当前不支持（external 侧只允许 none/bearer）
...
/** 当前允许的 external identityMode。 */
const EXTERNAL_IDENTITY_MODES = new Set(["none", "bearer"]);

/**
 * 解析 external outbound auth（ 精确规则）。
 *
...
export async function resolveOutboundRuntimeAuth(params: {
  ...
  if (!EXTERNAL_IDENTITY_MODES.has(params.identityMode)) {
    throw new OutboundRuntimeAuthError(
      ...
      `External Runtime 不支持 identityMode=${params.identityMode}（当前仅 none/bearer）`,
    );
  }
  ...
  if (ref.lifecycleState !== "active") {
    // revoked/rotated：fail closed，不自动切换其他 CredentialRef。
    throw new OutboundRuntimeAuthError(
...
/**
 * 把 RuntimeTransportAuth 映射为 HTTP header。
 *
 * - none：完全不发送 Authorization；
...
export function outboundAuthHeaders(
  ...
  // External Runtime 收到 workload_token：本地 fail closed，不发网络。
  throw new OutboundRuntimeAuthError(
    "identity_mode_invalid",
