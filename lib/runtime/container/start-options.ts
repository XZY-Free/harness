...
type ContainerStartOptions = {
  ...
  /** S1（02-P2-5）：额外 docker run 参数（如 --add-host）。 */
  extraArgs?: string[];
};
...
export async function prepareContainerStartOptions(args: {
  ...
  return {
    ...
    secretsCache,
    // S1（02-P1-9）：cleanup 改 no-op——文件跨 exec 复用，由 cleanupSecretFileCache 在容器停止时清理
    cleanup: async () => {},
  };
}

/**
 * S1 修复（02-P1-9）：清理 thread 的缓存 secret env 文件（容器停止时调）。
