...
export interface RunContainerOpts {
  ...
  cpus: string;
  env: string[];
  /** V3.8：docker network 模式（如 "none"）。undefined=默认 bridge。 */
  networkMode?: string;
  /** V3.8：额外 docker run 参数（如 --add-host）。 */
  extraArgs?: string[];
  /** V3.8：secret env 文件路径（--env-file，不写命令行防泄露）。 */
  envFile?: string;
  /** S1（04-G2）：进程数上限 → docker --pids-limit。0/undefined=不限。 */
  pidsLimit?: number;
  /** S1（04-G2）：文件描述符上限 → docker --ulimit nofile。0/undefined=不限。 */
  openFilesLimit?: number;
  /** S1（02-P1-6）：容器 rootfs 磁盘配额（bytes）→ docker --storage-opt size=。0/undefined=不限。 */
...
export async function runContainer(opts: RunContainerOpts): Promise<string> {
  ...
  // V3.8：网络策略 --network（disabled=none；open/allowlist=默认 bridge 不加）
  if (opts.networkMode) {
    args.push("--network", opts.networkMode);
  }
  // V3.8：secret env 文件（--env-file，不写命令行防泄露）
  if (opts.envFile) {
    args.push("--env-file", opts.envFile);
  }
  // S1（04-G2）：进程数 + 文件描述符硬限（cgroup，无需 root）。
  ...
  // S1（02-P1-6）：容器 rootfs 磁盘配额（需 overlay2/devicemapper 配额支持，不支持时 docker 报错）
 ...
 *  Phase 5 Stage C：供 DevServerPreviewRuntime 启动长驻 dev server（npm run dev）。 */
 ...
 */
/** 最小 shell 单引号转义（供容器内路径/命令传参，防注入）。S1 修复（02-P2-6）。 */
...
export async function execDetachedWithPid(
  ...
  // S1 修复（02-P2-6）：pidFile/logPath 经 shQuote 转义（taskId 虽为 UUID 安全，但路径拼接入 shell
...
export async function execInContainer(
  ...
  // V3.8：logCapBytes 只能收紧（与 MAX_OUTPUT 取 min），不放宽。
