...
describe("RunnerSigningIdentityRegistry", () => {
 describe("validate", () => {
  ...
  it("已撤销 Key → runner_key_revoked", () => {
   const registry = new RunnerSigningIdentityRegistry([
    makeEntry({ keyId: "key-a", runnerIdentity: "runner-1", revokedAt: "2026-01-01T00:00:00.000Z" }),
   ]);
   const result = registry.validate({
    keyId: "key-a",
    runnerIdentity: "runner-1",
  ...
  it("过期 Key → runner_key_expired", () => {
   const registry = new RunnerSigningIdentityRegistry([
    makeEntry({ keyId: "key-a", runnerIdentity: "runner-1", validUntil: "2025-12-31T23:59:59.000Z" }),
   ]);
   const result = registry.validate({
    keyId: "key-a",
    runnerIdentity: "runner-1",
  ...
  it("尚未生效 Key → runner_key_expired", () => {
   const registry = new RunnerSigningIdentityRegistry([
    makeEntry({ keyId: "key-a", runnerIdentity: "runner-1", validFrom: "2027-01-01T00:00:00.000Z" }),
   ]);
   const result = registry.validate({
    keyId: "key-a",
    runnerIdentity: "runner-1",
  ...
  it("同一 Runner 合法轮换 Key → 成功", () => {
   const registry = new RunnerSigningIdentityRegistry([
    makeEntry({ keyId: "key-old", runnerIdentity: "runner-1", validUntil: "2026-06-30T23:59:59.000Z" }),
    makeEntry({ keyId: "key-new", runnerIdentity: "runner-1", validFrom: "2026-07-01T00:00:00.000Z" }),
   ]);
   // 用新 Key 验证
   const result = registry.validate({
    keyId: "key-new",
 ...
 describe("getActivePublicKeys", () => {
  it("仅返回未撤销且在有效期内的公钥", () => {
   const registry = new RunnerSigningIdentityRegistry([
    ...
    makeEntry({ keyId: "key-revoked", publicKey: "pk-revoked", revokedAt: "2026-01-01T00:00:00.000Z" }),
    makeEntry({ keyId: "key-expired", publicKey: "pk-expired", validUntil: "2025-12-31T23:59:59.000Z" }),
    makeEntry({ keyId: "key-future", publicKey: "pk-future", validFrom: "2027-01-01T00:00:00.000Z" }),
   ]);
