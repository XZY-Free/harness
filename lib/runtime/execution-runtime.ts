...
/**
 * Phase 5 Stage A：HostExecutionRuntime——`ExecutionRuntime` 的宿主实现。
 *
...
// ─── V3.2 Stage B：后台长跑能力（plan §6） ───────────────────
//
// 后台方法放在本文件而非 types.ts：types.ts 当前有未提交的并行 V3.1 Stage C 改动
...
/**
 * 后台能力接口（Host/Container 均实现）。Stage C 工具经此调用 startBackground/stopBackground。
...
export class HostExecutionRuntime implements BackgroundCapableExecutionRuntime {
  /** V3.8：懒加载的 secret env（首次 exec 时解析，缓存后续复用）。 */
  private secretsCache?: SecretEnvMap;

  constructor(
    private readonly threadId: string,
    /** V3.8：per-thread 资源配额（作为上限，只能收紧；undefined=零回归）。 */
    private readonly quota?: ResourceQuota,
    /** V3.8：secret 解析回调（懒加载，首次 exec 时调）。 */
  ...
  async exec(
    ...
    // V3.8：timeout 取 caller opts 与 quota 的 min（只能收紧，无 quota 零回归）。
    ...
    // V3.8：logCap 取 caller opts / quota / MAX_OUTPUT 的 min（只能收紧）。
    let cap = MAX_OUTPUT;
    ...
    // V3.8：懒加载 secret env（首次 exec 时解析）
    ...
    try {
      ...
      // S1（04-G2）：host Linux 用 prlimit 施加 nofile/nproc 软限（非 Linux 原样返回）。
      const limited = wrapWithHostRlimits(command, this.quota);
      // S1（02-P1-2）：host Linux bwrap 沙箱（config-gated，默认 off，bwrap 不可用 fail-open）。
      ...
      const subprocess = execa(sandboxed, {
        ...
        // P1 修复(02-P1-1):env 白名单过滤,防 AI 命令 printenv 泄露平台 secret。
      ...
      // S1（02-P1-8）：流式回写——caller 传 onChunk 时逐块推送 stdout/stderr
      if (opts?.onChunk) {
  ...
  async startBackground(command: string, opts: BackgroundStartOpts): Promise<BackgroundHandle> {
    ...
    // S1（04-G2）：host Linux 用 prlimit 施加 nofile/nproc 软限（包裹在重定向之前，限额作用于 prlimit 进程及其子进程）。
    // S1（02-P1-2）：host Linux bwrap 沙箱（外层包裹 prlimit 命令，config-gated）。
    const limited = wrapWithHostRlimits(command, this.quota);
    const sandboxed = await wrapWithHostSandbox(limited, cwd);
  ...
  /**
   * V3.2 Stage B：host 停止后台进程——tree-kill 整个进程组（SIGTERM → SIGKILL 兜底）。
   */
 ...
 * 递归收集 pid 的全部后代。
 *
 * S1 修复（02-P2-7）：原仅用 `pgrep -P`，pgrep 不可用时静默返回空（漏杀孙子进程）。
...
/**
 * Phase 5 Stage B：ContainerExecutionRuntime——`ExecutionRuntime` 的容器实现。
 *
...
export class ContainerExecutionRuntime implements BackgroundCapableExecutionRuntime {
  /** V3.8：懒加载的 secret env（首次 exec 时解析，写入 --env-file，缓存复用）。 */
  private secretsCache?: SecretEnvMap;

  constructor(
    private readonly threadId: string,
    /** V3.8：per-thread 资源配额（container 模式硬配额 docker --memory/--cpus）。 */
    private readonly quota?: ResourceQuota,
    /** V3.8：per-thread 网络策略（container 模式 docker network 治理）。 */
    private readonly networkPolicy?: NetworkPolicy,
    /** V3.8：secret 解析回调（懒加载，首次 exec 时调，写入 --env-file）。 */
    private readonly secretResolver?: () => Promise<SecretEnvMap>,
  ) {}
  ...
  async exec(
    ...
    try {
      ...
      try {
        ...
        // V3.8：quota 的 timeoutMs/logCapBytes 作为上限（只能收紧）。无 quota 时透传 opts（零回归）。
        let execOpts = opts;
  ...
  /**
   * V3.2 Stage B：container 后台启动长跑进程。
   *
  ...
  /**
   * V3.2 Stage B：container 停止后台进程——读取 PID 文件精确 kill。
...
async function stopContainerBackgroundByPid(
  ...
  // P1-13: 兜底 pkill -f——fallbackCommand 含 taskId(UUID,唯一),pkill -f 精确匹配命令行整串,
