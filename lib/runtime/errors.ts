 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §6.2-§6.4、§6.13、
 ...
 *
 * 事实源：本阶段不报错，Turn 保持 accepted 等待后续接入 Runtime。
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §6.9、§14 §2.1 规则 4。
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §6.9。
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §6.10（InvocationCommand 表）、
 *         ../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.7-3.10。
 * 映射 404 RESOURCE_NOT_FOUND（隐藏式，不泄露存在）。
 */
 ...
 *
 * 事实源：§6.10（invocationId 可空，queued 后由调度器绑定）。
 * 映射 404 RESOURCE_NOT_FOUND（隐藏式，不泄露存在）。
 */
 ...
 *
 * 事实源：../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.10（Resume）、§6（生命周期）。
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §6.2（终态不可恢复）、§13（Worker 失联恢复）。
 ...
 * 的非终态 Invocation 可重调度）；§6.2（lost 为终态不可恢复）。
 *
