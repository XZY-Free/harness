...
/**
 * V3.2 Stage A：BackgroundTask registry——进程内 Map + DB 双层（蓝图 §12 V3.2 / plan §4 / §5）。
 *
 ...
 * - `markRunning` → DB `running` + `task.started` 事件（Stage B 的 startBackground 在拿到 pid 后调）。
 ...
 *
 * stop 回调（tree-kill / pkill）由 Stage B 的 ExecutionRuntime.stopBackground 经
...
/** S1 修复（02-P2-3）：新增 process_exit，区分「进程退出清理」与「thread 结束」。 */
...
interface LiveEntry {
  ...
  command: string;
  /** Stage B 注入的真实 stop 回调；缺省时 markStopped 只更 DB。 */
  stop?: StopHandle;
}
 ...
 *
 * S1 修复（02-P1-5）：对带 pid 的孤儿进程 best-effort 发 SIGTERM（防资源泄漏）。
 * P1-14: kill 前读 /proc/{pid}/cmdline 校验 pid 仍是同一进程(防 PID 复用误杀非自己起的进程)。
  ...
  for (const t of orphans) {
    if (t.runtimeType === "host" && typeof t.pid === "number" && t.pid > 0) {
      try {
        ...
        // P1-14: 校验 pid 仍是同一进程,防 PID 复用误杀
...
/** 进程退出时停止所有 active 后台任务（best-effort，不阻塞退出）。S1（02-P2-3）：reason=process_exit。 */
...
/**
 * Stage B 注入真实 stop 回调（tree-kill / pkill）。
 *
 * S1 修复（02-P2-9）：增加 threadId 校验——只允许该 taskId 所属 thread 注入 stop handle，
