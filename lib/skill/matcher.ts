 ...
 *
 * 本表与 skillStopwordsConfig.custom 合并,运行时通过 stopwordSet() 取合并集。
...
/**
 * 用 Intl.Segmenter（Node 原生 ICU 分词,支持 zh,无依赖）对 CJK 文本做真分词。
 *
...
export function extractKeywords(description: string | null | undefined): string[] {
  ...
  for (const w of cnWords) {
    ...
    keywords.add(w);
    // 4+ 字 CJK 段额外 bigram(Segmenter 偶尔切得过细,bigram 兜底召回)
    if (w.length >= 4) {
