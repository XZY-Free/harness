...
async function buildTransport(config: McpServerConfig): Promise<McpTransport> {
 ...
 // : command 白名单,防 host 侧 spawn 任意命令(curl /bin/sh 等)
 assertMcpCommand(config.command);
 const { StdioClientTransport } = await import("@modelcontextprotocol/sdk/client/stdio.js");
 // env 隔离——仅传最小系统 env(PATH/HOME) + server 配置 env，不继承全量 process.env
 ...
 // : SSRF 守卫——拒内网/元数据/非 http(s) URL + DNS rebinding 校验
...
export async function connectServer(
 ...
 // 记录 server 版本 + 能力协商到 DB（原仅落日志,日志轮转丢失不可追溯）。
 ...
 * 结果走 a oversized 摘要（由 executeToolRun / package-builder 自动处理）。
 *
