...
// S1（10-P1-2）：MCP callTool 结果结构校验 schema。
...
// S1（10-P1-4）：listTools TTL 缓存（60s），避免每次 listMcpTools 都 JSON-RPC 往返
const listToolsCache = new Map<string, { tools: unknown[]; expiry: number }>();
const LIST_TOOLS_TTL_MS = 60_000;

/**
 * V3.4 MCP 工具：listMcpTools / callMcpTool（蓝图 §5.4）。
 *
 * 工具名归一 `mcp.<server>.<tool>` 作为 permissionKey（依赖 Stage A 多前缀修复）。
 ...
 *
 * S1（10-P2-2）：allowedTools 参数级约束——若 server 配置的 allowedTools 含本工具的
...
export function mcpEvaluate(args: {
  ...
  /** S1（10-P2-2）：server 的 allowedTools 配置（解析后注入 allow 规则）。 */
  ...
  // S1（10-P2-2）：从 allowedTools 派生 allow 规则（参数级约束）
...
/**
 * S1（10-P2-2）：从 allowedTools 配置派生 allow 规则。
 *
...
/** S1（10-P2-2）：argMatcher 匹配（与 engine.matchArg 等价，但用于 mcpEvaluate 层）。 */
...
/** S1（10-P2-2）：allowedTools 列表中是否有 entry 的 toolName 段匹配给定工具名。 */
  ...
  return {
        ...
        try {
          return await executeToolRun(threadId, "listMcpTools", { server }, async (signal) => {
            ...
            for (const s of target) {
              try {
                // S1（10-P1-4）：TTL 缓存——60s 内复用 listTools 结果
                ...
                for (const t of tools) {
                  ...
                  // S1（10-P2-2）：allowedTools 参数级约束——工具名匹配（解析 entry 取 toolName 段）
    ...
    callMcpTool: tool({
      ...
      execute: async ({ server, tool: toolName, args = {} }) => {
        ...
        // S1（10-P2-2）：预先查 server 配置拿 allowedTools，构造带参数级约束的 evaluate 闭包
        ...
        try {
          return await executeToolRun(
            ...
            async (signal) => {
              // P1-4 兼容：runner 闭包内自处理连接/调用错误，转成 {ok:false} 返回，
              ...
              // S1（10-P1-2）：结果结构校验——content 必须是数组，每项须含 type 字段。
