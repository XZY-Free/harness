...
/**
 * V3.4 MCP server registry（蓝图 §5.4）。
 *
...
/** Studio/API 返回时对 env 做脱敏（secret 值替换为 ***），不落明文。S1（10-P2-1）：扩展关键词。 */
...
// V6-M3-3（B7）：ping TTL 缓存——60s 内不重复 ping，减少 listTools 开销
const pingTimestamps = new Map<string, number>();
const PING_TTL_MS = 60_000;

// S1（10-P2-4）：per-server 调用计数 + 限流（默认 10 req/min）
...
export async function getOrConnect(name: string, deps?: McpClientDeps): Promise<McpClient> {
  ...
  // V6-M3-3（B7）：onclose 回调——server 进程退出时主动清理池条目
...
/** S1（10-P2-4）：调用前检查限流。供 callMcpTool 使用。 */
...
/** 关闭全部 client（进程退出清理，对齐 V3.2 closeAllBackgroundTasks 模式）。 */
