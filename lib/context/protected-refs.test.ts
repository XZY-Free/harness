...
// ─── pinned-facts store（P0 后改 DB 持久化，mock db） ──────────

import { vi } from "vitest";

const pinnedStore = new Map<string, string[]>();

vi.mock("@/lib/db/queries", () => ({
  getThreadById: vi.fn(async (id: string) =>
    pinnedStore.has(id) ? { id, pinnedFacts: pinnedStore.get(id) } : null,
  ),
  updateThreadPinnedFacts: vi.fn(async (id: string, facts: string[] | null) => {
    if (facts === null || facts.length === 0) pinnedStore.delete(id);
    else pinnedStore.set(id, facts);
  }),
  // S1（03-P2-3）：mock 事务化读-改-写（模拟 SELECT FOR UPDATE + UPDATE 的原子效果）
  mutateThreadPinnedFacts: vi.fn(
    async (id: string, mutator: (cur: string[]) => string[] | null) => {
      const current = pinnedStore.has(id) ? [...(pinnedStore.get(id) as string[])] : [];
      const next = mutator(current);
      if (next === null || next.length === 0) pinnedStore.delete(id);
      else pinnedStore.set(id, next);
      return next ?? [];
    },
  ),
}));
...
import {
  _clearPinnedFactsForTest,
  addPinnedFact,
  getPinnedFacts,
  removePinnedFact,
} from "./pinned-facts";

describe("pinned-facts store（DB 持久化）", () => {
  it("add / get / remove（去重）", async () => {
    await _clearPinnedFactsForTest("t-store");
    await addPinnedFact("t-store", "用 Tailwind");
    await addPinnedFact("t-store", "用 Tailwind"); // 去重
    await addPinnedFact("t-store", "端口 3000");
    expect(await getPinnedFacts("t-store")).toEqual(["用 Tailwind", "端口 3000"]);
    await removePinnedFact("t-store", "用 Tailwind");
    expect(await getPinnedFacts("t-store")).toEqual(["端口 3000"]);
  });

  it("空 thread → 空数组", async () => {
    await _clearPinnedFactsForTest("t-empty");
    expect(await getPinnedFacts("t-empty")).toEqual([]);
  });
});
