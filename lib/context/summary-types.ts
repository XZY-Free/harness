...
/**
 * S1 修复（03-P1-1）：从一段消息中提取工具调用证据（toolName + 关键入参 + 结果状态）。
 *
...
export type TurnSummary = {
  ...
  /**
   * S1 修复（03-P1-1）：本轮工具调用证据（toolName + 入参摘要 + 结果状态）。
...
export function extractTurnSummary(args: {
  ...
  // S1 修复（03-P1-2）：取最后一条**含非空 text part** 的 user 消息。
...
export function extractDecisionLog(args: {
  ...
  // S1 修复（03-P2-6）：信心改为加权打分（替代原三档粗糙推断）。
...
export type SubagentSummary = {
  ...
  /** 证据引用（transcriptPath / outputArtifactId）。transcript 本身不回传，只引用。 */
  evidenceRefs: string[];
  /** 建议交接（下一步）。 */
  handoff: string;
  /** 结构化摘要正文（落库/注入父 context layer）。 */
  text: string;
};

/**
 * 从 SubagentRun 生成结构化 subagent 摘要（V3.5 填充 V3.3a 占位 slot）。
 *
 * 纯函数、确定性、不调 LLM。只读 run 字段产出 summary；**transcript 不进摘要**，
 * 只引用 transcriptPath/outputArtifactId（父 context layer 只见 summary，不见 transcript）。
 *
 * run 为 null/undefined（无子代理）→ 返回空摘要（pending 语义：无可汇总）。
 */
export function extractSubagentSummary(
  ...
  const resultSummary = run.resultSummary ?? null;
  const evidenceRefs = [run.transcriptPath, run.outputArtifactId].filter(
  ...
  const handoff =
    status === "completed"
      ...
      : status === "failed" || status === "timed_out"
        ? `子代理未成功（${run.errorMessage ?? status}），父 agent 需重试或换路径`
        : status === "cancelled"
          ? "子代理被取消，父 agent 决定是否重新派生"
          : "";
  ...
  const lines = [
    ...
    `目标: ${goal}`,
    `状态: ${status}`,
    resultSummary ? `结果: ${resultSummary}` : null,
    evidenceRefs.length > 0 ? `证据: ${evidenceRefs.join(", ")}` : null,
    handoff ? `交接: ${handoff}` : null,
  ].filter((l): l is string => Boolean(l));
