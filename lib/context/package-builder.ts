...
/**
 * S1 修复（03-P1-4）：microcompact —— 裁剪超大 text / tool-result part。
 *
 * V6-M2-4（C1）扩展：除 text part 外，也裁剪 tool-result part（JSON output 超 cap 时截断）。
  ...
  const out = history.map((m) => {
    const parts = (m.parts ?? []).map((p) => {
      if (p && typeof p === "object") {
        ...
        // V6-M2-4: tool-result part 裁剪（output 超 cap token 时截断）
        if (type === "tool-result") {
...
export async function buildContextPackage(
  ...
  // S1 修复（03-P1-3）：预热真 tokenizer（gpt-tokenizer o200k_base），后续 sync estimateTokens
  ...
  // V3.3b Stage C：memory 注入 + 预算裁剪（纯加性；无 memory 时不改变任何路径）。
  const memories = input.memories ?? [];
  const hasMemory = memories.length > 0;

  // S1 修复（03-P1-4）：软警告线 microcompact。
  ...
  // S1（06-P2-6）：按 scope 分组 + 组内按 kind 排序，提升可读性（原平铺数组顺序）。
  ...
  // S1 修复（03-P1-5）：高置信 user/project 记忆纳入 protected 不裁（类似 pinned facts），
  ...
  // 零回归路径：无任何触发且无 memory → 逐字直通（与 V3.3a 逐字一致）。
  ...
  const sections: string[] = ["[历史上下文摘要（V3.3a 压缩派生视图，原始消息未删除）]"];
