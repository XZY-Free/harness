 ...
 *
 * S1 修复（03-P1-3 升级真 tokenizer）：原 CJK 友好的 char 估算（CJK=1, ASCII=char/4）只是
 ...
 *
 * S1 修复（03-P2-2）：原对非文本 part 用 `JSON.stringify(whole part)` 高估（含 type/toolCallId/
...
/**
 * S1 修复（03-P2-5）：上下文窗口可视化状态（供 Studio 实时展示占用/阈值）。
 *
