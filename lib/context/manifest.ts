...
/**
 * V3.0 Stage C：Context Manifest 生成与持久化。
 *
 * 每次模型调用前构建一个 `ContextManifest`，结构化记录「本轮模型看到了哪些来源」。
 * V3.0 只记录来源与摘要，**不存完整 prompt / 用户消息正文 / 完整工具输出**（§6.2 隐私约束）。
 *
...
export type ContextManifestInput = {
  ...
  /** V7 S2-1：归属 ThreadRun（由 route.ts 传入预创建的 runId）。 */
  runId?: string | null;
  ...
  /**
   * V3.3a：本轮 token 预算（`Infinity` 表示无配置/永不压缩）。可选，不破坏 V3.0 调用方。
   * 写入 manifest 供审计（不参与 V3.0 manifest 计算）。
   */
  tokenBudget?: number;
  /**
   * V3.3a：本轮装配应用的 ContextSummary id 列表（由 package builder 传入）。可选。
   * 空数组/省略 = 未压缩。写入 manifest 供审计与 Studio 展示。
   */
  appliedSummaryIds?: string[];
  /**
   * V3.3b Stage 0：buildContextPackage 的真实装配 manifest 摘要。
   *
   * 传入后 manifest 的 protectedRefs/excludedCandidates/compressed/afterTokens 与真实模型输入
   * 一致（不再用静态默认）；未传入时保持 V3.0 静态行为（零回归）。
  ...
  /**
   * V3.4：本轮访问的外部资料来源（webFetch/webSearch/searchDocs/MCP）。
   * 非空时填 external layer（sourceId="external.fetch"），记录 sourceUrl/contentHash 列表供审计/可解释；
   * 空/省略时保持 V3.0 默认 sourceId="none"（零回归）。
  ...
  /**
   * V3.3b Stage C：本轮注入的长期记忆（填 memory layer；空时 sourceId:none 零回归）。
...
export type ContextManifest = {
  ...
  estimatedTokens: number;
  /**
   * V3.3b Stage 0：本轮是否压缩装配（来自 buildContextPackage 的真实 manifest）。
   * 与真实模型输入一致——非静态推断，避免 manifest 与装配脱节。
   */
  compressed: boolean;
  /**
   * V3.3b Stage 0：本轮装配后真实模型输入 token（压缩后 ≠ estimatedTokens）。
...
/**
 * S1 修复（03-P2-4）：token 估算统一到 budget.ts。
 *
 ...
 * - external / memory：V3.0 默认空
 *
 * protectedRefs：最新用户指令、当前 plan/todo 等不可压缩来源。V3.0 标记 instructions 与
...
export function buildContextManifest(input: ContextManifestInput): ContextManifest {
  ...
  // external layer：V3.4 填实。非空 externalSources → sourceId="external.fetch"，
  // inline 记 sourceUrl/contentHash 列表（只摘要，不塞原文）；空 → 保持 V3.0 默认 "none"（零回归）。
  ...
  // memory layer：V3.3b 填实。非空 memories → sourceId="memory.store"，inline 记 id/scope/kind/score/reason/status；
  // 空 → 保持 V3.0 默认 "none"（零回归）。
  ...
  // V3.3b Stage 0：有 packageManifest 时用真实 protected/excluded（与装配一致）；
  // 否则保持 V3.0 静态默认（零回归）。protectedRef.kind → layer 全归 thread（protected
  ...
  return {
    ...
    checksums,
    estimatedTokens,
    // V3.3b Stage 0：与真实模型输入一致（来自 packageManifest；未传则零回归默认）。
    compressed: pkg?.compressed ?? false,
    afterTokens: pkg?.afterTokens ?? null,
    // V7 S2-1：归属 ThreadRun 透传。
...
// （§6.2 / Stage C 验收：manifest 写入失败不会让 /api/chat 直接 500）。
