...
/**
 * V3.7 Stage A：checkpoint 创建 / 列表 / 恢复（plan §5）。
 *
...
/** 生成 `snow-checkpoint-{12 字符 shortId}`。S1（09-P2-5）：12 字符降碰撞概率，碰撞时重试。 */
...
export async function createCheckpoint(
  ...
  // S1（09-P2-4）：取 diff 摘要（变更文件列表，供回滚前快速判断 checkpoint 内容）
 ...
 *
 * S1（09-P1-3）：restore 前检查工作区脏状态——有未提交改动时抛错拒绝（防数据丢失），
...
export async function restoreCheckpoint(
  ...
  // S1（09-P1-3）：脏检查——有未提交改动时拒绝 restore（防数据丢失）
