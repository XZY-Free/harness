...
/**
 * Stage D：CI/CD webhook 交接（部署触发/状态查询/回滚）。
 *
...
async function fetchWithRetry(
  ...
  // : SSRF 守卫——防 webhook URL 被配为内网/元数据端点,secret 透传+写操作。
  // 含 DNS rebinding 校验(域名解析到内网/元数据 IP)。
  await assertSafeExternalUrlResolved(url, "cicd webhook");
  // : 注册 cicd token 到全局脱敏 registry,4xx body 经 redactTextGlobal 替换,
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (response.status >= 400 && response.status < 500) {
          ...
          // :body 仅进服务端日志,不入 error message——防恶意 webhook 在 4xx 响应体
          // 回显 Authorization 头值,经 errorMessage 落 deployment 表 + deployment.failed 事件泄露 token。
          // : body 经 redactTextGlobal 替换已注册的 cicd token。
...
export async function triggerDeploy(params: TriggerDeployParams): Promise<CicdDeployResponse> {
  ...
  // : cicdJobId 来自外部 CI/CD 响应,后继拼入 status URL。校验字符集 + 拒绝路径穿越,
 ...
 *
 * 支持 per-thread token（与 triggerDeploy 一致）。
...
export async function queryStatus(
  ...
  // : cicdJobId 已在 triggerDeploy 校验字符集;此处额外 encodeURIComponent 防注入
  const url = deployConfig.cicdStatusUrl.replace("{jobId}", encodeURIComponent(cicdJobId));
  const headers: Record<string, string> = {};
  // : per-thread token 优先于全局（与 triggerDeploy 一致）
