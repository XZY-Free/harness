...
export const toolCallTable = mysqlTable(
  "ToolCall",
  {
  ...
  (t) => ({
    invocationSequenceUq: uniqueIndex("ToolCall_invocation_callSequence_uq").on(
      t.invocationId,
      t.callSequence,
    ),
    toolOperationUq: uniqueIndex("ToolCall_tool_operationId_uq").on(t.toolId, t.operationId),
    tenantInvocationIdx: index("ToolCall_tenant_invocation_idx").on(t.tenantId, t.invocationId),
    tenantToolStateIdx: index("ToolCall_tenant_tool_state_idx").on(
      t.tenantId,
      t.toolId,
      t.callState,
    ),
...
export type ToolCall = InferSelectModel<typeof toolCallTable>;
export type NewToolCall = InferInsertModel<typeof toolCallTable>;

// ─── CapabilityReview ────────────────────────────────────

export const capabilityReviewTable = mysqlTable(
  "CapabilityReview",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
  ...
  (t) => ({
    tenantStateCreatedIdx: index("CapabilityReview_tenant_state_created_idx").on(
      t.tenantId,
      t.reviewState,
      t.createdAt,
    ),
    tenantResourceIdx: index("CapabilityReview_tenant_resource_idx").on(
      t.tenantId,
      t.resourceType,
      t.resourceId,
    ),
...
export type CapabilityReview = InferSelectModel<typeof capabilityReviewTable>;
export type NewCapabilityReview = InferInsertModel<typeof capabilityReviewTable>;
