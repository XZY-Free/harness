...
/**
 * Invocation 表：一次 AgentRevision + RuntimeRevision 的执行（§6.2 L366-387）。
 *
 * 关键约束：
...
/**
 * ExecutionBinding 表：一条 Invocation 恰有一条不可变绑定（§6.3 L405-423）。
 *
 * 关键约束：
...
/**
 * InvocationAttempt 表：整个 Invocation 的基础设施重调度（§6.4 L389-403）。
 *
 * 关键约束：
...
/**
 * ExecutionOwnership 表：Invocation 执行权管理（§6.13 L516-523）。
 *
 * 关键约束：
...
/**
 * RuntimeSessionBinding 表：Runtime 维护的会话引用（§6.11 L506-508）。
 *
 * 关键约束：
...
/**
 * RuntimeEventIngress 表：Runtime 回传候选事件的持久批次账本（§6.9 L486-500）。
 *
 * 关键约束：
...
/**
 * RuntimeConformanceResult 表：RuntimeRevision 一致性测试结果（§6.x + 15-machine-contracts §5 L94-110）。
 *
 * 关键约束：
 ...
 * - capabilities 必须来自探测和一致性测试，管理员不能手工勾选未支持能力。
 */
export const runtimeConformanceResultTable = mysqlTable(
  "RuntimeConformanceResult",
  {
    id: varchar("id", { length: 36 })
      .primaryKey()
    ...
    runtimeRevisionId: varchar("runtimeRevisionId", { length: 36 })
      .notNull()
      .references(() => runtimeRevisionTable.id),
    tenantId: varchar("tenantId", { length: 36 })
      .notNull()
      .references(() => tenant.id),
  ...
  (t) => ({
    revisionCaseUq: uniqueIndex("RuntimeConformanceResult_revision_case_uq").on(
      t.runtimeRevisionId,
      t.caseId,
    ),
    revisionIdx: index("RuntimeConformanceResult_revision_idx").on(t.runtimeRevisionId),
    casePassedIdx: index("RuntimeConformanceResult_case_passed_idx").on(t.caseId, t.passed),
  }),
);

export type RuntimeConformanceResult = InferSelectModel<typeof runtimeConformanceResultTable>;
export type NewRuntimeConformanceResult = InferInsertModel<typeof runtimeConformanceResultTable>;
