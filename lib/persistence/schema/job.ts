...
export const jobTable = mysqlTable(
  ...
  (t) => ({
    tenantAgentIdx: index("Job_tenant_agent_idx").on(t.tenantId, t.agentId),
    tenantStateIdx: index("Job_tenant_state_idx").on(t.tenantId, t.jobState),
    tenantThreadIdx: index("Job_tenant_thread_idx").on(t.tenantId, t.threadId),
    tenantReplacesIdx: index("Job_tenant_replaces_idx").on(t.tenantId, t.replacesJobId),
    tenantTypeStateIdx: index("Job_tenant_type_state_idx").on(t.tenantId, t.jobType, t.jobState),
  }),
);
...
/**
 * JobEvent 表：Job 事件 Outbox（§6.1 文末、§9.2）。
 *
 * 关键约束：
 ...
 * - JobEvent 不出现在员工 Thread SSE；只有 job_result projection 才进入 ThreadEvent。
 * - 与 ThreadEvent 平行承担各自事件流，复用 ProjectionCheckpoint/EventDeliveryFailure。
 */
export const jobEventTable = mysqlTable(
  ...
  (t) => ({
    jobSequenceUq: uniqueIndex("JobEvent_job_sequence_uq").on(t.jobId, t.eventSequence),
    jobIdempotencyUq: uniqueIndex("JobEvent_job_idempotency_uq").on(t.jobId, t.idempotencyKey),
    tenantJobIdx: index("JobEvent_tenant_job_idx").on(t.tenantId, t.jobId),
    tenantJobInvocationIdx: index("JobEvent_tenant_job_invocation_idx").on(
      t.tenantId,
      t.jobId,
      t.invocationId,
    ),
    tenantJobOccurredIdx: index("JobEvent_tenant_job_occurred_idx").on(
      t.tenantId,
      t.jobId,
      t.occurredAt,
    ),
...
export const jobCommandTable = mysqlTable(
  ...
  (t) => ({
    jobIdempotencyUq: uniqueIndex("JobCommand_job_idempotency_uq").on(t.jobId, t.idempotencyKey),
    tenantJobStateIdx: index("JobCommand_tenant_job_state_idx").on(
      t.tenantId,
      t.jobId,
      t.commandState,
    ),
    tenantReplacementIdx: index("JobCommand_tenant_replacement_idx").on(
      t.tenantId,
      t.replacementJobId,
    ),
  }),
...
/**
 * JobResultProjection 表：job_result ThreadItem 的权威关联对象（§5.4）。
 *
 * 关键约束：
 * - itemId 唯一外键 → ThreadItem(id)（一对一）。
 * - jobId 外键 → Job(id)。
