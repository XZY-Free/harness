...
} from "@/lib/conversations/shared-budget-queries";
...
describe("S09-C07 getSharedBudgetUsage（DB 集成）", () => {
  ...
  it("多 sibling 聚合 budgetUsedJson", async () => {
    ...
    await db
      ...
      .where(eq(threadRelationTable.id, nth(relationIds, 0)));
    await db
      .update(threadRelationTable)
      .set({
        budgetUsedJson: {
      ...
      .where(eq(threadRelationTable.id, nth(relationIds, 1)));
  ...
  it("child_count 聚合超限抛 SharedBudgetExhaustedError", async () => {
    ...
    for (const relId of relationIds) {
      await db
        ...
        .where(eq(threadRelationTable.id, relId));
    }
