 ...
 *
 * 关键约束：
 * - 创建/删除 PendingInput 不生成 user_message Item（、）。
 * - admitted/removed 不可编辑/删除/重排（行 339）。
...
import { pendingInputTable, threadTable } from "@/lib/persistence/schema/conversation";
...
/** 投影 PendingInput 行为 list item。 */
function projectListItem(row: PendingInput): PendingInputListItem {
 return {
 id: row.id,
...
export async function listPendingInputs(
 ...
 .select({ id: threadTable.id, queueVersionNo: threadTable.pendingQueueVersionNo })
 .from(threadTable)
 ...
 const rows = await db
 .select()
 .from(pendingInputTable)
 .where(
 and(eq(pendingInputTable.threadId, threadId), eq(pendingInputTable.inputState, "pending")),
 )
 .orderBy(asc(pendingInputTable.queuePosition));
 ...
 *
 * 关键约束：
 * - 创建 PendingInput 不生成 user_message Item（）。
...
export async function createPendingInput(params: {
 ...
 .select({ maxPos: sql<string>`COALESCE(MAX(${pendingInputTable.queuePosition}), 0)` })
 .from(pendingInputTable)
 .where(
 and(
 eq(pendingInputTable.threadId, params.threadId),
 eq(pendingInputTable.inputState, "pending"),
 ),
 );
 ...
 await tx.insert(pendingInputTable).values({
 id: pendingInputId,
 ...
 .select()
 .from(pendingInputTable)
 .where(eq(pendingInputTable.id, pendingInputId))
 .limit(1);
 if (!created) {
...
export async function editPendingInput(params: {
 ...
 .select()
 .from(pendingInputTable)
 .where(eq(pendingInputTable.id, params.pendingInputId))
 .for("update")
 .limit(1);
...
export async function reorderPendingInputs(params: {
 ...
 .where(eq(pendingInputTable.id, id));
 }
 ...
 *
 * 关键约束：
 * - 删除 PendingInput 不生成 user_message Item（）。
 */
...
export async function getPendingInputById(
 ...
 const [row] = await db
 .select({ item: pendingInputTable })
 .from(pendingInputTable)
 .innerJoin(threadTable, eq(pendingInputTable.threadId, threadTable.id))
 .where(and(eq(threadTable.tenantId, tenantId), eq(pendingInputTable.id, pendingInputId)))
 .limit(1);
...
export async function findPendingInputByClientMessageId(
 ...
 eq(pendingInputTable.threadId, threadId),
 isNull(pendingInputTable.removedAt),
 eq(pendingInputTable.clientMessageId, clientMessageId),
 ),
 )
 .limit(1);
 return row?.item ?? null;
