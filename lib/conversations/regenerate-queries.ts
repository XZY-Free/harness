...
/** Regenerate 的 binding_mode 选项（行 425）。 */
...
/** 允许启动 Regenerate 的 Turn 状态集合（行 430：cancelled 不可恢复）。 */
...
export async function startRegeneration(params: {
 ...
 .where(
 and(
 eq(threadEventTable.invocationId, newInvocationId),
 eq(threadEventTable.eventType, "turn.regeneration_started"),
 ),
 )
 .limit(1);
 if (!eventRow) {
