...
/**
 * Employee Interaction API route handler 公共助手（S04-C03）。
 *
 * 事实源：docs/architecture/api-and-events.md §3（Employee Interaction API）、
 * （身份与授权：Employee API 走员工 SSO Session/OAuth Token）、
 * （公共请求头：Idempotency-Key / If-Match / X-Request-ID）、
 * （成功与错误格式）。
 *
 * 职责：
 * - resolveEmployeePrincipal：员工身份解析（resolvePrincipal + audience=employee）。
 * - parseThreadSettingsEtag：从 Thread 设置 ETag 提取 versionNo。
 * - schemaInvalidTable / etagMismatchTable：错误响应工具（与 admin route-helpers 对齐）。
 ...
 * - Agent 必须 enabled 且同租户；否则 404（不泄露存在，行 143）。
 */
