 ...
 *
 * 事实源：§14 §2.1 规则 4、§8 错误码表 EVENT_SEQUENCE_GAP。
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §5.6、
 *         ../v11-agentkit-platform/11-api-and-event-boundaries.md §3.6-3.10。
 ...
 *
 * 事实源：§5.6 行 339（admitted 输入不可编辑/删除）。
 ...
 * （不完整或包含非 pending id）。
 *
 * 事实源：§5.6、../v11-agentkit-platform/11-api-and-event-boundaries.md §3.8。
 * 映射 409 BUSINESS_CONSTRAINT_VIOLATION。
 */
 ...
 *
 * 事实源：§2.3（If-Match 头）、§2.5（412 ETAG_MISMATCH）。
 ...
 *
 * 事实源：../v11-agentkit-platform/02-agent-thread-and-runtime.md §3.7 行 366
 ...
 *
 * 事实源：../v11-agentkit-platform/10-core-data-model.md §5.2（ThreadRelation.source_turn_id）。
 * 映射 409 BUSINESS_CONSTRAINT_VIOLATION。
 */
...
/**
 * Child Thread 结果投影失败（05 文档 §9 行 412-417；12 文档 §4.2 行 263）。
 *
 * 触发场景：
 * - relation 不存在或跨租户不可见
...
/**
 * finalizeChildThreadCancellation 失败（05 文档 §16 行 322-333；12 文档 §4.3 行 294）。
 *
 * 触发场景：
