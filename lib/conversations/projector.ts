...
async function projectToThreadList(
 ...
 // 事实源：（PendingInput 表，不参与 thread_list_projection 字段）
 await tx
 .update(threadListProjectionTable)
 .set({
 latestEventSequence: event.eventSequence,
...
async function projectToTurnTimeline(
 ...
 // 事实源：（PendingInput 表，不参与 turn_timeline_projection 字段）
 ...
 *
 * 事实源：规则 5（Item 投影器按 Event sequence 幂等更新，checkpoint 只在事务提交后前移）。
 *
 * 流程：
 ...
 *
 * 事实源：规则 1（按流内 sequence 读取，不按 occurred_at 排序）。
