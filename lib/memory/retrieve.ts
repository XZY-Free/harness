...
/**
 * V3.3b Stage B：混合检索 lexical recall + semantic embedding rerank。
 *
 * 用户指令覆盖方案 §1 的纯关键词决策：
 ...
 *
 * 返回 { memories, lexicalCandidates, embedding }：embedding.status 供 Stage C 注入 manifest
...
// S1（06-P2-7）：scope/confidence 权重从 memoryConfig 读（env 可覆盖），原硬编码。
 ...
 *
 * S1（06-P2-1）：CJK 连续段原作为一个 token（"实现登录页" 单 token），中文召回率低。
...
export async function retrieveMemories(
  ...
  // S1（06-P1-2）：批量取 candidate 的 active embedding（单查询替代 N+1）。
  const candidateIds = [...candidatePool.keys()];
  const embMap = await listActiveEmbeddingRows(candidateIds, provider.name);
  // S1（06-P2-3）+ V6-M2-7：当前 provider 缺失的 candidate，批量 fallback 到任意 provider 的老 embedding
  ...
  // V3.3b Stage B §7 综合分（内部排序用，不进返回结构）：
