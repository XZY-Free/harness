...
/**
 * V3.3b Stage A：长期记忆 store（facade over queries）。
 *
 ...
 *
 * V3.3b Stage B：createMemory 末尾触发 indexMemory（去重命中也触发），对 normalized text 生成
...
export type CreateMemoryResult = {
  ...
  /** V3.3b Stage B：embedding 索引结果（诚实反映 disabled/stale/ready/error，不静默伪装）。 */
 ...
 *
 * V3.3b Stage B：写入/去重后触发 indexMemory（去重命中也触发——confidence 可能升高，
...
export async function createMemory(args: CreateMemoryArgs): Promise<CreateMemoryResult> {
  ...
  // V3.3b Stage B：触发 embedding 索引（去重命中也触发）。失败不阻断——返回 semanticStatus。
...
/**
 * S1 修复（06-P1-5）：更新 memory text（agent 写错时无需 revoke + 重建，保留 provenance 链）。
 *
 * S1（06-P1-1）：接通 markEmbeddingStale——text 变化时先标 stale 再 reindex，
