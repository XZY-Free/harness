...
// 断言 indexMemory 被调（去重命中也调），不干扰既有 memory.created/revoked 事件计数。
...
describe("createMemory 去重", () => {
  it("无重复 → 新建 + memory.created 事件（threadId 取自 provenance），deduplicated=false", async () => {
    ...
    expect(queries.appendThreadEvent).toHaveBeenCalledWith(
      "t1",
      "memory.created",
      expect.objectContaining({ memoryId: "m-new", scope: "project", kind: "convention" }),
    );
  ...
  it("命中重复 → 合并 provenance + confidence 取较高，deduplicated=true，不新建、不写事件", async () => {
...
describe("revokeMemory soft delete", () => {
  it("存在 → status=revoked + memory.revoked 事件", async () => {
    ...
    expect(queries.appendThreadEvent).toHaveBeenCalledWith(
      "t1",
      "memory.revoked",
      expect.objectContaining({ memoryId: "m-1", reason: "过时", revokedBy: "u1" }),
    );
  });

  it("不存在 → null，不写事件", async () => {
