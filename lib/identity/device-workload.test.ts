 ...
 * - resolver（workload）：resolveWorkloadPrincipal runtime/gateway/service。
 */
...
import {
  ...
  revokeDevice,
  touchDevice,
} from "@/lib/identity/device-queries";
import {
...
describe("resolver（workload）", () => {
  it("resolveWorkloadPrincipal runtime token 解析成功", () => {
    ...
    const principal = resolveWorkloadPrincipal(headers, "runtime");
  ...
  it("resolveWorkloadPrincipal 缺 token 抛 AuthenticationError", () => {
    expect(() => resolveWorkloadPrincipal(new Headers(), "runtime")).toThrow(AuthenticationError);
  });
